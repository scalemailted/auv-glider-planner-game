...
Raw source files belong under gitignored `external_data/reference_bathymetry/`. Compact ANCHOR runtime artifacts and `manifest.json` live under `assets/reference_bathymetry/`. The browser app reads only the compact global overview raster/metadata and staged regional patch artifacts; it does not download NOAA/GEBCO data or run Python at runtime. If a selected region is not staged, the browser exports a patch request instead of pretending to generate reference data. The global overview is a selection layer, not mission-resolution bathymetry. Mission-ready generation uses staged regional patches such as the ETOPO 2022 15 arc-second Monterey Canyon fixture. The global atlas allows arbitrary boundary selection, but live browser generation is budget-gated. Oversized selections can be exported as patch requests, but they are not generated live in Alpha. See [Reference Bathymetry Data Pipeline](reference_bathymetry_data_pipeline.md).

The Alpha boundary budget estimates selected lon/lat bounds against source resolution, source cells, preview mesh size, field-grid size, depth layers, time frames, and expected public benchmark-bundle size. `OK` regions are within the live browser budget, `WARN` regions are near the browser limits or require decimation, and `BLOCKED` regions cannot generate live but can still export a patch request when the selected bounds are otherwise valid.

The focused Playwright workflows write the REF-ATLAS-UX-R1.2 owner package under `artifacts/owner-review/ref-atlas-ux-r1-2/` by default, or an `ANCHOR_E2E_OWNER_REVIEW_DIR` override. The first ten screenshots cover full-world global atlas default, reset view, land/ocean visibility, Monterey overlay on the full world, Monterey focused view, loaded patch workspace, non-staged bbox patch request, generated bathymetry, generated fields, and Planning launch readiness. `qa-summary.json` records default/reset viewport, global overview bounds/aspect, patch coverage counts, Monterey overlay normalized/screen bounds, selected/matched/loaded fixture IDs, overview digest, bathymetry/field/environment digests, hidden-truth status, raw-path exclusion, and unchanged simulation/scoring flags.
