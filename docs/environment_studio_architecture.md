...
Raw source files belong under gitignored `external_data/reference_bathymetry/`. Compact ANCHOR runtime artifacts and `manifest.json` live under `assets/reference_bathymetry/`. The browser app reads only the compact global overview raster/metadata and staged regional patch artifacts; it does not download NOAA/GEBCO data or run Python at runtime. If a selected region is not staged, the browser exports a patch request instead of pretending to generate reference data. The global atlas overview is a low-resolution selection layer, not mission-resolution bathymetry. Mission-ready environments use staged high-resolution regional patches such as the ETOPO 2022 15 arc-second Monterey Canyon fixture. See [Reference Bathymetry Data Pipeline](reference_bathymetry_data_pipeline.md).
