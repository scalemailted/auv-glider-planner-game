...
The focused Playwright workflows write the REF-ATLAS-UX-R1.2 owner package under `artifacts/owner-review/ref-atlas-ux-r1-2/` and the REF-ATLAS-INTERACT-R1.3 operational-window package under `artifacts/owner-review/ref-atlas-operational-window-r1/` by default, or an `ANCHOR_E2E_OWNER_REVIEW_DIR` override. R1.3 evidence covers default atlas view, tiny-click normalization, regional window selection, Gulf-scale multi-tile status, multi-tile patch-request export, Monterey fixture load, generation pipeline, and Planning launch readiness. `qa-summary.json` records operational scale, budget status, multi-tile request digest, selected/matched/loaded fixture IDs, bathymetry/field/environment digests, hidden-truth status, raw-path exclusion, cleanup counts, and unchanged simulation/scoring flags.
