...
-> Global Atlas Staging Scene with patch coverage overlay
...
Environment Studio is a staged workflow. The Global Atlas Staging Scene is used to select and inspect an operational boundary window. Continue to 3D Bathymetry is enabled only when the selected operational window matches an app-hosted mission-ready tile set. Request-only and Gulf-scale regions can open a coarse regional preview for inspection, then route to patch request or multi-tile request export for offline staging. Coarse preview is explicitly not mission-ready and cannot generate fields, compose environments, launch Planning, or export official benchmark artifacts.
