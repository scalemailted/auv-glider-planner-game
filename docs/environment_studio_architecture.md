...
The focused Playwright workflows write the REF-ATLAS-UX-R1.2 owner package under `artifacts/owner-review/ref-atlas-ux-r1-2/`, the REF-ATLAS-INTERACT-R1.3 operational-window package under `artifacts/owner-review/ref-atlas-operational-window-r1/`, and the REF-ATLAS-INTERACT-R1.4 deep-zoom/editor package under `artifacts/owner-review/ref-atlas-zoom-window-r1/` by default, or an `ANCHOR_E2E_OWNER_REVIEW_DIR` override. R1.4 evidence covers full-world reset, deep Gulf zoom, typed operational-window editing, tiny-selection guidance, Gulf-scale multi-tile status, multi-tile patch-request export, Monterey fixture load, generation pipeline, and Planning launch readiness. `qa-summary.json` records max zoom, zoom reached, typed-window and tiny-selection status, operational scale, budget status, multi-tile request digest, selected/matched/loaded fixture IDs, bathymetry/field/environment digests, hidden-truth status, raw-path exclusion, cleanup counts, and unchanged simulation/scoring flags.
