...
One preprocessed public ETOPO fixture is currently checked in. The bundled manifest is labeled `AVAILABLE` and contains a Monterey Canyon `lowResolutionReferencePatch` at 60 arc-second source resolution, 90 columns x 72 rows. Environment Studio must present it as low-resolution reference bathymetry and must not claim a 15 arc-second mission-ready regional patch until that source is actually staged. It must not be presented as calibrated survey bathymetry, certified navigation data, HYCOM, Copernicus, an operational ocean forecast, or a validated real-world ocean product.
