...
-> Global Atlas Setup Scene with patch coverage overlay
-> Regional 3D Bathymetry Workspace after a staged hosted tile is selected
-> Generated Reference Environment after bathymetry and fields are generated
```

Environment Studio has two stages. The Global Atlas Setup Scene is for selecting and inspecting an operational boundary. Every valid boundary can open an interactive 3D bathymetry preview. If high-resolution staged tiles are not available, the preview uses app-hosted overview/LOD data and is not mission-ready. Field generation, Planning launch, and benchmark export require staged mission-ready bathymetry tiles. Continue to Mission-Ready Bathymetry is enabled only when the selected operational window matches an app-hosted mission-ready tile set. Request-only and Gulf-scale regions route to patch request or multi-tile request export for offline staging.
...
ENV-STUDIO-HANDOFF-R0 captures the product direction in [Environment Studio Product Handoff](environment_studio_product_handoff.md). The Global Atlas Setup Scene now exposes a curated-region selector for seed operational windows and a bathymetry mode scaffold. Curated presets place editable boundaries and report source, bounds confidence, staging status, available-now state, and staging requirements in the right inspector. Bathymetry modes are explicit: Real Reference uses `referenceRaster` authority and is the current implemented path; Reference-Enhanced Synthetic uses planned `enhancedSyntheticRaster` authority; Fully Synthetic Sandbox uses planned `syntheticRaster` authority. Display modes remain separate from simulation bathymetry authority, and the selector does not silently change mission authority.

The active source mode is `referenceBathymetryAtlas`, and the default stage is `globalAtlasSelector`. The manifest distinguishes a global overview artifact (`role=globalOverview`, 60 arc-second source, world bounds, compact raster preview) from regional fixtures (`role=missionReadyPatch` or `lowResolutionReferencePatch`). The `anchor.reference-bathymetry-atlas` artifact records source dataset identity, provider/version/resolution/unit metadata, overview bounds/digest, patch coverage overlays, layer summaries, provenance, claim boundaries, and `atlasDigest`. The selected `anchor.reference-bathymetry-window` records lon/lat bounds, fixture availability, local projection metadata, output/preview resolution, sampled depth/wet-land/slope statistics, detected tags, validation, provenance, claim boundaries, and `patchDigest`.
...
Raw source files belong under gitignored `external_data/reference_bathymetry/`. Compact ANCHOR runtime artifacts and `manifest.json` live under `assets/reference_bathymetry/`. ANCHOR hosts curated app-ready reference bathymetry tiles as static assets. The browser does not download NOAA/GEBCO data at runtime. External public bathymetry data is downloaded and preprocessed offline, then registered as staged ANCHOR tile artifacts in `assets/reference_bathymetry/tile-library-manifest.json`. The Pages build copies the staged tile-library manifest, Monterey tile assets, mesh LODs, global overview artifacts, and loader files into `_site`. The raster/grid artifact remains authoritative for bathymetry sampling and environment generation. Derived low-poly meshes are visualization/inspection artifacts only. The browser app reads only the compact global overview raster/metadata, staged regional patch artifacts, and tile-library metadata; it does not download NOAA/GEBCO data or run Python at runtime. If a selected region is not staged, the browser opens an adaptive-LOD interactive 3D preview and exports a patch request instead of pretending to generate reference data. The global overview is a selection layer, not mission-resolution bathymetry. Mission-ready generation uses staged regional patches such as the ETOPO 2022 15 arc-second Monterey Canyon fixture. The atlas supports deep zoom and operational-window editing. The atlas operational window can be moved and resized directly. Drag inside the rectangle to move it, drag edges to resize one side, and drag corners to resize both connected sides. Large operational windows are valid selections, but live Alpha generation remains budget-gated; oversized regions export patch or multi-tile requests. Large Gulf-scale windows can be selected, previewed interactively with adaptive LOD, and exported as multi-tile patch requests, but live Alpha generation remains limited to staged browser-safe regional patches. Oversized but valid Gulf-scale and basin-scale windows are selectable, report `MULTI_TILE_REQUIRED`, and export `anchor.reference-bathymetry-multitile-patch-request`; they are not generated live in Alpha. The deterministic `gulf_segment_demo_15s` request-only region crosses the expected 30N ETOPO tile boundary and is used as multi-tile staging-contract coverage. See [Reference Bathymetry Data Pipeline](reference_bathymetry_data_pipeline.md).
...
The Alpha boundary budget estimates selected lon/lat bounds against source resolution, source cells, preview mesh size, field-grid size, depth layers, time frames, and expected public benchmark-bundle size. `OK` regions are within the live browser budget, `WARN` regions are near the browser limits or require decimation, `MULTI_TILE_REQUIRED` regions are valid operational windows that require offline tiling, and `BLOCKED` regions cannot generate live because the selection itself is invalid or unsupported for Alpha.

The focused Playwright workflows write the REF-ATLAS-UX-R1.2 owner package under `artifacts/owner-review/ref-atlas-ux-r1-2/`, the REF-ATLAS-INTERACT-R1.3 operational-window package under `artifacts/owner-review/ref-atlas-operational-window-r1/`, the REF-ATLAS-INTERACT-R1.4 deep-zoom/editor package under `artifacts/owner-review/ref-atlas-zoom-window-r1/`, and the REF-ATLAS-BOX-EDIT-R1 rectangle editor package under `artifacts/owner-review/ref-atlas-box-edit-r1/` by default, or an `ANCHOR_E2E_OWNER_REVIEW_DIR` override where supported. REF-TILE-LIB-R1A.1 writes the static tile-library alpha-readiness package under `artifacts/owner-review/ref-tile-lib-r1a/`. ENV-STAGING-SCENE-R1 writes the staged-scene owner package under `artifacts/owner-review/env-staging-scene-r1/`; ENV-STAGING-SCENE-R1.1 writes CTA-gating evidence under `artifacts/owner-review/env-staging-scene-r1-1/`; MULTITILE-OPAREA-R1 writes Gulf-scale coarse-preview evidence under `artifacts/owner-review/multitile-oparea-r1/`; ENV-BATHY-PREVIEW-R1 writes any-valid-boundary preview evidence under `artifacts/owner-review/env-bathy-preview-r1/`; ENV-STUDIO-ALPHA-UX-R2 writes simplified-atlas and interactive-bathymetry-preview evidence under `artifacts/owner-review/env-studio-alpha-ux-r2/`; ENV-STUDIO-UX-POLISH-R2.1 writes viewport-cleanup, dynamic-panel, camera-control, return-state, atlas-LOD, and Monterey-regression evidence under `artifacts/owner-review/env-studio-ux-polish-r2-1/`. R1.4 evidence covers full-world reset, deep Gulf zoom, typed operational-window editing, tiny-selection guidance, Gulf-scale multi-tile status, multi-tile patch-request export, Monterey fixture load, generation pipeline, and Planning launch readiness. REF-ATLAS-BOX-EDIT-R1 evidence covers direct rectangle draw, hover handles, move, edge resize, corner resize, large-window budget state, Gulf multi-tile request behavior, Monterey fixture load, generation pipeline, and Planning launch readiness. R1A.1 evidence covers hosted tile-library loading, Monterey mesh LOD non-authority, benchmark export, browser runtime fetch safety, Gulf requestOnly behavior, and cleanup counts. ENV-STAGING-SCENE-R1.1 evidence covers no-selection disabled Continue, Gulf disabled Continue and Load, Gulf multi-tile request primary action, not-staged patch request primary action, Monterey Continue, and RegionalBathymetryScene transition. ENV-BATHY-PREVIEW-R1 evidence covers valid not-staged preview, Gulf preview, multi-tile request availability from preview, Monterey staged preview, and field-generation path availability after bathymetry confirmation. ENV-STUDIO-ALPHA-UX-R2 evidence covers the simplified left panel, right inspector ownership, Gulf-scale adaptive-LOD 3D preview, camera presets, multi-tile request action, Monterey staged 3D preview, and field-generation path availability after bathymetry confirmation. ENV-STUDIO-UX-POLISH-R2.1 evidence covers a clean terrain-dominant regional viewport, hidden/collapsed unavailable coarse actions, right-inspector ownership, rotate/pan/wheel/reset/top-down/oblique camera controls, return-to-atlas state preservation, atlas LOD status, and unchanged simulation/scoring/planner/field-equation flags. `qa-summary.json` records max zoom, zoom reached, typed-window and tiny-selection status, operational scale, budget status, multi-tile request digest, selected/matched/loaded fixture IDs, bathymetry/field/environment digests, hidden-truth status, raw-path exclusion, cleanup counts, and unchanged simulation/scoring flags.
