...
`packages/mission-simulator` owns deterministic mission-state contracts, mission input identity, canonical state snapshots, events, observations, raw metric summaries, and selected pure helpers. It consumes `packages/environment` artifacts and does not generate scientific fields. Browser ANCHOR still owns Play/Pause scheduling, Phaser lifecycle, Three.js presentation, route editing, official scoring, and replay playback. See `docs/mission_simulator_package.md`.