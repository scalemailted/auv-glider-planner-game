...
Current Environment Studio status: contracts, validation helpers, digests, hidden-truth checks, and documentation are defined, and the browser Studio opens to a staged Reference Bathymetry Atlas workflow. Environment Studio has two stages. The Global Atlas Setup Scene is for selecting and inspecting an operational boundary. It includes a Curated Region selector that places editable seed windows for Monterey Canyon, Hawaii, Puerto Rico, Florida Straits, Gulf Shelf/Canyon, Northeast US Shelf Break, California Shelf Break, and Alaska Fjord/Shelf without downloading data or claiming mission readiness unless staged app-hosted tiles exist. Every valid boundary can open an interactive 3D bathymetry preview. If high-resolution staged tiles are not available, the preview uses app-hosted overview/LOD data and is not mission-ready. Field generation, Planning launch, and benchmark export require staged mission-ready bathymetry tiles. The Bathymetry Mode selector records the current Real Reference authority and planned Reference-Enhanced Synthetic / Fully Synthetic Sandbox modes without switching unimplemented generation paths. Continue to Mission-Ready Bathymetry is enabled only when the selected operational window matches an app-hosted mission-ready tile set. The Regional Bathymetry Workspace uses the center panel as an interactive 3D bathymetry viewport. Detailed provenance, bounds, LOD, and launch status are shown in the right inspector. The left panel is dynamic and only shows actions available for the current scene mode. Returning to the atlas preserves the selected operational boundary and map viewport. High-zoom atlas detail uses app-hosted overview/LOD assets only; the browser does not download NOAA/GEBCO data at runtime. ANCHOR uses compact preprocessed public bathymetry/topography fixtures as the default source; raw NOAA/GEBCO files are not runtime assets and belong under gitignored `external_data/reference_bathymetry/`. ANCHOR hosts curated app-ready reference bathymetry tiles as static assets. The browser does not download NOAA/GEBCO data at runtime. External public bathymetry data is downloaded and preprocessed offline, then registered as staged ANCHOR tile artifacts. The Pages build delivers the staged tile-library manifest, Monterey rasters, Monterey mesh LODs, global overview artifacts, and loader files. The regional 3D mesh is a decimated visualization artifact. The reference raster/grid remains authoritative for bathymetry sampling, masks, environment generation, simulation, and benchmark export. The checked-in manifest is labeled `AVAILABLE` with a compact ETOPO 2022 60 arc-second global overview selection layer, an ETOPO 2022 15 arc-second Monterey Canyon `missionReadyPatch` from the NOAA surface-elevation non-ice fallback tile, and a preserved ETOPO 2022 60 arc-second `lowResolutionReferencePatch`. The static tile library stages those Monterey rasters and keeps Gulf request-only until offline preprocessing is run, including the deterministic `gulf_segment_demo_15s` staging-contract region. The browser starts at the Global Atlas Setup Scene, overlays staged patch coverage, opens adaptive-LOD interactive 3D preview for valid unstaged regions, opens the mission-ready Regional 3D Bathymetry Workspace for hosted mission-ready selections, and only then generates the reference-derived environment. The procedural synthetic world/globe workflow remains an experimental sandbox / compatibility path. BATHY-DATA-R1 owner-review evidence is written to `test-results/bathy-data-r1-owner-review/` by the focused browser workflow. REF-ATLAS-UX-R1.2 owner acceptance evidence is written to `artifacts/owner-review/ref-atlas-ux-r1-2/` or an explicit local override. REF-TILE-LIB-R1A.1 owner acceptance evidence is written to `artifacts/owner-review/ref-tile-lib-r1a/` by the tile-library alpha-readiness audit. ENV-STAGING-SCENE-R1 owner acceptance evidence is written to `artifacts/owner-review/env-staging-scene-r1/` by the staged-scene audit. ENV-STAGING-SCENE-R1.1 owner acceptance evidence is written to `artifacts/owner-review/env-staging-scene-r1-1/` by the staged-scene CTA audit. MULTITILE-OPAREA-R1 owner acceptance evidence is written to `artifacts/owner-review/multitile-oparea-r1/` by the multi-tile operational-area audit. ENV-BATHY-PREVIEW-R1 owner acceptance evidence is written to `artifacts/owner-review/env-bathy-preview-r1/` by the any-valid-boundary preview audit. ENV-STUDIO-ALPHA-UX-R2 owner acceptance evidence is written to `artifacts/owner-review/env-studio-alpha-ux-r2/` by the alpha UX readiness audit. ENV-STUDIO-UX-POLISH-R2.1 owner acceptance evidence is written to `artifacts/owner-review/env-studio-ux-polish-r2-1/` by the R2.1 UX polish audit. The current implemented path is public reference bathymetry patch -> bathymetry artifact -> deterministic synthetic bathymetry-conditioned fields -> package-backed environment artifact -> validated Planning launch -> public benchmark bundle. Launch warnings are classified, non-blocking warnings may allow launch, and blocking warnings or failures prevent launch. Generated currents, scalars, hotspots, and hazards remain deterministic synthetic benchmark artifacts, not operational forecast products. Mission simulation, official scoring, planner behavior, benchmark fairness, and existing Alpha workflows remain unchanged. See `docs/feedback_r1_environment_studio.md`, `docs/environment_studio_architecture.md`, `docs/environment_studio_product_handoff.md`, and `docs/reference_bathymetry_data_pipeline.md`.
