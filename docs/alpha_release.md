...
Initial tracked items are `ALPHA-FB-001` Environment Studio authoring, `ALPHA-FB-002` Researcher Notebook Launchpad, `ALPHA-FB-003` hosted Google Colab smoke pending, `ALPHA-FB-004` feedback/diagnostic taxonomy, `ALPHA-FB-006` regional 3D bathymetry authoring, `ALPHA-FB-008` Synthetic Ocean Atlas operational-window selection, `ALPHA-FB-009` procedural atlas field engine plus window-conditioned bathymetry generation, and `ALPHA-FB-010` package-backed synthetic current/scalar/hotspot regeneration. P0/P1 items must not be ignored.
...
Current Environment Studio status: contracts, validation helpers, digests, hidden-truth checks, and documentation are defined, and the browser Studio opens to a Synthetic Ocean Atlas stage. Users select an operational window, inspect inferred regional context and current/scalar regime hints, generate a Regional Mission Recipe, then generate deterministic regional 3D bathymetry detail with a window-conditioned builder. FIELD-REGEN-R1 adds an explicit Generate Currents & Science Fields action that records compact package-backed synthetic current, scalar, and hotspot metadata with diagnostics and digests. The atlas and regenerated fields are synthetic; they are not a real Earth map, HYCOM, Marine Copernicus, calibrated ocean products, operational forecasts, ecological forecasts, or navigation data. Mission launch from edited Studio projects, sculpting, real patch import, scientific equation changes beyond the package-backed synthetic generators, scoring changes, and planner changes remain out of scope. See `docs/feedback_r1_environment_studio.md` and `docs/environment_studio_architecture.md`.
