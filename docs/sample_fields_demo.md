...
Guided examples also expose model-aware initial-condition controls. Curated Seed uses the fixture unchanged, Interactive Canvas lets students edit generation 0 with a fixed model-specific brush palette, and Deterministic Random Seed creates a seeded starting state without introducing unseeded randomness. Conway exposes default/mixed, block, blinker, glider, and deterministic-random fixtures; Forest Fire, SIR, Greenberg-Hastings, Sandpile, Wa-Tor, Traffic CA, Wireworld, and ocean analogs expose valid state brushes for their fixed rule family. Canvas edits reset the generation index to 0, keep playback paused, and are exported as initialCondition metadata with mode, fixture, brush, edit count, generation, and edited cells. Ocean analog editing remains an event/process-layer teaching surface, not physical advection.
