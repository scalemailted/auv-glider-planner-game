...
The Sample / ROI Field Demo organizes `S(x,y,t)` behavior into Temporal Pattern, Spatial Pattern, and Evolution Model. This separates prior-agnostic baselines such as static hotspots or direct periodic functions from evolutionary processes such as growth/decay, diffusion, neighbor activation, moving features, recovery, and forecast drift. Current-dependent sample behavior belongs primarily in the Coupled Fields Demo, which overlays `F(x,y,t)` and `S(x,y,t)`.
