...
- `Demos`: Flow Fields Demo, Sample / ROI Field Demo, and Coupled Fields Demo.
...
The Sample / ROI Field Demo organizes `S(x,y,t)` behavior into Temporal Pattern, Spatial Pattern, State Model, and Process Model. State Model separates Time-Indexed / Memoryless fields, State-Evolving / Markovian processes, and History-Aware / Non-Markovian behavior. A field can be dynamic and still memoryless when it is computed directly from `x`, `y`, and `t`; diffusion or neighbor activation are state-evolving examples; revisit recovery is history-aware when sampling or observation history matters. Current-dependent sample behavior belongs primarily in the Coupled Fields Demo, which overlays `F(x,y,t)` and `S(x,y,t)`.
