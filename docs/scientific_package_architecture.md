...
## SIM-PKG-R2 Mission Simulator Package Note

`packages/mission-simulator` now owns authoritative mission-state transitions, manifests, inputs, normalized state, command-result contracts, physics and route-progress helpers, environment sampling, events, observations, terminal evaluation, raw metrics, snapshots, and result digests. It consumes canonical `EnvironmentArtifact` identity from `packages/environment`. It does not own environment generation, route planning, route editing, official score aggregation, Play/Pause scheduling, Three.js rendering, Phaser lifecycle, or replay playback. Browser, headless, and benchmark adapters share the same package authority while preserving existing public behavior for R2. See `docs/mission_simulator_package.md` and `packages/mission-simulator/MODEL_CARD.md`.