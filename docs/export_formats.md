...
## `anchor.headless.roundtrip-report.json`

`type: "anchor.headless.roundtrip-report"` is the H3 report for solver-packet / submitted-plan / Node-headless bundle compatibility. It records packet identity, plan identity, selected agent, visibility validation, plan validation, adapted runtime-plan metadata, headless episode summary, output files, hidden-truth leak checks, and explicit boundary flags.

The report can be written as `roundtrip_report.json` next to a headless bundle and embedded in `bundle.json` as `roundtripReport`. Public default roundtrips omit hidden fields and keep `T_hiddenTruth` out of solver-visible and browser-summary artifacts. The report documents an educational headless execution; it is not official browser scoring and does not add a route planner or Python simulator.
