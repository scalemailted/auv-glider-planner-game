...
`type: "anchor.syntheticRoiScenario"` is the compact seeded scenario export from the Sample / ROI Field Demo Scenario Generation card. It is separate from `anchor.demo.sample-roi-field`: demo artifacts preserve the current visualization snapshot or bounded demo time window, while synthetic ROI scenarios package the active reference signature, custom component recipe, or legacy preset compatibility metadata as a validated time-series process for notebooks and teaching replay.
