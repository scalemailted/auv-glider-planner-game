...
anchor.result may include terrainAwareValidation and compact terrainValidationMetadata blocks. These preserve launch readiness, issue codes, severity counts, boundary flags, compact actual terrain diagnostics, and planned-versus-actual comparison data without duplicating full bathymetry grids or terrain meshes. Result exports may also include compact public terrainEvents generated by canonical Simulation. The metadata is public-safe, does not expose hidden truth, and does not change official scoring. Launch validation is a frozen prediction; runtime terrain diagnostics are based on canonical actual state.

## Mission Editor Export Boundary
