...
- THREE-R1.1 stabilizes the Three Mission Workspace runtime: renderer startup errors are captured in `ANCHOR_MISSION_RENDER_DEBUG`, pointer conversion uses the actual canvas rect in CSS pixels, and drop-zone deployment selection goes through the canonical selected-start path. THREE-R1.1C repairs end-to-end waypoint placement after deployment and standardizes mouse gestures: left click for the active planning action, left drag pan, right drag orbit, and wheel zoom.
