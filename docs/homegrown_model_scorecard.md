...
| Currents | PASS. 4D current field, sampler, diagnostics, depth/time axes, source metadata, V2/V3 generator backends, and vertical profile contracts are deterministic. | PASS. Manufactured current catalog checks interpolation and finite diagnostics; FLOW-PKG-R2 manufactured vertical-profile smokes cover barotropic, surface-intensified, linear-shear, thermocline-jet, bottom-boundary, and nonuniform-depth interpolation behavior. | PARTIAL. Production synthetic currents include bathymetry masks, coastline projection, isobath steering, vertical shear, temporal change, named components, material depth structure in V3 mixed regional fields, and no below-bottom vectors. | FAIL / NOT PRESENT. No HYCOM, Copernicus, mooring, drifter, or glider-current comparison fixture is present. | PASS for current-aware route/sampler regression; not for calibrated forecast skill. |
...
## FLOW-PKG-R2 Current Generator Note

packages/currents now owns deterministic bathymetry-conditioned current generation backends. `cpuBathymetryConditionedSyntheticV2` preserves exact compatibility. `cpuBathymetryConditionedSyntheticV3` is the normal generated mixed-regional backend and uses declared coherent vertical profiles rather than random slab noise. Equal vectors across depth are valid only for explicit barotropic or vertically mixed controls. This remains synthetic benchmark validation, not calibrated ocean forecast validation.
