...
node tools/js/smoke_environment_studio_curated_regions.mjs
...
node tools/js/audit_environment_studio_product_handoff.mjs
...
node tools/js/smoke_reference_environment_artifact_composition.mjs
node tools/js/smoke_reference_environment_launch_validation.mjs
node tools/js/smoke_reference_environment_planning_adapter.mjs
node tools/js/smoke_reference_environment_benchmark_bundle.mjs
node tools/js/audit_reference_environment_public_safety.mjs
node tools/js/smoke_regional_bathymetry_viewport_cleanup.mjs
node tools/js/smoke_regional_bathymetry_camera_controls.mjs
node tools/js/smoke_environment_studio_return_state.mjs
node tools/js/audit_environment_studio_ux_polish_r2_1.mjs
...
The BATHY-DATA-R1 owner-review package is written to `test-results/bathy-data-r1-owner-review/` after the focused browser workflow. It always includes screenshots named `01-reference-bathy-overview-or-blocked.png`, `02-reference-bathy-fixture-selector.png`, `03-reference-patch-selected.png`, and `04-reference-patch-generated-3d-bathymetry.png`; blocked states also include `05-blocked-instructions-if-no-data.png`. The `qa-summary.json` records the actual screenshot list, fixture status, overview digest, fixture count, selected fixture ID, selected patch digest, bathymetry artifact digest, default source mode, procedural sandbox default status, no hidden-truth exposure, and no simulation/scoring changes. If the checked-in manifest is `AVAILABLE`, the browser must show fixture role, source variant, and actual source resolution; the preferred Monterey Canyon fixture is now a 15 arc-second `missionReadyPatch`, and the 60 arc-second `lowResolutionReferencePatch` remains selectable as fallback. If the checked-in manifest is `NO_REFERENCE_DATA_FIXTURE`, the browser must show blocked instructions, selection/generation controls must not create placeholder reference bathymetry, and scripts must not claim GEBCO/ETOPO-derived completion.

The FIELD-REGEN-R1 smokes assert reference-bathymetry-conditioned package-backed synthetic `CurrentField4D` generation, scalar field generation, hotspot candidates, hazard candidates, stable digests, finite diagnostics, zero nonzero land/below-bottom current vectors, depth/time variation, compact project metadata, dependency-state transitions to `CURRENT` for current/scalar/hotspots/hazards, `NEEDS_VALIDATION` for starts/drop zones, `CURRENT` or `REQUIRES_COMPOSITION` for the composed environment artifact, and no hidden-truth, HYCOM, Marine Copernicus, calibrated-product, or operational-forecast claims.
...
The focused Playwright workflows are `Global Atlas Visual Correctness`, `Atlas to Monterey Patch to Environment`, `Reference Patch Generates Environment Fields`, `Export / Import Generated Reference Environment`, `Reference Environment Owner Walkthrough`, and `Reference Environment Export/Benchmark Roundtrip`. They generate REF-ATLAS-UX-R1.2 browser evidence for the full-world global overview default, reset viewport, land/ocean visibility, Monterey overlay coordinate placement, staged Monterey patch loading, non-staged bbox patch-request export, generated bathymetry, generated field digests, launch-warning classifications, Planning launch, mission execution, Debrief, public benchmark export, and round-trip export/import. The owner package path is `artifacts/owner-review/ref-atlas-ux-r1-2/`; set `ANCHOR_E2E_OWNER_REVIEW_DIR` when a local synced filesystem blocks that directory.
...
REF-ATLAS-BOX-EDIT-R1 adds direct rectangle editing to the Global Reference Bathymetry Atlas. The atlas operational window can be moved and resized directly. Drag inside the rectangle to move it, drag edges to resize one side, and drag corners to resize both connected sides. Large operational windows are valid selections, but live Alpha generation remains budget-gated; oversized regions export patch or multi-tile requests. `node tools/js/smoke_reference_atlas_rectangle_hit_testing.mjs` covers corner, edge, center, and background hit-test priority. `node tools/js/smoke_reference_atlas_rectangle_editor.mjs` covers move preservation, edge/corner resize side preservation, min size, clamp behavior, Monterey availability, and Gulf multi-tile gating. `node tools/js/audit_reference_atlas_rectangle_editor_acceptance.mjs` writes `artifacts/owner-review/ref-atlas-box-edit-r1/` with screenshots for draw, hover handles, move, edge resize, corner resize, large-window budget state, Gulf multi-tile request, Monterey loading, and Planning launch readiness.
...
ENV-STUDIO-UX-POLISH-R2.1 adds viewport and interaction acceptance for the Regional Bathymetry Workspace. Its audit checks that the center panel is terrain-dominant, region details live in the right inspector, unavailable coarse actions are hidden or collapsed, camera rotate/pan/wheel/reset/top-down/oblique controls remain usable, Return to Atlas preserves selection and viewport state, atlas LOD/detail status is reported, Monterey mission-ready generation still appears, cleanup counts are zero, and simulation, scoring, planner behavior, and field equations remain unchanged.
