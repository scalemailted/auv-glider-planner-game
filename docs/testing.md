...
The focused Playwright workflows are `Reference Bathymetry Atlas Opens`, `Reference Patch Generates Bathymetry`, `Reference Patch Generates Environment Fields`, `Export / Import Generated Reference Environment`, `Reference Environment Owner Walkthrough`, and `Reference Environment Export/Benchmark Roundtrip`. They generate the BATHY-DATA-R1 owner-review screenshots and summary package plus FIELD-REGEN-R1/ENV-COMPOSE-LAUNCH-R1.1 browser evidence for generated field digests, composed environment digests, launch-warning classifications, Planning launch, mission execution, Debrief, public benchmark export, and round-trip export/import. The R1.1 owner package path is `test-results/env-compose-launch-r1-1-owner-review/`; set `ANCHOR_E2E_OWNER_REVIEW_DIR` when a local synced filesystem blocks that directory. For local alpha retest handoff, copy the validated package to ignored `artifacts/owner-review/env-compose-launch-r1-1/`.
