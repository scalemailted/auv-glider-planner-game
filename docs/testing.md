...
The focused Playwright workflows are `Global Reference Atlas Opens`, `Select Monterey Patch from Atlas and Generate Environment`, `Reference Patch Generates Environment Fields`, `Export / Import Generated Reference Environment`, `Reference Environment Owner Walkthrough`, and `Reference Environment Export/Benchmark Roundtrip`. They generate REF-ATLAS-UX-R1 browser evidence for the global overview default, patch coverage overlay, staged Monterey patch loading, generated bathymetry, generated field digests, composed environment digests, launch-warning classifications, Planning launch, mission execution, Debrief, public benchmark export, and round-trip export/import. The owner package path is `artifacts/owner-review/ref-atlas-ux-r1/`; set `ANCHOR_E2E_OWNER_REVIEW_DIR` when a local synced filesystem blocks that directory.
