...
The BATHY-DATA-R1 owner-review package is written to `test-results/bathy-data-r1-owner-review/` after the focused browser workflow. It always includes screenshots named `01-reference-bathy-overview-or-blocked.png`, `02-reference-bathy-fixture-selector.png`, `03-reference-patch-selected.png`, and `04-reference-patch-generated-3d-bathymetry.png`; blocked states also include `05-blocked-instructions-if-no-data.png`. The `qa-summary.json` records the actual screenshot list, fixture status, overview digest, fixture count, selected fixture ID, selected patch digest, bathymetry artifact digest, default source mode, procedural sandbox default status, no hidden-truth exposure, and no simulation/scoring changes. If the checked-in manifest is `AVAILABLE`, the browser must show fixture role and actual source resolution; the current Monterey Canyon fixture is a 60 arc-second `lowResolutionReferencePatch`, not a 15 arc-second mission-ready patch. If the checked-in manifest is `NO_REFERENCE_DATA_FIXTURE`, the browser must show blocked instructions, selection/generation controls must not create placeholder reference bathymetry, and scripts must not claim GEBCO/ETOPO-derived completion.
