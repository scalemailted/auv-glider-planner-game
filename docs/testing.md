...
For ALPHA-R1, run `node tools/js/validate_alpha_release_manifest.mjs`, `node tools/js/audit_alpha_release_readiness.mjs`, and the focused browser workflows in `tests/e2e/alpha_r1_external_preview.spec.js`. The Alpha checks cover onboarding, guided mission entry, researcher quick start, feedback/diagnostic export, error recovery, Pages subpath access, and compact layout. They must preserve the four Product Hub pillars, keep Google Colab hosting smoke visibly `PENDING`, and verify diagnostic bundles do not include hidden truth or automatic telemetry.
