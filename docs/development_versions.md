...
- Refactored the pure Sample / ROI Field Demo taxonomy into Spatial Field, Spatial Parameters, Temporal Pattern, Pattern Evolution, State Model, Sampling Effects, and Display controls with Time-Indexed, State-Evolving, and History-Aware labels. `Clustered Field` plus Cluster Count and Cluster Size replaces separate single/bimodal/multi-cluster modes; Forecast/truth/uncertainty and current-coupled sample behavior are intentionally outside this pure demo. See [sample_fields_demo.md](sample_fields_demo.md).
