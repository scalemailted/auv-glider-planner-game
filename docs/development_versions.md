...
- Refactored the pure Sample / ROI Field Demo taxonomy into Spatial Pattern, Temporal Pattern, Pattern Evolution, State Model, Depletion / Recovery, and Display controls with Time-Indexed, State-Evolving, and History-Aware labels. Forecast/truth/uncertainty and current-coupled sample behavior are intentionally outside this pure demo. See [sample_fields_demo.md](sample_fields_demo.md).
