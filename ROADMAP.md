...
- `ENV-STUDIO-R0/R1`: converts Alpha environment-authoring feedback into a unified Environment Studio architecture, pure editor contracts, validation/digest helpers, package smokes, and a visible Simulation Lab browser thin slice for domain setup, deterministic bathymetry tile generation, simple mosaic validation, dependency-state display, and project import/export. It does not add mission launch from edited Studio projects, scientific engines, simulation changes, scoring changes, planner changes, or a fifth Product Hub card. See [Environment Studio Architecture](docs/environment_studio_architecture.md) and [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md).
