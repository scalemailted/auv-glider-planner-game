...
- `ENV-STUDIO-R0/R1/R1.1/R2`, `ENV-WORLD-R1/R1A`, `ENV-ATLAS-R1/R1.1`, and `FIELD-REGEN-R1`: converts Alpha environment-authoring feedback into a unified Environment Studio architecture, pure editor contracts, validation/digest helpers, package smokes, and a visible Simulation Lab browser workflow. ENV-WORLD-R1/R1A makes the current first screen a deterministic, pan/zoomable, tiled synthetic world map with a hard owner-review visual acceptance package: users choose a world style/seed, inspect semantic layers, draw an operational boundary window, inspect sampled regional context and regime hints, then produce structured regional 3D bathymetry with distance fields, seeded feature placement, controlled roughness, validation, provenance, and digests. Broad generator controls are advanced disclosure rather than the main Stage 1 workflow. The earlier Synthetic Ocean Atlas remains a pure compatibility/algorithm substrate. FIELD-REGEN-R1 adds an explicit package-backed synthetic current/scalar/hotspot regeneration action with compact digests, component plans, and diagnostics. It does not add mission launch from edited Studio projects, start/drop-zone validation, benchmark bundle export, sculpting, calibrated ocean data, simulation changes, scoring changes, planner changes, benchmark fairness changes, or a fifth Product Hub card. See [Environment Studio Architecture](docs/environment_studio_architecture.md) and [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md).
