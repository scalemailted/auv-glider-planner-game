...
- `ENV-STUDIO-R0/R1/R1.1`, `ENV-ATLAS-R1/R1.1`, and `FIELD-REGEN-R1`: converts Alpha environment-authoring feedback into a unified Environment Studio architecture, pure editor contracts, validation/digest helpers, package smokes, and a visible Simulation Lab browser workflow. ENV-ATLAS-R1 pivots the first screen to a Synthetic Ocean Atlas; ENV-ATLAS-R1.1 makes that atlas a deterministic field engine and adds a window-conditioned bathymetry builder. Users select an operational window, inspect inferred regional context and current/scalar regime hints, generate a Regional Mission Recipe, then produce structured regional 3D bathymetry with distance fields, seeded feature placement, controlled roughness, validation, provenance, and digests. FIELD-REGEN-R1 adds an explicit package-backed synthetic current/scalar/hotspot regeneration action with compact digests, component plans, and diagnostics. It does not add mission launch from edited Studio projects, start/drop-zone validation, benchmark bundle export, sculpting, calibrated ocean data, simulation changes, scoring changes, planner changes, benchmark fairness changes, or a fifth Product Hub card. See [Environment Studio Architecture](docs/environment_studio_architecture.md) and [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md).
