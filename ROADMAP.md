...
- `ENV-STUDIO-R0/R1/R1.1` and `ENV-ATLAS-R1`: converts Alpha environment-authoring feedback into a unified Environment Studio architecture, pure editor contracts, validation/digest helpers, package smokes, and a visible Simulation Lab browser workflow. ENV-ATLAS-R1 pivots the first screen to a Synthetic Ocean Atlas: select an operational window, inspect inferred regional context and current/scalar regime hints, generate a Regional Mission Recipe, then produce regional 3D bathymetry detail through the existing generator path. It does not add mission launch from edited Studio projects, current/scalar/hotspot regeneration, sculpting, scientific-engine changes, simulation changes, scoring changes, planner changes, benchmark fairness changes, or a fifth Product Hub card. See [Environment Studio Architecture](docs/environment_studio_architecture.md) and [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md).
