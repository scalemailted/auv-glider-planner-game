...
- `ENV-STUDIO-R0/R1/R1.1/R2`, `ENV-WORLD-R1/R1A`, `ENV-GLOBE-R1`, `ENV-ATLAS-R1/R1.1`, `FIELD-REGEN-R1`, `BATHY-DATA-R1/R1.1/R1.2`, `ENV-COMPOSE-LAUNCH-R1/R1.1`, and `REF-ATLAS-UX-R1`: converts Alpha environment-authoring feedback into a unified Environment Studio architecture, pure editor contracts, validation/digest helpers, package smokes, and a visible Simulation Lab browser workflow. The active front door is the Reference Bathymetry Atlas, backed by compact preprocessed public bathymetry/topography artifacts under `assets/reference_bathymetry/`. Raw NOAA/GEBCO source data belongs outside git under `external_data/reference_bathymetry/`. BATHY-DATA-R1 adds downloader/preprocessor scripts and a runtime manifest; BATHY-DATA-R1.1 labels the original Monterey Canyon fixture as an ETOPO 2022 60 arc-second `lowResolutionReferencePatch`; BATHY-DATA-R1.2 adds a true ETOPO 2022 15 arc-second Monterey Canyon `missionReadyPatch` from the NOAA surface-elevation non-ice fallback tile while preserving the 60s fallback; REF-ATLAS-UX-R1 adds the global overview default, patch coverage overlay, explicit staged patch load, and owner-review package at `artifacts/owner-review/ref-atlas-ux-r1/`. The earlier Synthetic Ocean Atlas and synthetic world/globe modules remain compatibility/algorithm substrates and experimental sandbox paths. The implemented reference-derived path is public reference bathymetry patch -> bathymetry artifact -> deterministic synthetic bathymetry-conditioned fields -> package-backed environment artifact -> validated Planning launch -> public benchmark bundle, with package-backed current/scalar/hotspot/hazard generation, compact digests, classified launch warnings, owner-review evidence, and public benchmark-bundle export. It does not add sculpting, calibrated ocean data, simulation changes, scoring changes, planner changes, benchmark fairness changes, or a fifth Product Hub card. See [Environment Studio Architecture](docs/environment_studio_architecture.md), [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md), and [Reference Bathymetry Data Pipeline](docs/reference_bathymetry_data_pipeline.md).
- `FEEDBACK-OPS-R1`: adds `alpha/feedback-ledger.json` and clarifies the in-app Researcher Notebook Launchpad for notebook, public bundle, fallback Colab, and local finalizer workflows. It does not change scientific models, mission simulation, scoring, planner behavior, or benchmark fairness.
