...
- `ENV-STUDIO-R0/R1/R1.1/R2`, `ENV-WORLD-R1/R1A`, `ENV-GLOBE-R1`, `ENV-ATLAS-R1/R1.1`, `FIELD-REGEN-R1`, and `BATHY-DATA-R1`: converts Alpha environment-authoring feedback into a unified Environment Studio architecture, pure editor contracts, validation/digest helpers, package smokes, and a visible Simulation Lab browser workflow. The active front door is the Reference Bathymetry Atlas, backed by compact preprocessed public bathymetry/topography artifacts under `assets/reference_bathymetry/`. Raw NOAA/GEBCO source data belongs outside git under `external_data/reference_bathymetry/`. BATHY-DATA-R1 adds downloader/preprocessor scripts, a manifest, and blocked-state UI; the checked-in manifest remains `NO_REFERENCE_DATA_FIXTURE` until local ETOPO/GEBCO source files are downloaded and preprocessed. The earlier Synthetic Ocean Atlas and synthetic world/globe modules remain compatibility/algorithm substrates and experimental sandbox paths. FIELD-REGEN-R1 adds an explicit package-backed synthetic current/scalar/hotspot regeneration action with compact digests, component plans, and diagnostics. It does not add mission launch from edited Studio projects, start/drop-zone validation, benchmark bundle export, sculpting, calibrated ocean data, simulation changes, scoring changes, planner changes, benchmark fairness changes, or a fifth Product Hub card. See [Environment Studio Architecture](docs/environment_studio_architecture.md), [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md), and [Reference Bathymetry Data Pipeline](docs/reference_bathymetry_data_pipeline.md).
