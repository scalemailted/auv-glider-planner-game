...
assert.equal(monterey.openBathymetryPreviewEnabled, true);
assert.equal(monterey.openCoarsePreviewEnabled, true);
assert.equal(monterey.previewMode, 'stagedSingleTile');
assert.equal(monterey.previewSource, 'hostedMissionReadyTile');
assert.equal(monterey.previewAction.missionReadyTileAvailable, true);
assert.equal(monterey.previewAction.recommendedAction, 'continueToMissionReadyBathymetry');
...
assert.equal(unstaged.openBathymetryPreviewEnabled, true);
assert.equal(unstaged.openCoarsePreviewEnabled, true);
assert.equal(unstaged.previewMode, 'coarsePreview');
assert.equal(unstaged.previewSource, 'globalOverview');
assert.equal(unstaged.previewAction.stagingRequired, true);
assert.equal(unstaged.previewAction.recommendedAction, 'openBathymetryPreview');
...
assert.equal(gulf.openBathymetryPreviewEnabled, true);
assert.equal(gulf.openCoarsePreviewEnabled, true);
assert.equal(gulf.previewMode, 'coarsePreview');
assert.equal(gulf.previewSource, 'globalOverview');
...
assert.equal(gulf.selectedRegionScale.operationalSelectionStatus, 'VALID');
assert.equal(gulf.selectedRegionScale.generationBudgetStatus, 'MULTI_TILE_REQUIRED');
assert.equal(gulf.selectedRegionScale.coarsePreviewAvailable, true);
assert.equal(gulf.previewAction.multiTileRequired, true);
assert.equal(gulf.previewAction.recommendedAction, 'openBathymetryPreview');
...
assert.equal(directActions.openCoarsePreview.enabled, true);
assert.equal(directActions.openCoarsePreview.label, 'Open 3D Bathymetry Preview');
