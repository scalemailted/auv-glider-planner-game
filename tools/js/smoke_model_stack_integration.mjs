...
assert.equal(worldToGridCell(gfxR3aTransform, gfxR3aWorldPoint.x, gfxR3aWorldPoint.y, gfxR3aWorldPoint.z).col, 3, 'GFX-R3A coordinate roundtrip preserves col');
const gfxR3aInput = missionWorldRenderInputFromWorkspace({ app: { state: gfxR3aFixture.state } });
const gfxR3aViewModel = buildMissionWorldRenderViewModel(gfxR3aInput);
const gfxR3aValidation = validateMissionWorldRenderViewModel(gfxR3aViewModel);
assert.equal(gfxR3aValidation.valid, true, 'GFX-R3A mission render view model validates');
const gfxR3aSummary = missionWorldRenderViewModelSummary(gfxR3aViewModel);
assert.ok(gfxR3aSummary.gliderCount >= 2, 'GFX-R3A view model includes live gliders');
assert.ok(gfxR3aSummary.waypointCount >= 2, 'GFX-R3A view model includes waypoints');
...
assert.equal(typeof THREE_MISSION_CAMERA_CONTROLLER_VERSION, 'string', 'THREE-R1.1C camera controller imports');
assert.equal(THREE_MISSION_CAMERA_MOUSE_MAPPING.LEFT, 'PAN', 'THREE-R1.1C left drag maps to pan');
assert.equal(THREE_MISSION_CAMERA_MOUSE_MAPPING.RIGHT, 'ROTATE', 'THREE-R1.1C right drag maps to orbit/rotate');
assert.equal(THREE_MISSION_CAMERA_MOUSE_MAPPING.MIDDLE, 'DOLLY', 'THREE-R1.1C middle drag maps to dolly');
assert.ok(fs.readFileSync('src/game/three/ThreeMissionCameraController.js', 'utf8').includes('orbitBy'), 'THREE-R1.1C camera controller exposes orbit');
assert.ok(fs.readFileSync('src/game/three/ThreeMissionCameraController.js', 'utf8').includes('panBy'), 'THREE-R1.1C camera controller exposes pan');
assert.ok(fs.readFileSync('src/game/three/ThreeMissionCameraController.js', 'utf8').includes('zoomByDelta'), 'THREE-R1.1C camera controller exposes zoom');
const threeR11cTool = setMissionPlanningTool(createMissionPlanningToolState({ selectedAgentId: 'glider-alpha' }), 'selectDeploymentCell', { selectedAgentId: 'glider-alpha' });
