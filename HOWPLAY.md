...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. It uses preprocessed public bathymetry/topography fixtures as the default Reference Bathymetry Atlas source. Environment Studio is a staged workflow. The Global Atlas Staging Scene is used to select and inspect an operational boundary window. Continue to 3D Bathymetry is enabled only when the selected operational window matches an app-hosted mission-ready tile set. Request-only and Gulf-scale regions can open a coarse regional preview for inspection, then export patch or multi-tile request artifacts for offline staging. The browser supports deep zoom and typed operational-window editing, overlays staged patch coverage, and opens the regional workspace only for staged mission-ready selections. Coarse preview is not mission-ready, cannot generate fields, cannot compose an environment, and cannot launch Planning. The atlas operational window can be moved and resized directly. Drag inside the rectangle to move it, drag edges to resize one side, and drag corners to resize both connected sides. Large operational windows are valid selections, but live Alpha generation remains budget-gated; oversized regions export patch or multi-tile requests. ANCHOR hosts curated app-ready reference bathymetry tiles as static assets. The browser does not download NOAA/GEBCO data at runtime. External public bathymetry data is downloaded and preprocessed offline, then registered as staged ANCHOR tile artifacts. Raw source files live under gitignored `external_data/reference_bathymetry/`, and compact runtime fixtures live under `assets/reference_bathymetry/`. The Pages build delivers the same staged tile-library manifest, Monterey rasters, mesh LODs, and global overview artifacts. The regional 3D mesh is a decimated visualization artifact. The reference raster/grid remains authoritative for bathymetry sampling, masks, environment generation, simulation, and benchmark export. The checked-in state currently reports `AVAILABLE` with a compact ETOPO 2022 60 arc-second global overview selection layer, a true ETOPO 2022 15 arc-second Monterey Canyon mission-ready reference patch, a preserved ETOPO 2022 60 arc-second low-resolution fallback, and a static tile-library manifest that stages those Monterey rasters plus non-authoritative mesh LODs. Gulf regions remain request-only until offline preprocessing is run, including the deterministic `gulf_segment_demo_15s` multi-tile request region. The global overview is not mission-resolution bathymetry; the Studio prefers the 15s fixture when it is available and still does not claim calibrated survey bathymetry or operational forecast data. The global atlas separates the selected operational window from the live-generation budget: large valid Gulf-scale windows remain selectable, but export a multi-tile patch request instead of running browser generation.
