...
Simulation Lab `Demos` includes four field laboratories: Flow Fields Demo for current vectors `F(x,y,t)`, Process Lab for deterministic or seeded spatiotemporal processes `S(x,y,t)`, Coupled Fields Demo for combining flow plus sampling processes plus mission-relevant structure, and Uncertainty / Forecast Demo for likelihood, belief, forecast/truth, uncertainty, information gain, and update effects. In Process Lab, the normal workflow starts with `Mode`: Example Processes, Custom Composer, Process Paint, or Rule Allocation Sandbox. Source / Initial Field is the deterministic or seeded substrate where a process starts, recurs, or has initial support; it is not uncertainty or belief. The right panel opens on Process Example View with source, related models, rule/update-function framing, sampling interpretation, component recipe, and failure signs; clicking a heatmap cell switches it to Cell Inspector. The display layer can show Sampling Value, Source Field, Sampling Value + Source Overlay, Cell / Node States, Graph Topology, Process Influence Messages, State Transitions, Community / Group View, ROI Meaning, depletion/freshness value, and diagnostics. Scenario Generation remains a supporting notebook/replay feature, not a batch dataset builder. Reference signatures and legacy presets inspired by real-world processes are simplified educational examples, not validated process models. Forecast/truth/uncertainty belongs in the Uncertainty / Forecast Demo. See [Deterministic Spatiotemporal Process Lab](docs/sampling_process_lab.md) and [Uncertainty / Forecast Demo](docs/uncertainty_forecast_demo.md).

Learning Labs open as standalone static pages. Start with the [Learning Labs Index](labs/index.html) for the six-lab syllabus, then read [Deterministic Spatiotemporal Processes](labs/deterministic-spatiotemporal-processes.html) for rule-update intuition, [Deterministic Dynamic Flow Fields](labs/deterministic-dynamic-flow-fields.html) for vector-current intuition, [Oracle / Deterministic Coupled Sampling Space](labs/oracle-deterministic-coupled-sampling-space.html) for known process + flow + constraint coupling, [Stochastic / Uncertainty](labs/stochastic-uncertainty.html) for hidden truth, forecast error, observations, and belief updates, [Stochastic Coupled Sampling Space](labs/stochastic-coupled-sampling-space.html) for belief-based acquisition, hidden-event evidence, reachability, and regret, and [Planner / Mission Evaluation](labs/planner-mission-evaluation.html) for waypoint routes, reward/cost/risk tradeoffs, simulation, debrief, solver fairness, and regret. The intended rhythm is read all six Learning Labs, then use Challenge Mode or Simulation Lab to plan, execute, debrief, compare, and export.
...
Example Processes in Process Lab connect known process families to editable deterministic recipes. The selector is grouped into Foundational CA Models, including Conway, Forest Fire, SIR, Greenberg-Hastings, Sandpile, Wa-Tor, Traffic CA, and Wireworld, and Observable Process Patterns such as Propagating Fronts, Excitable Waves, Local Birth-Death Emergence, Recurrent Stationary Hotspots, Diffusive / Epidemic Spread, Directed Feature Transport, Domain / Cluster Formation, Threshold Cascades / Avalanches, Pattern Formation / Morphogenesis, Congestion / Density Waves, and Structured Signal Propagation. The CA taxonomy framing describes mechanism families; it does not make the demo an exact simulator for any named model.
