...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. Use it to define a browser-friendly regional bathymetry recipe: choose environment scale, source-grid resolution, regional layout template, feature mix, randomization locks, and deterministic seed; generate a multi-archetype synthetic region; inspect the default 3D bathymetry preview or switch to top-down, seam, slope, wet/land, cross-section, and multi-glider suitability diagnostics; then import/export `anchor.environment-studio-project` JSON.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. It is not a volumetric geology editor and not a calibrated real-ocean bathymetry product. It does not launch missions from edited projects, regenerate currents/scalars/hotspots, add sculpting, change scientific equations, alter scoring, or add planners.
