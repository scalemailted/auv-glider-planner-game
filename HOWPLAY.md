...
Simulation Lab `Demos` includes four field laboratories: Flow Fields Demo for current vectors `F(x,y,t)`, Spatiotemporal Sampling Process Lab for deterministic or seeded sampling processes `S(x,y,t)`, Coupled Fields Demo for combining flow plus sampling processes plus mission-relevant structure, and Uncertainty / Forecast Demo for likelihood, belief, forecast/truth, uncertainty, information gain, and update effects. In the sampling-process lab, the normal workflow starts with `Pattern Source / Mode`: Reference Signature, Custom Composer, Process Paint, or Random Rule Lab. Source / Initial Field is the deterministic or seeded substrate where a process starts, recurs, or has initial support; it is not uncertainty or belief. The right panel opens on Recipe / Signature View with source, inspired-by models, observable signature, ROI meaning, component recipe, and failure signs; clicking a heatmap cell switches it to Cell Inspector. The display layer can show Sampling Value, Source Field, Sampling Value + Source Overlay, Cell / Node States, Graph Topology, Process Influence Messages, State Transitions, Community / Group View, ROI Meaning, depletion/freshness value, and diagnostics. Scenario Generation remains a supporting notebook/replay feature, not a batch dataset builder. Reference signatures and legacy presets inspired by real-world processes are simplified educational examples, not validated process models. Forecast/truth/uncertainty belongs in the Uncertainty / Forecast Demo. See [Spatiotemporal Sampling Process Lab](docs/sampling_process_lab.md) and [Uncertainty / Forecast Demo](docs/uncertainty_forecast_demo.md).
...
Reference Observable Process Signatures in the Sample / ROI Field Demo connect known process families to editable ROI recipes. The visible selector uses broad observable-process labels such as Propagating Fronts, Excitable Waves, Local Birth-Death Emergence, Recurrent Stationary Hotspots, Diffusive / Epidemic Spread, Directed Drift / Transport, Domain / Cluster Formation, Threshold Cascades / Avalanches, Pattern Formation / Morphogenesis, Congestion / Density Waves, and Structured Signal Propagation. Known CA/grid-process examples such as forest-fire CA, Brian's Brain, Life-like rules, epidemic grids, sandpiles, Ising/voter/Schelling dynamics, traffic CA, and Wireworld-style signal paths appear as inspired-by references in the right panel and exports, not as separate dropdown clutter. The CA taxonomy framing describes mechanism families; it does not make the demo an exact CA simulator.
