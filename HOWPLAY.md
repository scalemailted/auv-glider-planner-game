...
Simulation Lab `Demos` includes four field laboratories: Flow Fields Demo for deterministic synthetic current vectors `F(x,y,t)` with diagnostics for speed, divergence, vorticity, strain, terrain masking, and claim boundaries; Process Lab for deterministic or seeded spatiotemporal processes; Coupled Fields Demo for deterministic/oracle coupled sampling spaces; and Uncertainty / Forecast Demo for likelihood, belief, forecast/truth, uncertainty, information gain, and update effects. In Process Lab, the normal workflow starts with `Mode`: Foundational CA Models, Ocean-Relevant Process Analogs, Custom Composer, Process Paint, or Rule Allocation Sandbox. Coupled Fields now adds analytical scalar process engines for known process `C(x,y,t)`, known flow `F(x,y,t)`, known constraints, and oracle objective `S*(x,y,t)`. It includes a CA/grid-process baseline but does not replace the Process Lab CA sandbox. Uncertainty, belief, hidden truth, forecast error, and information gain belong in the Uncertainty / Forecast Demo. See [Deterministic Spatiotemporal Process Lab](docs/sampling_process_lab.md), [Coupled Fields Demo](docs/coupled_fields_demo.md), and [Uncertainty / Forecast Demo](docs/uncertainty_forecast_demo.md).
