...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. Use it to choose a Synthetic Ocean Atlas preset, select or adjust an operational window, inspect inferred regional context and recommended mission recipe, then generate a regional 3D bathymetry project. The atlas map is field-derived: land/ocean, shelf, basin, island/seamount, canyon, river-mouth, strait/sill, current-regime, scalar-regime, and mission-suitability hints are deterministic synthetic fields. Advanced feature-mix and source-tile controls remain available after generation as provenance and tuning tools.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The atlas is synthetic and normalized, not real latitude/longitude, real Earth imagery, or an operational forecast. Generated bathymetry uses structured procedural features, distance fields, seeded feature placement, controlled roughness, smoothing, validation, and provenance; noise is not the sole terrain model. After generating a region, use `Generate Currents & Science Fields` to create compact package-backed synthetic current, scalar, and hotspot metadata with diagnostics. It does not launch missions from edited projects, validate start/drop zones, export benchmark bundles, add sculpting, alter scoring, or add planners. Regenerated fields are synthetic atlas-conditioned benchmark fixtures, not HYCOM, Marine Copernicus, calibrated ocean products, operational forecasts, or navigation data.
