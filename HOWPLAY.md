...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. Use it to generate a deterministic synthetic globe from a style and seed, rotate/zoom across the globe, inspect layers, select a small operational globe region, inspect sampled regional context, then generate a regional 3D bathymetry project. Broad world controls and boundary sizing are advanced disclosure controls. The globe visualizes field-derived equirectangular world fields: land/ocean, distance-to-coast, shelf, basin, island/seamount, canyon, river-mouth, strait/sill, coarse flow-regime, scalar-regime, diversity, and suitability hints are deterministic synthetic fields.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The globe is synthetic and normalized, not real Earth, GEBCO, ETOPO, HYCOM, Copernicus, calibrated survey data, certified navigation data, or an operational forecast. The primary view is a globe selector; any flat projection is secondary diagnostic context. Generated bathymetry uses structured procedural features, distance fields, seeded feature placement, controlled roughness, smoothing, validation, and provenance; noise is not the sole terrain model. Currents, scalars, hotspots, start/drop-zone validation, benchmark bundles, and launch into Planning remain staged follow-ups. Environment Studio does not alter scoring, planners, mission simulation, or benchmark fairness.
