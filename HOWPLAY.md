...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. Use it to choose a Synthetic Ocean Atlas preset, select or adjust an operational window, inspect inferred regional context and recommended mission recipe, then generate a regional 3D bathymetry project. Advanced feature-mix and source-tile controls remain available after generation as provenance and tuning tools.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The atlas is synthetic and normalized, not real latitude/longitude, real Earth imagery, or an operational forecast. It is not a volumetric geology editor and not a calibrated real-ocean bathymetry product. It does not launch missions from edited projects, regenerate currents/scalars/hotspots, add sculpting, change scientific equations, alter scoring, or add planners.
