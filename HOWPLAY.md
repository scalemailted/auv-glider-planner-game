...
In the Sample / ROI Field Demo, `Event Likelihood Field`, `Spatial Pattern / Geometry`, and `Value Distribution` are separate concepts. Event Likelihood Field describes where events are prone to originate or move next. Spatial Pattern / Geometry describes geometry such as Constant Field, Clustered Field, bands, fronts, or seeded texture. Value Distribution describes how cell values are assigned inside that geometry, such as Constant Value, Uniform Random, or Gaussian / Normal.
