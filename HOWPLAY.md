...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. Use it to pan/zoom a Reference Bathymetry Atlas, inspect dataset metadata, draw a lon/lat bounding box, review the selected patch summary, then generate a regional 3D bathymetry project. The checked-in atlas is currently an explicit `NO_REFERENCE_DATA_FIXTURE` placeholder for workflow testing; it is not GEBCO, ETOPO, calibrated survey data, or a navigation product. The procedural synthetic globe/world workflow remains available only as an experimental sandbox.
