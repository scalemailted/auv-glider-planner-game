...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. It uses preprocessed public bathymetry/topography fixtures as the default Reference Bathymetry Atlas source. The browser opens to a Global Atlas Selector, overlays staged patch coverage, and moves to a Regional Patch Workspace only after a staged fixture is loaded. The browser app does not download NOAA or GEBCO data at runtime; raw source files live under gitignored `external_data/reference_bathymetry/`, and compact runtime fixtures live under `assets/reference_bathymetry/`. The checked-in state currently reports `AVAILABLE` with a compact ETOPO 2022 60 arc-second global overview selection layer, a true ETOPO 2022 15 arc-second Monterey Canyon mission-ready reference patch, and a preserved ETOPO 2022 60 arc-second low-resolution fallback. The global overview is not mission-resolution bathymetry; the Studio prefers the 15s fixture when it is available and still does not claim calibrated survey bathymetry or operational forecast data. The global atlas separates the selected operational window from the live-generation budget: large valid Gulf-scale windows remain selectable, but export a multi-tile patch request instead of running browser generation.
