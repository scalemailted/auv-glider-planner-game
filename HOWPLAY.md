...
Simulation Lab `Demos` includes four field laboratories: Flow Fields Demo for current vectors `F(x,y,t)`, Sample / ROI Field Demo for pure sample value `S(x,y,t)`, Coupled Fields Demo for how currents shape or transport sample value, and Uncertainty / Forecast Demo for forecast, truth, uncertainty, information gain, and update effects. In the pure sample demo, controls are organized as Spatial Field, Spatial Parameters, Temporal Pattern, Spatial Evolution, State Model, Sampling Effects, and Display. Spatial Pattern covers Uniform Field, Gradient / Trend, Clustered Field, Patchy / Correlated Field, Sparse Targets, Linear Band, Front / Boundary, Boundary Band, Monitoring Stations, and Seeded Texture. Cluster Count and Cluster Size parameterize `Clustered Field`; single-cluster, two-cluster, and multi-cluster cases are not separate theories. Spatial Pattern, Temporal Pattern, Spatial Evolution, State Model, Sampling Effect, and Display Layer each have quick hover help plus an `About ...` behavior explainer, and the Current Composition card summarizes how selected components combine. The Cell Inspector reports the selected pattern with relevant parameters. Spatial Evolution controls whether a pattern is stationary, drifts continuously, jumps between bursts, wanders, or propagates to neighbors. Forecast/truth/uncertainty belongs in the Uncertainty / Forecast Demo. See [Sample / ROI Field Demo](docs/sample_fields_demo.md) and [Uncertainty / Forecast Demo](docs/uncertainty_forecast_demo.md).
