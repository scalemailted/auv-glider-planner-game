...
In the Sample / ROI Field Demo, `Spatial Pattern` and `Value Distribution` are separate concepts. Spatial Pattern describes geometry such as Constant Field, Clustered Field, bands, fronts, or seeded texture. Value Distribution describes how cell values are assigned inside that geometry, such as Constant Value, Uniform Random, or Gaussian / Normal.
