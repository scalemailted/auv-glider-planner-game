...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. Use it to generate a synthetic world map from a style and seed, adjust broad world controls, pan/zoom across the tiled map, draw an operational boundary window, inspect sampled regional context, then generate a regional 3D bathymetry project. The map is field-derived: land/ocean, distance-to-coast, shelf, basin, island/seamount, canyon, river-mouth, strait/sill, coarse flow-regime, scalar-regime, diversity, and suitability hints are deterministic synthetic fields.
