...
Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. Reference patch export preserves atlas/patch/bathymetry digests, validation status, wet/land and coastline summaries, field-regeneration digests, and dependency states. The explicit Generate Currents & Science Fields action follows public reference bathymetry patch -> bathymetry artifact -> synthetic bathymetry-conditioned fields -> environment artifact, producing deterministic synthetic benchmark currents, scalars, hotspots, start/drop-zone candidates, and hazard candidates. Start/drop-zone validation, benchmark bundles, and launch into Planning remain staged follow-ups. Environment Studio does not alter scoring, planners, mission simulation, or benchmark fairness.
