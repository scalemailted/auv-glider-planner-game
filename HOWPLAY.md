...
The Main Menu is the starting screen. It is now a full-viewport product hub with four primary cards:
...
- `Simulation Lab`: scientific sandboxes, benchmark modes, Environment Studio, headless bundle viewer, external solver workflows, Mission Editor, and import/export tools.
- `Learning Labs`: explanatory articles plus companion sandbox launch points that teach the science step by step.
- `Methods & Validation`: Inspect model assumptions, numerical tests, reference comparisons, provenance, and known limitations.

The Product Hub also shows an `ANCHOR Alpha` badge, release version, tagline, claim boundary, `Start Alpha Tour`, `View Known Limitations`, and `Feedback & Diagnostics`. `Start Alpha Tour` can launch a guided mission, a researcher notebook/benchmark path, Methods & Validation, or the normal hub.

Methods & Validation is not a game mode and does not certify operational ocean validity; it presents the current evidence baseline and claim boundaries. Simulation Lab Benchmark Modes exposes Planner Benchmark, Adaptive Benchmark, and Full Autonomy Benchmark. Planner Benchmark uses the existing planning workspace, simulator, and Debrief for fixed-objective route attempts. Adaptive Benchmark now previews a transparent mission manager that recommends objectives after observations or belief diagnostics; the player or solver still chooses the route. Full Autonomy remains contract-only. These modes do not implement a new planner, scoring redesign, or MARL/RL. See [Benchmark Modes](docs/benchmark_modes.md), [Benchmark Route Execution Contract](docs/benchmark_route_execution_contract.md), and [Adaptive Benchmark Mission Manager](docs/adaptive_benchmark_mission_manager.md).

Simulation Lab Demos includes field laboratories and architecture previews: Flow Fields Demo for deterministic synthetic current vectors `F(x,y,t)` with diagnostics for speed, divergence, vorticity, strain, terrain masking, and claim boundaries; Process Lab for deterministic or seeded spatiotemporal processes; Coupled Fields Demo for deterministic/oracle coupled sampling spaces; Uncertainty / Forecast Demo for hidden truth, forecast/expected state, noisy observations, belief, surprise, forecast error, unknown-event probability, and sampling-priority preview; Sampling Priority Demo for global `A_global(x,y,t)` acquisition value; and Flow-Coupled Sampling Demo for glider-specific direct-target `Q_glider(g,x,y,t)` action value. In Process Lab, the normal workflow starts with `Mode`: Foundational CA Models, Ocean-Relevant Process Analogs, Custom Composer, Process Paint, or Rule Allocation Sandbox. Coupled Fields now adds analytical scalar process engines for known process `C(x,y,t)`, known flow `F(x,y,t)`, known constraints, and oracle objective `S*(x,y,t)`. It includes a CA/grid-process baseline but does not replace the Process Lab CA sandbox. Uncertainty, belief, hidden truth, forecast error, and hidden-event suspicion belong in the Uncertainty / Forecast Demo. Global acquisition / sampling usefulness belongs in the Sampling Priority Demo, which teaches that event intensity is not sampling priority and does not perform route planning or flow-coupled action-value optimization. Flow-coupled direct target selection belongs in the Flow-Coupled Sampling Demo, which starts from `A_global` and evaluates current assist/opposition, cross-current risk, travel time, energy, hazards, timing, and redundancy for one selected glider without building routes. See [Deterministic Spatiotemporal Process Lab](docs/sampling_process_lab.md), [Coupled Fields Demo](docs/coupled_fields_demo.md), [Uncertainty / Forecast Demo](docs/uncertainty_forecast_demo.md), [Sampling Priority Demo](docs/sampling_priority_demo.md), and [Flow-Coupled Sampling Demo](docs/flow_coupled_sampling_demo.md). Renderer Architecture Preview remains available in Simulation Lab to inspect renderer boundaries; Three.js is now the production mission environment, while Phaser is transitional shell/lab infrastructure and a query-gated diagnostic fallback.

Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. It uses preprocessed public bathymetry/topography fixtures as the default Reference Bathymetry Atlas source. Environment Studio has two stages. The Global Atlas Setup Scene is for selecting and inspecting an operational boundary. Every valid boundary can open an interactive 3D bathymetry preview. If high-resolution staged tiles are not available, the preview uses app-hosted overview/LOD data and is not mission-ready. Field generation, Planning launch, and benchmark export require staged mission-ready bathymetry tiles. Continue to Mission-Ready Bathymetry is enabled only when the selected operational window matches an app-hosted mission-ready tile set. The browser supports deep zoom and typed operational-window editing, overlays staged patch coverage, and opens the mission-ready regional workspace only for staged mission-ready selections. The atlas operational window can be moved and resized directly. Drag inside the rectangle to move it, drag edges to resize one side, and drag corners to resize both connected sides. Large operational windows are valid selections, but live Alpha generation remains budget-gated; oversized regions export patch or multi-tile requests. ANCHOR hosts curated app-ready reference bathymetry tiles as static assets. The browser does not download NOAA/GEBCO data at runtime. External public bathymetry data is downloaded and preprocessed offline, then registered as staged ANCHOR tile artifacts. Raw source files live under gitignored `external_data/reference_bathymetry/`, and compact runtime fixtures live under `assets/reference_bathymetry/`. The Pages build delivers the same staged tile-library manifest, Monterey rasters, mesh LODs, and global overview artifacts. The regional 3D mesh is a decimated visualization artifact. The reference raster/grid remains authoritative for bathymetry sampling, masks, environment generation, simulation, and benchmark export. The checked-in state currently reports `AVAILABLE` with a compact ETOPO 2022 60 arc-second global overview selection layer, a true ETOPO 2022 15 arc-second Monterey Canyon mission-ready reference patch, a preserved ETOPO 2022 60 arc-second low-resolution fallback, and a static tile-library manifest that stages those Monterey rasters plus non-authoritative mesh LODs. Gulf regions remain request-only until offline preprocessing is run, including the deterministic `gulf_segment_demo_15s` multi-tile request region. The global overview is not mission-resolution bathymetry; the Studio prefers the 15s fixture when it is available and still does not claim calibrated survey bathymetry or operational forecast data. The global atlas separates the selected operational window from the live-generation budget: large valid Gulf-scale windows remain selectable, previewable with adaptive LOD, and exportable as multi-tile patch requests instead of running browser generation.

The Global Atlas Setup Scene includes a Curated Region dropdown. `None` leaves the current custom boundary unchanged; presets such as Monterey Canyon, Hawaii / Island Slope, Puerto Rico Trench / Island Shelf, Florida Straits, Gulf Shelf / Canyon Segment, Northeast US Shelf Break, California Shelf Break, and Alaska Fjord / Shelf Region place editable seed windows and focus the atlas. These presets do not download data or claim mission readiness unless staged app-hosted tiles exist. The Bathymetry Mode selector names Real Reference, Reference-Enhanced Synthetic, and Fully Synthetic Sandbox; only Real Reference is implemented as the current mission-authority path in this scaffold.
...
Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. Reference patch export preserves atlas/patch/bathymetry digests, validation status, wet/land and coastline summaries, field-regeneration digests, launch-validation summaries, benchmark-bundle digests, and dependency states. The explicit environment workflow is public reference bathymetry patch -> bathymetry artifact -> deterministic synthetic bathymetry-conditioned fields -> package-backed environment artifact -> validated Planning launch -> public benchmark bundle. Currents, scalars, hotspots, start/drop-zone candidates, and hazard candidates are deterministic synthetic benchmark artifacts, not operational forecast products. Mission Workspace still owns glider count, routes, dive profiles, execution, replanning, and scoring; Environment Studio does not alter scoring, planners, mission simulation, or benchmark fairness.

For alpha retesting, follow [Reference-Derived Environment Alpha Retest Protocol](docs/alpha_reference_environment_retest_protocol.md) and record feedback with `alpha/reference-environment-retest-feedback-template.json`. The current retest includes hosted Monterey tile loading, mesh-vs-raster authority, Planning launch/mission completion, any-valid-boundary bathymetry preview, Gulf coarse preview, and Gulf requestOnly/multi-tile request export.
...
For first-time play, use `Start Alpha Tour` and choose `Play a Guided Mission`, or click `Challenge Mode` in the hub and choose `Tutorials`.
...
The Planning Console now exposes continuous mission controls directly: Waypoint Placement has Free Placement, Snap to Cell, and Snap to Feature; Water Column controls switch physical/exploded display and active layers; Dive Planning writes selected dive profile and target-layer metadata; Field Rendering offers Layer Slices, Smoothed Slices, Volumetric Cloud, and Hybrid. Volumetric Cloud currently reports the WebGL layered-slice fallback. See [Continuous Mission UI Runtime Stabilization](docs/continuous_mission_ui_runtime_stabilization.md), [Continuous Mission Geometry](docs/continuous_mission_geometry.md), [Canonical 3D Glider Dive Execution](docs/canonical_3d_glider_dive_execution.md), and [Volumetric Field Interpolation and Rendering](docs/volumetric_field_interpolation_and_rendering.md).
...
DIVE-UX-R1 makes the right Mission Waypoints panel the selected-segment editor. Click a destination waypoint card to edit the flight profile of the segment that reaches it: W1 edits Selected Start -> W1, W2 edits W1 -> W2, and so on. Apply commits through the canonical planning commands; Cancel discards the draft. The left Mission Console remains for mission-wide controls, display controls, execution, imports, exports, and global/default setup. See [Right Panel Route Instruction Editor](docs/right_panel_route_instruction_editor.md) and [Incoming Segment Flight Profile UX](docs/incoming_segment_flight_profile_ux.md).
