...
Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The atlas is synthetic and normalized, not real latitude/longitude, real Earth imagery, or an operational forecast. Generated bathymetry uses structured procedural features, distance fields, seeded feature placement, controlled roughness, smoothing, validation, and provenance; noise is not the sole terrain model. It is not a volumetric geology editor and not a calibrated real-ocean bathymetry product. It does not launch missions from edited projects, regenerate currents/scalars/hotspots, add sculpting, change scientific equations, alter scoring, or add planners.
