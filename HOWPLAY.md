...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. Use it to generate a synthetic world map from a style and seed, pan/zoom across the tiled map, inspect layers, draw an operational boundary window, inspect sampled regional context, then generate a regional 3D bathymetry project. Broad world controls and boundary sizing are advanced disclosure controls. The map is field-derived: land/ocean, distance-to-coast, shelf, basin, island/seamount, canyon, river-mouth, strait/sill, coarse flow-regime, scalar-regime, diversity, and suitability hints are deterministic synthetic fields.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The world map is synthetic and normalized, not real latitude/longitude, real Earth imagery, calibrated survey data, or an operational forecast. It is rendered through deterministic tiles/chunks for exploration, not as one DOM cell per map pixel. Generated bathymetry uses structured procedural features, distance fields, seeded feature placement, controlled roughness, smoothing, validation, and provenance; noise is not the sole terrain model. Currents, scalars, hotspots, start/drop-zone validation, benchmark bundles, and launch into Planning remain staged follow-ups. Environment Studio does not alter scoring, planners, mission simulation, or benchmark fairness.
