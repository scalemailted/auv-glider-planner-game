...
Simulation Lab `Demos` includes four field laboratories: Flow Fields Demo for current vectors `F(x,y,t)`, Sample / ROI Field Demo for pure sample value `S(x,y,t)`, Coupled Fields Demo for how currents shape or transport sample value, and Uncertainty / Forecast Demo for forecast, truth, uncertainty, information gain, and update effects. In the pure sample demo, controls are organized as Behavior Preset, Event Likelihood Field, Spatial Pattern, Value Distribution, Spatial Parameters, Temporal Pattern, Spatial Evolution, Motion Scope, State Model, Sampling Effects, and Display. Behavior Presets are curated starting points such as Recurring Hotspots, Migrating Patch, Expanding Front, Patchy Rainfall, Drifting Storm Cells, Freshness / Revisit Value, Neighbor Spread, Oscillating Ecological Field, Forest Fire Front (inspired), and Life-Like Cellular Emergence (inspired). Selecting a preset fills in primitive controls; changing a primitive marks the preset as modified. Event Likelihood Field is the primitive `L(x,y,t)` substrate for event origins, sparse candidate sites, jump destinations, random-walk bias, and propagation likelihood; separated seeded anchors keep multi-modal, patchy, and sparse candidate examples from collapsing into one quadrant. It can be static or dynamic; dynamic likelihood has its own temporal pattern and spatial evolution controls, separate from the observed sample-value controls. The Display Layer can show Event Likelihood directly or overlay high-likelihood cells on top of Sample Value, making it easier to compare where events tend to originate with where reward is currently realized. Spatial Pattern covers Constant Field, Gradient / Trend, Clustered Field, Patchy / Correlated Field, Sparse Targets, Linear Band, Front / Boundary, Boundary Band, Monitoring Stations, and Seeded Texture. Value Distribution covers Constant Value, Uniform Random, and Gaussian / Normal. Cluster Count and Cluster Size parameterize `Clustered Field`; single-cluster, two-cluster, and multi-cluster cases are not separate theories. Temporal Pattern includes persistent sustained/periodic/bursty/intermittent/rapid/random/wavy modes plus explicit finite Pulse Then Silence and semi-finite Long-Tail Decay examples. The left panel stays focused on compact controls and Explain buttons; clicking Explain opens right-panel Behavior Help for Behavior Preset, Event Likelihood Field, Spatial Pattern, Value Distribution, Temporal Pattern, Spatial Evolution, State Model, Sampling Effect, or Display Layer. Clicking a heatmap cell switches the right panel back to Cell Inspector with event likelihood, the selected pattern, Motion Scope, and relevant parameters. Spatial Evolution controls whether a pattern is stationary, drifts continuously, jumps between bursts, wanders, or propagates to neighbors. Motion Scope defaults to Per Feature so drift and random walk move features or local regions rather than sliding the whole heatmap; Global is an explicit advanced comparison. Presets inspired by real-world processes are simplified educational examples, not validated process models. Forecast/truth/uncertainty belongs in the Uncertainty / Forecast Demo. See [Sample / ROI Field Demo](docs/sample_fields_demo.md) and [Uncertainty / Forecast Demo](docs/uncertainty_forecast_demo.md).

Keep the field questions separate: `F(x,y,t)` asks how water moves, `L(x,y,t)` asks where events tend to originate, `S(x,y,t)` asks where sampling has realized value, and `U(x,y,t)` asks what the planner does not know.
...
In the Sample / ROI Field Demo, `Event Likelihood Field`, `Spatial Pattern`, and `Value Distribution` are separate concepts. Event Likelihood Field describes where events are prone to originate or move next. Spatial Pattern describes geometry such as Constant Field, Clustered Field, bands, fronts, or seeded texture. Value Distribution describes how cell values are assigned inside that geometry, such as Constant Value, Uniform Random, or Gaussian / Normal.
