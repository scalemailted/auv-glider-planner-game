...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. It uses preprocessed public bathymetry/topography fixtures as the default Reference Bathymetry Atlas source. The browser app does not download NOAA or GEBCO data at runtime; raw source files live under gitignored `external_data/reference_bathymetry/`, and compact runtime fixtures live under `assets/reference_bathymetry/`. The checked-in state currently reports `NO_REFERENCE_DATA_FIXTURE`, so the Studio shows blocked instructions instead of presenting procedural placeholder bathymetry as reference data. Run `npm.cmd run download:reference-bathy`, `npm.cmd run preprocess:reference-bathy`, then `npm.cmd run audit:reference-bathy` to populate fixtures. The procedural synthetic globe/world workflow remains available only as an experimental sandbox.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. Reference patch export preserves atlas/patch/bathymetry digests, validation status, wet/land and coastline summaries, and deferred current/scalar/hotspot dependency states. Currents, scalars, hotspots, start/drop-zone validation, benchmark bundles, and launch into Planning remain staged follow-ups. Environment Studio does not alter scoring, planners, mission simulation, or benchmark fairness.
