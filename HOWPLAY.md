...
Environment Studio lives under `Simulation Lab -> Editor & Import Tools`. Use it to define a browser-friendly domain and resolution, select a seeded synthetic bathymetry archetype, generate a compact tile or 2x2 mosaic, inspect validation and dependency state, and import/export `anchor.environment-studio-project` JSON. It is an authoring thin slice only: it does not launch missions from edited projects, regenerate currents or scalar fields, change scientific equations, alter scoring, or add planners.
