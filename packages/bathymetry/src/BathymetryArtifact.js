...
import { normalizeBathymetryManifest, bathymetryManifestDigest } from './BathymetryManifest.js';
import { createBathymetrySourceMetadata } from './BathymetrySourceMetadata.js';

export const BATHYMETRY_ARTIFACT_VERSION = 'bathymetry-artifact-bathy-pkg-r1';

export function createBathymetryArtifact(options = {}) {
  return normalizeBathymetryArtifact(options);
}

export function normalizeBathymetryArtifact(value = {}) {
  ...
  const manifest = normalizeBathymetryManifest(value.manifest ?? {
  ...
  const artifactBase = {
    ...
    sourceMetadata: createBathymetrySourceMetadata(value.sourceMetadata ?? bathymetry.sourceMetadata ?? {}),
    provenance: normalizeProvenance(value.provenance ?? bathymetry.provenance ?? {}),
    manifest,
    manifestDigest: value.manifestDigest ?? manifest.manifestDigest ?? bathymetryManifestDigest(manifest),
...
export function validateBathymetryArtifact(value = {}) {
...
export function bathymetryArtifactSummary(value = {}) {
  const artifact = value.type === 'anchor.bathymetry.artifact' ? value : normalizeBathymetryArtifact(value);
...
export function bathymetryArtifactDigest(value = {}) {
  const payload = { ...value };
