import { fieldDigestOf, stableDigest } from './EnvironmentUtil.js';

export const ENVIRONMENT_FIELD_REGISTRY_VERSION = 'environment-field-registry-env-pkg-r1';

export function createEnvironmentFieldRegistry(options = {}) {
  const entries = [
...
export function normalizeEnvironmentFieldRegistry(value = {}) {
...
export function validateEnvironmentFieldRegistry(value = {}) {
...
export function environmentFieldRegistrySummary(value = {}) {
  const registry = normalizeEnvironmentFieldRegistry(value);
  const roleCounts = {};
...
function entriesFromBathymetry(bathymetry = null, role = null) {
  ...
  return [normalizeEntry({
    ...
    artifactDigest: fieldDigestOf(bathymetry),
...
function entriesFromCurrents(fields = [], roles = {}) {
  return normalizeArray(fields).map((field, index) => {
    ...
    return normalizeEntry({
      ...
      artifactDigest: fieldDigestOf(field),
