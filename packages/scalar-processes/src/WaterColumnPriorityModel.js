import { collapseWaterColumnField, bestWaterColumnDepthLayer, waterColumnFieldSummary } from './WaterColumnFieldModel.js';
import {
...
export const WATER_COLUMN_PRIORITY_MODEL_VERSION = 'water-column-priority-model-p11';

export function computeWaterColumnPriority(fieldPack = {}, configInput = {}, options = {}) {
  ...
  const topDown = collapseWaterColumnField(priorityField, config, { method: collapseMethod, accessibilityMask });
  const best = bestWaterColumnDepthLayer(priorityField, config, { accessibilityMask });
  const diagnostics = waterColumnFieldSummary(priorityField, config, { fieldId: 'A_global', accessibilityMask });
  const summary = {
...
export function summarizeWaterColumnPriority(priorityArtifact = {}) {
...
export function bestWaterColumnPriorityLayer(priorityArtifact = {}) {
...
export function validateWaterColumnPriorityArtifact(artifact = {}) {
  const errors = [];
  const warnings = [];
