...
import { computeScalarFieldDiagnostics, scalarFieldDigest } from '../ScalarFieldDiagnostics.js';

export const ATLAS_CONDITIONED_SCALAR_BUILDER_VERSION = 'atlas-conditioned-scalar-builder-field-regen-r1';

export function createAtlasConditionedScalarField(options = {}) {
  return buildAtlasConditionedScalarArtifact(options).scalarArtifact;
}

export function buildAtlasConditionedScalarArtifact(options = {}) {
  ...
  scalarArtifact.diagnostics = computeScalarFieldDiagnostics(scalarArtifact);
  scalarArtifact.digest = scalarField4DDigest(scalarArtifact);
  const validation = validateScalarField4D(scalarArtifact);
  const summary = scalarField4DSummary(scalarArtifact);
...
export function atlasScalarComponentPlan(options = {}) {
...
function createHotspotArtifact(options = {}) {
  ...
  return {
    ...artifactBase,
    hotspotDigest: scalarFieldDigest(artifactBase)
  };
}
