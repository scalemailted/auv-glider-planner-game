import { artifactDigest, digestRecord } from './digests.js';

export const ANCHOR_CONTRACTS_VERSION = 'anchor-contracts-arch-r1';

export const COORDINATE_FRAMES = Object.freeze({
  GRID: 'grid-index',
...
export const TYPED_ARRAY_LAYOUTS = Object.freeze({
...
export function normalizeAxisDefinition(axis = {}, fallbackName = 'axis', units = PHYSICAL_UNITS.SCALAR_VALUE) {
...
export function normalizeBathymetryManifest(input = {}) {
  ...
  return {
    ...manifest,
    digest: artifactDigest(manifest),
  };
}

export function validateBathymetryManifest(input = {}) {
  const errors = [];
  const warnings = [];
...
export function normalizeBathymetryArtifact(input = {}) {
...
export function validateBathymetryArtifact(input = {}) {
  const errors = [];
  const warnings = [];
...
export function normalizeCurrentFieldManifest(input = {}) {
  const axes = input.axes || {};
  const manifest = {
  ...
  return { ...manifest, digest: artifactDigest(manifest) };
}

export function normalizeCurrentField4D(input = {}) {
  const artifact = {
...
export function normalizeScalarProcessManifest(input = {}) {
  const manifest = {
...
export function normalizeScalarField4D(input = {}) {
  const artifact = {
...
export function normalizeSyntheticEnvironmentManifestContract(input = {}) {
  const manifest = {
...
export function normalizeGeneratedEnvironmentArtifactContract(input = {}) {
  const artifact = {
...
export { artifactDigest, digestRecord };
