import { MISSION_SCORE_GROUP_IDS, createMissionScoreConfig } from './MissionScoringSchema.js';
import { missionScoreComponentById } from './MissionScoreComponents.js';

export const MISSION_SCORE_AGGREGATOR_VERSION = 'mission-score-aggregator-score-r1';

export function aggregateScoreGroup(metrics = [], groupId, profile = {}, options = {}) {
  ...
  for (const metric of metricList) {
    const definition = missionScoreComponentById(metric.componentId);
...
export function aggregateMissionOutcomeScore({ normalizedMetrics, profile, scoreConfig, options = {} } = {}) {
  const config = createMissionScoreConfig({ ...(scoreConfig ?? {}), profileId: profile?.id ?? scoreConfig?.profileId, minimumCoverageFraction: scoreConfig?.minimumCoverageFraction ?? profile?.minimumCoverageFraction });
...
export function missionScoreAggregationSummary(result = {}) {
  return {
