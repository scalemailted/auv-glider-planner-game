import { missionOutcomeReportSummary } from './MissionOutcomeReport.js';
import { missionRegretReportSummary } from './MissionRegretModel.js';

export const MISSION_SCORECARD_VIEW_MODEL_VERSION = 'mission-scorecard-view-model-score-r1';

export function buildMissionScorecardViewModel({ missionOutcomeReport = null, regretReport = null, scoreProfile = null, options = {} } = {}) {
  const summary = missionOutcomeReportSummary(missionOutcomeReport ?? {});
  const regret = regretReport ? missionRegretReportSummary(regretReport) : missionOutcomeReport?.regretSummary ?? null;
...
export function missionScorecardViewModelSummary(viewModel = {}) {
  return {
