...
} from './BathymetryConditionedCurrentBuilder.js';

export const ATLAS_CONDITIONED_CURRENT_BUILDER_VERSION = 'atlas-conditioned-current-builder-field-regen-r1';
...
export function createAtlasConditionedCurrentField(options = {}) {
  return buildAtlasConditionedCurrentArtifact(options).currentArtifact;
}

export function buildAtlasConditionedCurrentArtifact(options = {}) {
  ...
  const base = createBathymetryConditionedCurrentField({
  ...
  const currentArtifact = createOceanCurrentField4D({
  ...
  currentArtifact.scientificDiagnostics = computeCurrentFieldScientificDiagnostics(currentArtifact);
  currentArtifact.digest = oceanCurrentField4DDigest(currentArtifact);
  const validation = validateOceanCurrentField4D(currentArtifact);
  const summary = oceanCurrentField4DSummary(currentArtifact);
  return {
...
export function atlasCurrentComponentPlan(options = {}) {
