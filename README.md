...
Environment Studio now starts from a deterministic synthetic globe rendered from high-resolution equirectangular world fields. The globe is not Earth and not operational ocean data; it is a synthetic field artifact used to select small operational windows for benchmark environment generation. The Stage 1 workflow is globe exploration, layer inspection, region selection, and regional bathymetry generation; generator tuning is advanced disclosure. Generated bathymetry is built from structured procedural features, distance fields, seeded feature placement, controlled roughness, validation, and provenance. Currents, scalars, hotspots, and launch into Planning remain staged follow-ups unless explicit adapters generate those artifacts.
...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Synthetic Globe** backed by deterministic high-resolution equirectangular synthetic world fields: choose a world style and seed, rotate/zoom the globe, inspect semantic layers, select a small operational globe region, inspect sampled regional context, then generate a regional 3D bathymetry project from that selected window. Broad generator controls and boundary sizing are advanced disclosure controls. Feature summary, validation, dependency state, and diagnostics remain available after generation as secondary controls.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The synthetic globe is deterministic and synthetic, not real Earth, GEBCO, ETOPO, HYCOM, Copernicus, calibrated survey data, certified navigation data, or an operational forecast. Display textures may be downsampled for browser interactivity, while the canonical equirectangular artifact remains deterministic and reproducible.

Environment Studio does not launch missions from edited projects yet. It preserves compact globe/window metadata, sampled context, bathymetry/coastline digests, open boundaries, feature zones, regime hints, axes, and grid shapes. The ENV-GLOBE-R1 visual acceptance package is generated under `test-results/env-globe-r1-owner-review/` by the focused browser workflow. Currents, scalars, hotspots, start/drop-zone validation, benchmark bundles, and launch-to-planning remain staged follow-ups unless a later validated workflow generates them. This does not add sculpting, import real GEBCO/ETOPO patches, alter mission simulation, change official scoring, add planners, or create a fifth Product Hub pillar. See [Environment Studio Architecture](docs/environment_studio_architecture.md) and [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md).
