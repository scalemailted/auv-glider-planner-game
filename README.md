...
Environment Studio now starts from a **Reference Bathymetry Atlas** workflow backed by compact preprocessed public bathymetry/topography artifacts under `assets/reference_bathymetry/`. The browser app does not download NOAA or GEBCO data at runtime, and raw source files belong under gitignored `external_data/reference_bathymetry/`. The checked-in manifest currently reports `NO_REFERENCE_DATA_FIXTURE`, so reference-backed selection/generation is intentionally blocked until `npm.cmd run download:reference-bathy` and `npm.cmd run preprocess:reference-bathy` produce fixtures. The procedural synthetic world/globe path remains available only as an experimental sandbox / compatibility workflow. Currents, scalars, hotspots, and launch into Planning remain staged follow-ups unless explicit adapters generate those artifacts.
...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Reference Bathymetry Atlas**: when preprocessed fixtures exist, users inspect source metadata, select a lon/lat bounding box, review selected patch depth/wet-land/coastline summaries, then generate a regional 3D bathymetry project from that selected patch. The checked-in manifest is labeled `NO_REFERENCE_DATA_FIXTURE`, so the app shows a blocked state and instructions instead of rendering procedural placeholder data as reference bathymetry. The procedural synthetic world/globe workflow is demoted to an experimental sandbox / compatibility path.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. Reference bathymetry uses preprocessed public bathymetry/topography source data, not calibrated survey data, certified navigation data, HYCOM, Copernicus, or an operational forecast. A real preprocessed reference bathymetry fixture is required before claiming true BATHY-DATA-R1 completion. See [Reference Bathymetry Data Pipeline](docs/reference_bathymetry_data_pipeline.md).
