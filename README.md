...
Environment Studio now starts from a **Reference Bathymetry Atlas** workflow backed by compact preprocessed public bathymetry/topography artifacts under `assets/reference_bathymetry/`. The browser app does not download NOAA or GEBCO data at runtime, and raw source files belong under gitignored `external_data/reference_bathymetry/`. The checked-in manifest currently reports `AVAILABLE` with a true ETOPO 2022 15 arc-second Monterey Canyon `missionReadyPatch` cropped from the NOAA surface-elevation non-ice fallback tile, while preserving the older ETOPO 2022 60 arc-second bedrock patch as a `lowResolutionReferencePatch`. The procedural synthetic world/globe path remains available only as an experimental sandbox / compatibility workflow. The explicit FIELD-REGEN-R1 action now follows the public reference bathymetry patch -> bathymetry artifact -> synthetic bathymetry-conditioned fields -> environment artifact path, generating deterministic synthetic currents, scalars, hotspots, and hazard candidates without launching missions or changing scoring.
...
Environment Studio does not launch missions from edited projects yet. It preserves compact reference dataset identity, manifest/atlas digest, selected patch digest, bathymetry/coastline digests, validation status, field-regeneration digests, axes, grid shapes, and dependency states. The BATHY-DATA-R1 owner-review package is generated under `test-results/bathy-data-r1-owner-review/` by the focused browser workflow. FIELD-REGEN-R1 can generate deterministic synthetic benchmark currents, scalars, hotspots, start/drop-zone candidates, hazard candidates, and a composed-or-honestly-deferred environment artifact from the reference patch; start/drop-zone validation, benchmark bundles, and launch-to-planning remain staged follow-ups. This does not add sculpting, alter mission simulation, change official scoring, add planners, or create a fifth Product Hub pillar. See [Environment Studio Architecture](docs/environment_studio_architecture.md), [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md), and [Reference Bathymetry Data Pipeline](docs/reference_bathymetry_data_pipeline.md).
