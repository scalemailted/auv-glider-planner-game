...
Environment Studio now uses a deterministic Synthetic Ocean Atlas field engine for operational-window selection. The atlas is not a real Earth map; it is a synthetic context map for benchmark-oriented mission-window selection. Generated bathymetry is built from structured procedural features, distance fields, seeded feature placement, controlled roughness, validation, and provenance. Noise is used for variation and texture, not as the sole terrain model. Current/scalar/hotspot regeneration and launch into Planning remain staged follow-ups unless explicit adapters generate those artifacts.
...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Synthetic Ocean Atlas**: choose a synthetic atlas preset, select or adjust an operational window, inspect inferred regional context, then generate a Regional Mission Recipe and regional 3D bathymetry project. Feature mix, tile provenance, validation, dependency state, and diagnostics remain available after generation as secondary controls.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The atlas is a deterministic synthetic context map, not a real Earth map. Large domains use preview decimation for interactivity, while exported source grids remain deterministic and reproducible. Current synthetic bathymetry is scientifically constrained and validation-aware, but not a calibrated real-ocean bathymetry product or operational forecast.

Environment Studio does not launch missions from edited projects yet. It preserves compact field-generation inputs such as masks, bathymetry/coastline digests, open boundaries, feature zones, regime hints, axes, and grid shapes. FIELD-REGEN-R1 adds an explicit **Generate Currents & Science Fields** action that creates package-backed synthetic `CurrentField4D`, `ScalarField4D`, and hotspot candidate metadata with digests, diagnostics, and component plans. Starts/drop zones remain `NEEDS_VALIDATION`, benchmark bundles remain `REQUIRES_REGENERATION`, and project export stores compact metadata rather than full 4D arrays. This does not add sculpting, import real GEBCO/ETOPO patches, alter mission simulation, change official scoring, add planners, or create a fifth Product Hub pillar. Generated fields are synthetic atlas-conditioned benchmark fixtures, not HYCOM, Marine Copernicus, calibrated ocean products, operational forecasts, or navigation data. See [Environment Studio Architecture](docs/environment_studio_architecture.md) and [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md).
