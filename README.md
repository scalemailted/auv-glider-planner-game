...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Reference Bathymetry Atlas**: users inspect the compact full-world overview, see available patch coverage, select operational lon/lat windows from local patch through Gulf-scale presets, load a staged Monterey patch when available, review selected patch depth/wet-land/coastline summaries, then generate a regional 3D bathymetry project from that loaded patch. The checked-in manifest is labeled `AVAILABLE` with a compact 60 arc-second global overview selection layer, a Monterey Canyon ETOPO 2022 15 arc-second `missionReadyPatch`, and a preserved 60 arc-second `lowResolutionReferencePatch` fallback. Large valid windows that exceed the live browser budget export `anchor.reference-bathymetry-multitile-patch-request` artifacts instead of generating placeholder data. The procedural synthetic world/globe workflow is demoted to an experimental sandbox / compatibility path.
