...
Normal browser use does not require a build step, backend, or Playwright. Phaser is vendored locally at `vendor/phaser.min.js`; the GFX-R2 bathymetry renderer uses the normal npm `three` dependency, so run `npm install` after a fresh checkout if `node_modules/three` is not present.
...
`Renderer Architecture Preview` is a Simulation Lab scaffold for the rendering stack. It keeps Phaser as the app/game shell, while the active `3D Bathymetric World View` uses a dedicated Three.js/WebGL renderer over public-safe view models. Renderer modules do not own simulation, scoring, planning, hidden truth, WebGPU fluid simulation, Python simulation, or MARL/RL. See [Renderer Architecture and WebGPU Strategy](docs/renderer_architecture_and_webgpu_strategy.md).
