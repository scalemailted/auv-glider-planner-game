...
- game-first mission planning workspace with a large Phaser tactical map, optional live Three.js Bathymetric 3D mission renderer, HTML/CSS mission-control overlays, top selected-glider planning HUD, bottom mission-time slider, waypoint drawer/table, and non-executable planning markers
...
Normal browser use does not require a build step, backend, or Playwright. Phaser is vendored locally at `vendor/phaser.min.js`; the GFX-R2/GFX-R3A/GFX-R3B Three.js renderers use the normal npm `three` dependency, so run `npm install` after a fresh checkout if `node_modules/three` is not present.
...
`Renderer Architecture Preview` is a Simulation Lab scaffold for the rendering stack. It keeps Phaser as the app/game shell, while the active `3D Bathymetric World View` uses a dedicated Three.js/WebGL renderer over public-safe view models. GFX-R3A adds an optional `Three.js Bathymetric 3D` renderer inside the live Mission Planning workspace, and GFX-R3B adds direct planning interactions through canonical workspace commands. It consumes the same public-safe mission world view model as the tactical map; waypoint placement remains 2.5D, depth/dive behavior still comes from existing planning controls, and camera navigation is separated from edit gestures. Renderer modules do not own simulation, scoring, planning, hidden truth, route optimization, WebGPU fluid simulation, Python simulation, or MARL/RL. See [Renderer Architecture and WebGPU Strategy](docs/renderer_architecture_and_webgpu_strategy.md) and [Three.js Mission Renderer Migration](docs/threejs_mission_renderer_migration.md).
...
Planning is the main game workspace. Phaser renders the default mission board, gliders, waypoints, global planning markers, currents, Gold Star priority targets, route overlays, guidance cone, and map hit targets. The World View control can switch the center viewport to a Three.js Bathymetric 3D renderer that shows the same live mission state and supports direct planning tools for hover/inspect, glider and waypoint selection, 2.5D waypoint placement/editing, planning markers, and Gold Star inspection. These interactions edit the same canonical mission plan used by the tactical view; they do not generate or optimize routes. Legacy Phaser remains the default and fallback renderer until simulation and replay/debrief parity are complete. The left Mission Console renders planning controls, status, route/cost estimate, solver/export actions, layer toggles, selected-glider performance details, and execute controls. The right Waypoint Timeline panel renders agent tabs and only the selected glider's executable waypoint sequence with pending/active/completed/missed status. The center viewport includes a top selected-glider planning HUD for projected route cost, estimated remaining fuel, planning window/time, expected ROI estimate, current assist/opposition, next surface estimate, likely reachability, route validity, and compact warnings. The bottom center is reserved for the mission-time slider in Planning and the mission-time playhead during Simulation.
