...
Version 2 is a playable static-web game built with vanilla JavaScript, HTML, CSS, Three.js, schema-driven core modules, and a lazy legacy Phaser island for older labs. The active mission shell is now a DOM router plus Mission Console, Three.js Mission World, and Waypoint Timeline: HTML/CSS owns routing, menus, forms, tables, imports/exports, and results; Three.js owns the production planning/simulation world view and direct mission interaction; the shared simulation core owns physics/scoring; Phaser loads only when a legacy lab route is opened. It has:

- DOM main menu, mission setup, mission briefing, planning, simulation, and debrief views, with legacy level-editor/dataset/lab scenes lazy-loaded through the Phaser island
...
- game-first mission planning workspace with a production Three.js bathymetric mission world, HTML/CSS mission-control overlays, top selected-glider planning HUD, bottom mission-time slider, waypoint drawer/table, and non-executable planning markers
- lazy legacy Phaser island for existing lab/editor/preview scenes while the normal mission route no longer starts Phaser scenes or uses Phaser update loops
...
Normal browser use does not require a build step, backend, Playwright, or `node_modules`. The production Three.js mission environment resolves the locked Three.js runtime through the `index.html` import map to `vendor/three/build/three.module.js`. Phaser remains vendored at `vendor/phaser.min.js` but is lazy-loaded only by the legacy island host when an older lab/editor route is opened. Development uses the npm `three` dependency as the source of truth for refreshing that curated vendor runtime with `npm.cmd run vendor:three`. See [Three.js Static Runtime](docs/threejs_static_runtime.md) and [Three.js First Mission Architecture](docs/threejs_first_architecture.md) for the vendor allowlist, checks, GitHub Pages build path, and Phaser deprecation boundary.

The visible page shell uses three bounded regions: `#mission-console` on the left, `#center-column` / `#game-root` in the center, and `#waypoint-timeline` on the right. The normal route mounts DOM views and the Three.js mission renderer inside the center viewport shell. Phaser mounts there only when a legacy route is explicitly opened. Scenario Setup and Mission Briefing use responsive HTML overlays inside the same center shell, so their cards stretch, wrap, and scroll within the available center space instead of rendering as tiny fixed cards or overlapping the side panels. Menu, setup, briefing, planning, simulation, debrief, and legacy island views follow the same center-region contract: center content is sized from the DOM shell or renderer host, not the full browser window. On narrow or portrait screens, the right waypoint panel hides and the left console stacks above the center viewport. Map bounds are recomputed from the visible top selected-glider HUD and bottom timeline, so the board scales uniformly into the usable simulator area. The map may letterbox inside the full-size canvas when the grid and viewport aspect ratios differ, but grid cells remain square and the map is never stretched independently on x/y. At launch, the DOM Main Menu view renders a full-viewport product hub in the center shell. The left Mission Console stays compact with context guidance, and the right waypoint panel is hidden on the hub until a mission or active scene needs it. The right panel then becomes the authoritative executable waypoint timeline for the selected glider. The center viewport also has compact HTML overlays: a top selected-glider planning HUD and a bottom mission-time slider/playhead. Planning and Simulation both use the bottom DOM timeline panel; Phaser no longer renders playback buttons or a timeline track over the map. Selected-glider performance details now live in the left Mission Console.
...
- [Phaser Migration Runtime](docs/phaser_migration_runtime.md)
...
The Three.js mission world and waypoint drawer both write through the same canonical `anchor.plan` helpers. Map clicks, global marker placement, drawer reorder/delete controls, import, and export stay synchronized through the core planning helpers.
...
- [Bathymetric World View](docs/bathymetric_world_view.md) documents the GFX-R2 Three.js/WebGL bathymetry renderer over the existing 2.5D water-column model, and [Three.js Mission Renderer Migration](docs/threejs_mission_renderer_migration.md) tracks GFX-R3A/GFX-R3B planning parity and MIG-R1 Three.js-first mission rendering.
