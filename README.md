...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It is a unified browser authoring workflow for deterministic synthetic environment projects: choose environment scale, source-grid resolution, regional layout template, feature mix, and randomization settings; generate multi-archetype regional bathymetry; inspect a 3D bathymetry preview plus top-down, seam, slope, wet/land, cross-section, and suitability diagnostics; and import/export `anchor.environment-studio-project` JSON.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. Large domains use preview decimation for interactivity, while exported source grids remain deterministic and reproducible. Current synthetic bathymetry is scientifically constrained and validation-aware, but not a calibrated real-ocean bathymetry product.
