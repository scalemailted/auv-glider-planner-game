...
Environment Studio now starts from a **Reference Bathymetry Atlas** workflow backed by compact preprocessed public bathymetry/topography artifacts under `assets/reference_bathymetry/`. Environment Studio has two stages. The Global Atlas Setup Scene is for selecting and inspecting an operational boundary. Every valid boundary can open an interactive 3D bathymetry preview. If high-resolution staged tiles are not available, the preview uses app-hosted overview/LOD data and is not mission-ready. Field generation, Planning launch, and benchmark export require staged mission-ready bathymetry tiles. Continue to Mission-Ready Bathymetry is enabled only when the selected operational window matches an app-hosted mission-ready tile set. ANCHOR hosts curated app-ready reference bathymetry tiles as static assets. The browser does not download NOAA/GEBCO data at runtime. External public bathymetry data is downloaded and preprocessed offline, then registered as staged ANCHOR tile artifacts. The regional 3D mesh is a decimated visualization artifact. The reference raster/grid remains authoritative for bathymetry sampling, masks, environment generation, simulation, and benchmark export. The checked-in manifest currently reports `AVAILABLE` with a compact ETOPO 2022 60 arc-second global overview selection layer, a true ETOPO 2022 15 arc-second Monterey Canyon `missionReadyPatch` cropped from the NOAA surface-elevation non-ice fallback tile, and the older ETOPO 2022 60 arc-second bedrock patch preserved as a `lowResolutionReferencePatch`. The static tile library stages those Monterey rasters plus non-authoritative mesh LODs and leaves Gulf regions request-only until offline preprocessing is run. The Pages build includes these staged static assets and the tile-library loader. The global atlas overview is not mission-resolution bathymetry; mission-ready environments use staged high-resolution regional patches. The atlas supports deep zoom and operational-window editing. The atlas operational window can be moved and resized directly. Drag inside the rectangle to move it, drag edges to resize one side, and drag corners to resize both connected sides. Large operational windows are valid selections, but live Alpha generation remains budget-gated; oversized regions export patch or multi-tile requests. Large Gulf-scale windows can be selected, previewed interactively with adaptive LOD, and exported as multi-tile patch requests, but live Alpha generation remains limited to staged browser-safe regional patches. The global atlas now separates the selected `OperationalWindow` from the browser `GenerationBudget`: large Gulf-scale windows can be selected and exported as multi-tile patch requests, while live Alpha generation remains disabled for regions that need offline preprocessing. The procedural synthetic world/globe path remains available only as an experimental sandbox / compatibility workflow. The implemented reference-derived path is: public reference bathymetry patch -> bathymetry artifact -> deterministic synthetic bathymetry-conditioned fields -> package-backed environment artifact -> validated Planning launch -> public benchmark bundle. Generated currents, scalars, and hotspots are deterministic synthetic benchmark fields, not operational forecast products, and the path does not change simulation or scoring.

ENV-STUDIO-HANDOFF-R0 adds a product handoff spec and a curated-region selector scaffold. Users can choose seed regions such as Monterey Canyon, Hawaii / Island Slope, Puerto Rico Trench / Island Shelf, Florida Straits, Gulf Shelf / Canyon Segment, Northeast US Shelf Break, California Shelf Break, and Alaska Fjord / Shelf Region to place editable operational windows. Bathymetry mode architecture now names Real Reference, Reference-Enhanced Synthetic, and Fully Synthetic Sandbox while keeping mission authority explicit and unchanged. See [Environment Studio Product Handoff](docs/environment_studio_product_handoff.md).
...
- typed export products for replayable challenges, solver packets, oracle datasets, structured results, plans, leaderboard records, and single-frame demo artifacts
...
9. Click `Execute`.
10. Review Debrief score, objectives, rating, and suggestions.
11. Retry, revise, continue to the next tutorial, generate another challenge, return to the editor, or export data for solver experiments.

## Documentation Index

- [Game design](docs/game_design.md)
- [Scientific AUV Planning Game Design](docs/game_design_scientific_auv_planning.md)
- [ANCHOR Alpha Release](docs/alpha_release.md)
- [Environment Studio Product Handoff](docs/environment_studio_product_handoff.md)
...
- [Sampling Priority to Glider Action Value Learning Lab](labs/sampling-priority-to-glider-action-value.html)
- [Legacy Sample / ROI Field Demo notes](docs/sample_fields_demo.md)
- [ROI Generator Demo](docs/roi_generator_demo.md)
- [Coupled Fields Demo](docs/coupled_fields_demo.md)
...
- [Right Panel Route Instruction Editor](docs/right_panel_route_instruction_editor.md)
- [Incoming Segment Flight Profile UX](docs/incoming_segment_flight_profile_ux.md)
- [DIVE-UX-R1 Visual Acceptance](docs/dive_ux_r1_visual_acceptance.md)
...
The Main Menu product hub includes field demos inside the `Simulation Lab` submenu. Demos are isolated concept scenes for validating field behavior before those fields are used inside full missions.

`Flow Fields Demo` is a Phaser sandbox for teaching and validating current vectors: the map shows an arrow grid, glider-like particles drift through the selected field, and particle heading follows movement. The demo starts in Dynamic mode with `Topology-Aware Composite`, a Blended Coastal Map, High dynamic complexity, High direction and magnitude variation, Meander spatial motion, Deflect Along Shore boundary handling, one Base Flow Field, and no additive layers. Mode only controls Static vs Dynamic field evolution. Dynamic mode uses continuous `demoTime` plus derived flow sample time with explicit evolution behavior controls (`Continuous`, `Looping / Cyclic`, `One-Shot Pulse`, `Meandering / Translating`), spatial motion, deterministic direction variation, magnitude variation, dynamic complexity, boundary mode, and evolution pattern controls so topology-aware fields visibly morph over time. Composition is handled by a `+ Add Flow Field` stack with per-layer weights, influence modes (`Global Blend`, `Spatial Pocket`, `Partitioned Region`), enable/remove controls, terrain modes for blended coasts, coast/island mixes, estuaries, channels, bays, island chains, and open water, playback-speed, flow-evolution-speed, magnitude-scale, and particle-speed controls. Reset, direction, pause/resume, and the unbounded `Demo Time` readout live in the compact bottom transport strip rather than on the visualization canvas; Main Menu navigation stays in the left console footer. Arrow length, opacity, and thickness encode current magnitude, and the console reports Current Field Diagnostics: speed min/mean/max, divergence, vorticity, strain, invalid vectors, terrain masking, claim level, and what the field is not. Land is deterministic from the demo seed; arrows over land are hidden, land cells are masked for diagnostics, and particles reset on land collision. Particle Speed is a visual tracer multiplier after sampling `F(x,y,t)`, while Magnitude Scale changes arrow display length only. It does not create missions, waypoints, scores, leaderboard entries, or route-validation state. See [Flow Fields Demo](docs/flow_fields_demo.md) for the controls, implementation files, and limitations.
...
Field concepts stay separate: `F(x,y,t)` is a physical current vector field, `C_k(t)` is cluster/community likelihood, `L_i(t)` is cell likelihood/readiness, `A_i(t)` is cell activation, `S(x,y,t)` is realized sample value, and `U(x,y,t)` is uncertainty or forecast error. `L(x,y,t)` is rendered as a cell-centered likelihood mesh: every cell has a probability-like value, while clusters/nodes/modes are sources or basins that influence that mesh. Dynamic sample fields can run on a hierarchical graph-message layer where clusters define regional tendency, cells define local likelihood and activation, neighbor edges pass influence, and node state/cooldown/freshness updates produce the next likelihood and sample-value fields. Presets now expose process contracts and Recipe View metadata so they remain transparent component recipes rather than opaque behavior models. Graph-backed fields emit edge-message and node-transition metadata for inspection/export, with inferred messages used only as a diagnostic fallback. Cellular automata are one special case of this model. `V_L(x,y,t)` and `V_S(x,y,t)` describe likelihood/sample feature evolution such as drift, ripple, front motion, random walk, or local activation; they are teaching analogs, not ocean flow.

`Coupled Fields Demo` is the Oracle / Deterministic Coupled Sampling Space playground. It combines known process field `C(x,y,t)`, known flow `F(x,y,t)`, known constraints, and deterministic oracle objective `S*(x,y,t)`. It now includes analytical scalar update engines such as Gaussian patch, source/diffusion/decay, advection/diffusion/decay, growth/diffusion/decay, front/boundary approximation, plus a CA/grid-process baseline. It is not a calibrated ocean model and does not include uncertainty or belief. See [Coupled Fields Demo](docs/coupled_fields_demo.md).
...
`Renderer Architecture Preview` is a Simulation Lab scaffold for the rendering stack. Three.js is now the production mission environment for planning and live simulation rendering; Phaser remains transitional shell/lab infrastructure plus the query-gated diagnostic fallback. Renderer modules consume public-safe view models and do not own simulation, scoring, planning, hidden truth, route optimization, WebGPU fluid simulation, Python simulation, or MARL/RL. THREE-R1.1 stabilizes the Mission Workspace runtime, canvas-based pointer calibration, and canonical drop-zone deployment selection; THREE-R1.1B adds visible Planning Tools and camera-control parity; see [Three.js Workspace Stabilization](docs/threejs_workspace_stabilization.md) and [Three.js Planning Tools and Camera Controls](docs/threejs_planning_tools_and_camera.md). See [Renderer Architecture and WebGPU Strategy](docs/renderer_architecture_and_webgpu_strategy.md), [Three.js Mission Renderer Migration](docs/threejs_mission_renderer_migration.md), and [Three.js First Mission Architecture](docs/threejs_first_architecture.md).
...
`Benchmark Modes` defines Planner Benchmark, Adaptive Benchmark, and Full Autonomy Benchmark. Planner Benchmark uses the existing planning workspace, simulator, and Debrief to emit benchmark route/result exports. P6 gives Adaptive Benchmark a transparent mission-manager preview that diagnoses evidence and recommends objectives while the player or solver still chooses the route. It does not implement adaptive route execution, a new planner, scoring redesign, full autonomy, or MARL/RL. See [Benchmark Modes](docs/benchmark_modes.md), [Benchmark Route Execution Contract](docs/benchmark_route_execution_contract.md), [Planner Benchmark Execution](docs/planner_benchmark_execution.md), [Planner Benchmark Route Overlay](docs/planner_benchmark_route_overlay.md), and [Adaptive Benchmark Mission Manager](docs/adaptive_benchmark_mission_manager.md).
...
Currents are mechanically meaningful, not just visual arrows. The Flow Fields Demo teaches the 2D concept `F(x, y, t) = <u, v>`, while generated water-column missions sample the production 4D current field `F(x, y, z, t)` from `OceanCurrentField4D` using canonical mission time. The same current semantics feed glider drift, speed over ground, ETA, fuel/energy, cross-current risk, shoreline risk, Travel Cost, Risk/Safety, Greedy Planner scoring, route diagnostics, simulation playback, debrief metrics, and solver exports. Synthetic generated currents are deterministic browser-side teaching fields and explicitly not calibrated HYCOM, Marine Copernicus, or operational forecast products.

FLOW-PKG-R2 makes `packages/currents` the production owner for bathymetry-conditioned current generation. Existing generated-current behavior is preserved as `cpuBathymetryConditionedSyntheticV2`; normal new mixed regional missions use `cpuBathymetryConditionedSyntheticV3` with declared vertical profiles. Equal currents across depth are valid only for an explicit depth-uniform or vertically mixed regime such as `barotropicDepthUniform`; otherwise depth layers may differ coherently by magnitude, direction, thermocline jet, shear, or bottom-boundary behavior. Three.js displays this structure but does not create it. See [Depth-Structured Current Generation](docs/depth_structured_current_generation.md) and [FLOW-PKG-R2 Migration Notes](docs/flow_pkg_r2_migration.md).
...
Before `Execute`, the game validates that the level, mission, starts, deployment selections, time settings, waypoint coordinates, route segments, estimated arrival time, and estimated fuel are executable. Invalid plans stay in Planning, disable the visible Execute controls, mark the affected waypoint in the right timeline, color invalid bottom-timeline waypoint icons, and highlight blocked map segments with a breach marker. Over-duration future waypoints are non-blocking warnings; terrain, invalid starts, invalid waypoint cells, and fuel failures remain hard blockers. The simulation engine also has max-step and invalid-position abort guards so direct imports or edge cases stop with a visible result rather than freezing the browser.
...
- time-aware `ROI Mode` planning perspectives: Value, Probability, Expected, Remaining, merged Risk / Safety, and Travel Cost
...
The Three.js mission world and waypoint drawer both write through the same canonical `anchor.plan` helpers. Map clicks, global marker placement, drawer reorder/delete controls, import, and export stay synchronized through the core planning helpers. Deployment start changes use a separate Three intent and canonical selected-start helper, so choosing a drop-zone cell does not create waypoint 1.
...
If simulation stops after blocked movement, unreachable waypoints, fuel failure, or a missed-waypoint cascade, the simulation pauses into a Route Failure modal instead of appearing frozen. Mission time expiration is a normal terminal condition: the glider stops at its last simulated position, active/later waypoints are marked missed, and debrief records `mission_time_expired`. The route-failure modal explains hard failed waypoints, reason, last successful waypoint, current glider position/time, and suggested fix. Recovery choices are `Replan`, `Skip WP`, `Continue`, `Debrief`, and `Main Menu` when safe for that failure type. The left Simulation Console also shows fallback recovery buttons. The right waypoint timeline marks missed waypoints with reason labels such as `MISSED: ROUTE BLOCKED`, `MISSED: UNREACHABLE`, `MISSED: MISSION TIME EXPIRED`, or `MISSED: FUEL EXCEEDED`.
...
Simulation resolves the waypoint plan under simplified educational physics from a clone-safe Execute launch payload. Playback controls live outside the mission world in the bottom timeline panel: Start, Prev, Next, End, Play/Pause, Step, and Finish. The left Simulation Console keeps primary recovery/navigation actions, while the Three.js mission world remains focused on terrain, currents, hazards, gliders, planned route, realized trajectory, sampled observations, surfacing events, and route/failure overlays. Three.js renders simulation state only; the portable simulation engine owns time, motion, observations, scoring, and replay semantics.
...
For surface-only missions, scheduled surfacing pauses now use an explicit surface-decision state. A browser dialog appears above the simulation map with Continue, Update Waypoints / Replan, Export, Import, and Finish actions, with the Simulation Console as an inline fallback. Update Waypoints enters Planning from the actual surfaced position; Commit Replan and Resume validates the edited route and resumes the paused simulation, while Cancel Replan returns to the same surface decision. Keyboard fallbacks remain available: `C` continues, `U` returns to waypoint planning from the surfaced position, and `F` finishes to Debrief.
...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Reference Bathymetry Atlas** staged workflow: users inspect the compact full-world overview, use deep zoom, see available patch coverage, select or type operational lon/lat windows from local patch through Gulf-scale presets, move the selected rectangle directly, and open bathymetry preview for every valid operational boundary. Continue to Mission-Ready Bathymetry remains enabled only when the selected window matches an app-hosted mission-ready tile set. Request-only and Gulf-scale regions open a coarse global-overview preview, but Generate Fields, Compose Environment, Launch Planning, and benchmark export remain disabled until app-hosted staged bathymetry exists. The atlas operational window can be moved and resized directly. Drag inside the rectangle to move it, drag edges to resize one side, and drag corners to resize both connected sides. Large operational windows are valid selections, but live Alpha generation remains budget-gated; oversized regions export patch or multi-tile requests. The checked-in manifest is labeled `AVAILABLE` with a compact 60 arc-second global overview selection layer, a Monterey Canyon ETOPO 2022 15 arc-second `missionReadyPatch`, a preserved 60 arc-second `lowResolutionReferencePatch` fallback, and request-only Gulf entries including `gulf_segment_demo_15s`. Large valid windows that exceed the live browser budget export `anchor.reference-bathymetry-multitile-patch-request` artifacts instead of generating placeholder data. The procedural synthetic world/globe workflow is demoted to an experimental sandbox / compatibility path.

The Global Atlas Setup Scene also includes a Curated Region selector and a bathymetry mode scaffold. Curated presets place editable operational windows and focus the atlas; they do not download NOAA/GEBCO data and do not claim mission readiness unless matching staged tiles exist. Bathymetry modes distinguish Real Reference, Reference-Enhanced Synthetic, and Fully Synthetic Sandbox; only Real Reference is the current implemented authority path.
...
Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The regional 3D mesh is a decimated visualization artifact. The reference raster/grid remains authoritative for bathymetry sampling, masks, environment generation, simulation, and benchmark export. Reference bathymetry uses preprocessed public bathymetry/topography source data, not calibrated survey data, certified navigation data, HYCOM, Copernicus, or an operational forecast. The preferred fixture is true 15 arc-second ETOPO 2022 reference data for Monterey Canyon; the 60 arc-second fixture remains available only as a low-resolution fallback. See [Reference Bathymetry Data Pipeline](docs/reference_bathymetry_data_pipeline.md).

Environment Studio can now compose a package-backed `EnvironmentArtifact`, validate start/drop-zone and public-safety readiness, launch the generated environment shell into the Planning workspace, and export a public `anchor.classical-planner-benchmark-bundle`. It preserves compact reference dataset identity, manifest/atlas digest, global overview digest, selected/loaded patch digest, curated-region metadata, bathymetry-mode claim metadata, bathymetry/coastline digests, field-regeneration digests, launch-validation digests, classified launch-warning summaries, benchmark-bundle digests, axes, grid shapes, and dependency states. The BATHY-DATA-R1 owner-review package is generated under `test-results/bathy-data-r1-owner-review/`; the REF-ATLAS-UX-R1.2 owner-review package is generated under `artifacts/owner-review/ref-atlas-ux-r1-2/`; the static tile-library alpha-readiness package is generated under `artifacts/owner-review/ref-tile-lib-r1a/` by `node tools/js/audit_reference_tile_library_alpha_readiness.mjs`; the multi-tile operational-area package is generated under `artifacts/owner-review/multitile-oparea-r1/` by `node tools/js/audit_multitile_operational_area_flow.mjs`; and the Environment Studio UX polish package is generated under `artifacts/owner-review/env-studio-ux-polish-r2-1/` by `node tools/js/audit_environment_studio_ux_polish_r2_1.mjs`. Non-blocking launch warnings may still allow Planning launch, while blocking warnings and failures prevent launch. The generated launch shell keeps full source field timing metadata but uses a browser-friendly launch window, and Mission Workspace remains responsible for glider count, routes, dive profiles, execution, replanning, and scoring. This does not add sculpting, alter mission simulation, change official scoring, add planners, expose hidden truth, or create a fifth Product Hub pillar. See [Environment Studio Product Handoff](docs/environment_studio_product_handoff.md), [Environment Studio Architecture](docs/environment_studio_architecture.md), [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md), and [Reference Bathymetry Data Pipeline](docs/reference_bathymetry_data_pipeline.md).

The reference-derived Monterey Canyon workflow and static tile-library workflow are ready for human alpha retest. Use [Reference-Derived Environment Alpha Retest Protocol](docs/alpha_reference_environment_retest_protocol.md) with `alpha/reference-environment-retest-feedback-template.json` before staging Gulf tiles, adding bathymetry variants, dataset factories, or additional environment-authoring features.
...
- Three.js mission world, waypoint drawer, timeline, and surfacing modal appear
- simulation can run
- debrief appears
...
- No React, TypeScript, Vue, Svelte, Angular, Next.js, or backend is required. Phaser 3 and Three.js are vendored/static browser libraries; Three.js is the production mission-world renderer, while Phaser is transitional shell/lab infrastructure.
...
Planner Benchmark can use the existing Simulation Lab setup, planning workspace, simulator, and Debrief to emit benchmark run-record, route-execution, and attempt-set JSON. Adaptive Benchmark has a P6 mission-manager preview and adaptive exports, but still does not execute adaptive routes, add a new planner, redesign scoring, add full autonomy, or train MARL/RL. See [Planner Benchmark Execution](docs/planner_benchmark_execution.md) and [Adaptive Benchmark Mission Manager](docs/adaptive_benchmark_mission_manager.md).
...
H3.1 connects the existing browser solver-packet workflow to the Node/OceanBox-JS headless runtime. Validate or run the checked-in example artifacts with:
...
P11 adds shared depth-layer metadata and summaries for the Node/OceanBox-JS headless runtime, solver roundtrips, Headless Bundle Viewer, and Adaptive Benchmark handoff context. Bundles can include `water_column_summary.json`, `depth_layer_priority.json`, combined-bundle `waterColumnSummary`, and `depthLayerPrioritySummary`. Observations and tracks carry `depthLayerId`, `depthMeters`, and `diveProfileId`. THREE-R1.2A visualizes that same contract in the production Three.js mission world with operational depth slabs, depth-aware current/observation/trajectory rendering, and `ANCHOR_WATER_COLUMN_RENDER_DEBUG`; THREE-R1.2A.1 makes new generated missions open with canonical synthetic multi-layer water-column config while older imported missions without `waterColumnConfig` remain explicit surface-only compatibility artifacts; THREE-R1.2A.3 adds continuous waypoint geometry, canonical educational dive execution, smoothed volumetric scalar display, and `ANCHOR_CONTINUOUS_MISSION_DEBUG`; THREE-R1.2A.3.1 stabilizes the visible Planning Console, normalized continuous UI state, free/snap placement controls, field-rendering controls, and execute-through-dive workflow without adding arbitrary XYZ route planning. This remains top-down 2.5D sampling context: it does not add full 3D route planning, a new planner, calibrated vertical ocean modeling, production data assimilation, Python simulation, or MARL/RL. See [2.5D Water-Column Sampling Model](docs/water_column_2p5d_sampling_model.md), [Three.js Volumetric Water Column](docs/threejs_volumetric_water_column.md), and [Three.js Volumetric Activation Hardening](docs/threejs_volumetric_activation_hardening.md).
...
- [Bathymetric World View](docs/bathymetric_world_view.md) documents the GFX-R2 Three.js/WebGL bathymetry renderer over the existing 2.5D water-column model, [Three.js Mission Renderer Migration](docs/threejs_mission_renderer_migration.md) tracks GFX-R3A/GFX-R3B planning parity and MIG-R1 Three.js-first mission rendering; [Three.js Planning Tools and Camera Controls](docs/threejs_planning_tools_and_camera.md) documents the visible tool/camera parity layer.
...
Normal generated Challenge missions now use the same current-presentation contract as focused current fixtures: grid render cells are mapped onto the canonical physical-meter `OceanCurrentField4D` axes before sampling, Planning and Simulation publish `ANCHOR_CURRENT_PRESENTATION_DEBUG`, and the gated `?runtimeShell=next` shell applies the same volumetric current augmentation. Optional Glider 2/3 controls may remain idle with zero waypoints; they do not block the routed Glider 1 execution path or fabricate observations. This pass restores visibility plumbing only; it does not add tracers, pathlines, new equations, WebGPU, planner changes, scoring changes, or Phaser removal.
