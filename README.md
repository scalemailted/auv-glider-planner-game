...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Reference Bathymetry Atlas**: users inspect the compact full-world overview, use deep zoom, see available patch coverage, select or type operational lon/lat windows from local patch through Gulf-scale presets, move the selected rectangle directly, load a staged Monterey patch when available, review selected patch depth/wet-land/coastline summaries, then generate a regional 3D bathymetry project from that loaded patch. The atlas operational window can be moved and resized directly. Drag inside the rectangle to move it, drag edges to resize one side, and drag corners to resize both connected sides. Large operational windows are valid selections, but live Alpha generation remains budget-gated; oversized regions export patch or multi-tile requests. The checked-in manifest is labeled `AVAILABLE` with a compact 60 arc-second global overview selection layer, a Monterey Canyon ETOPO 2022 15 arc-second `missionReadyPatch`, and a preserved 60 arc-second `lowResolutionReferencePatch` fallback. Large valid windows that exceed the live browser budget export `anchor.reference-bathymetry-multitile-patch-request` artifacts instead of generating placeholder data. The procedural synthetic world/globe workflow is demoted to an experimental sandbox / compatibility path.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. Reference bathymetry uses preprocessed public bathymetry/topography source data, not calibrated survey data, certified navigation data, HYCOM, Copernicus, or an operational forecast. The preferred fixture is true 15 arc-second ETOPO 2022 reference data for Monterey Canyon; the 60 arc-second fixture remains available only as a low-resolution fallback. See [Reference Bathymetry Data Pipeline](docs/reference_bathymetry_data_pipeline.md).
