...
- [Sample / ROI Field Demo](docs/sample_fields_demo.md)
...
`Sample / ROI Field Demo` is the companion sandbox for pure sample value `S(x,y,t)`: where, when, and how reward/objective value appears, changes, depletes, and recovers. Its controls separate Behavior Preset, Event Likelihood Field, Spatial Pattern / Geometry, Value Distribution, Temporal Pattern, Spatial Evolution, Motion Scope, State Model / Memory, Sampling Effects, and Display. Behavior presets are curated starting compositions such as Recurring Hotspots, Migrating Patch, Expanding Front, Patchy Rainfall, Drifting Storm Cells, Freshness / Revisit Value, Wandering Hotspot, Neighbor Spread, Ripple Activation, Oscillating Ecological Field, Forest Fire Front (inspired), and Life-Like Cellular Emergence (inspired); changing primitive controls marks a selected preset as modified. Event Likelihood Field is the primitive `L(x,y,t)` substrate for event origins, sparse candidate sites, jump destinations, random-walk bias, and propagation likelihood; multi-modal, patchy, and sparse-site likelihoods use separated seeded anchors for better domain coverage. The Display Layer defaults to Sample Value + Likelihood Overlay and can also show Sample Value, Event Likelihood, graph communities, node states, filtered graph messages, combined community/message diagnostics, depleted/freshness value, or raw base value so users can compare the generative substrate, graph hierarchy, and realized reward. Spatial Pattern / Geometry describes realized value geometry; Value Distribution describes assigned values, including Constant Value, Uniform Random, and Gaussian / Normal. Constant Field is only a spatial pattern with no geometry; unbiased event origins use Uniform Likelihood. Clustered value is parameterized by Cluster Count and Cluster Size rather than separate single/bimodal/multi-cluster modes. It supports seeded clusters, sparse targets, bands, sustained/periodic/bursty/intermittent/rapid-pulse/random-pulse/wavy timing, finite pulse-then-silence and long-tail modes, per-feature continuous drift, discrete jumps, bounded local random walks, neighbor propagation, knowledge-decay/revisit recovery, and raw/depleted/freshness value views. The sample-value layer is persistent by default: ordinary repeating behavior is balanced by seeded regeneration from the event likelihood substrate, and the console reports activity diagnostics such as mean value, active and high-value fractions, value range, bounding-box coverage, connected components, contrast usage, injected activity, decay loss, and depletion loss. The left console keeps compact controls and Explain buttons; clicking Explain opens right-panel Behavior Help for Behavior Preset, Event Likelihood Field, Spatial Pattern / Geometry, Value Distribution, Temporal Pattern, Spatial Evolution, State Model / Memory, Sampling Effect, or Display Layer. The right panel also serves as the Cell Inspector when a heatmap cell is selected and reports both `L(x,y,t)` and `S(x,y,t)`, plus graph community, node state, and local message diagnostics when available. Forecast/truth/uncertainty concepts are routed to the Uncertainty / Forecast Demo, and current-dependent sample behavior is reserved for the Coupled Fields Demo. It does not create missions, waypoint plans, leaderboard entries, or scoring results. See [Sample / ROI Field Demo](docs/sample_fields_demo.md).

Field concepts stay separate: `F(x,y,t)` is a physical current vector field, `C_k(t)` is cluster/community likelihood, `L_i(t)` is cell likelihood/readiness, `A_i(t)` is cell activation, `S(x,y,t)` is realized sample value, and `U(x,y,t)` is uncertainty or forecast error. `L(x,y,t)` is rendered as a cell-centered likelihood mesh: every cell has a probability-like value, while clusters/nodes/modes are sources or basins that influence that mesh. Dynamic sample fields can run on a hierarchical graph-message layer where clusters define regional tendency, cells define local likelihood and activation, neighbor edges pass influence, and node state/cooldown/freshness updates produce the next likelihood and sample-value fields. Cellular automata are one special case of this model. `V_L(x,y,t)` and `V_S(x,y,t)` describe likelihood/sample feature evolution such as drift, ripple, front motion, random walk, or local activation; they are teaching analogs, not ocean flow.
...
`Uncertainty / Forecast Demo` visualizes forecast, truth, uncertainty, information gain, forecast error, and delta-after-update layers. It includes seeded forecast/truth mismatch models, uncertainty patterns, temporal confidence behavior, click-to-observe updates, surface/global refresh concepts, and a right Cell Inspector. Truth is shown for education only; fair solver packets should treat truth as hidden unless oracle mode is explicit. See [Uncertainty / Forecast Demo](docs/uncertainty_forecast_demo.md).

All field demos include `Export Demo JSON` for notebook artifacts. Choose start time, end time, and timeframe count to export a bounded `frames[]` series using the current demo settings, while the top-level `fields` preserve the currently visible frame. These exports capture row-major field arrays, scene config, demo time, selected-cell inspector state, units, and coordinate convention. Sample / ROI exports also include graph-field metadata, community-id grids, and per-frame graph state/message layers when the graph engine is active. See [Export Formats](docs/export_formats.md).
