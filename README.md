...
`Sample / ROI Field Demo` is the companion sandbox for pure sample value `S(x,y,t)`: where, when, and how reward/objective value appears, changes, depletes, and recovers. Its controls separate Spatial Field, Value Distribution, Spatial Parameters, Temporal Pattern, Spatial Evolution, Motion Scope, State Model, Sampling Effects, and Display. Spatial Pattern describes geometry; Value Distribution describes value likelihood, including Constant Value, Uniform Random, and Gaussian / Normal. Clustered value is parameterized by Cluster Count and Cluster Size rather than separate single/bimodal/multi-cluster modes. It supports seeded clusters, sparse targets, bands, bursty/periodic/intermittent timing, stationary bursts, per-feature continuous drift, discrete jumps, bounded local random walks, neighbor propagation, knowledge-decay/revisit recovery, and raw/depleted/freshness value views. The left console keeps compact controls and Explain buttons; clicking Explain opens right-panel Behavior Help for Spatial Pattern, Value Distribution, Temporal Pattern, Spatial Evolution, State Model, Sampling Effect, or Display Layer. The right panel also serves as the Cell Inspector when a heatmap cell is selected. Forecast/truth/uncertainty concepts are routed to the Uncertainty / Forecast Demo, and current-dependent sample behavior is reserved for the Coupled Fields Demo. It does not create missions, waypoint plans, leaderboard entries, or scoring results. See [Sample / ROI Field Demo](docs/sample_fields_demo.md).
