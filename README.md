...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Synthetic World Map**: choose a world style and seed, pan/zoom through the tiled virtual map, inspect semantic layers, select or draw an operational boundary window, inspect sampled regional context, then generate a regional 3D bathymetry project from that selected window. Broad generator controls and boundary sizing are advanced disclosure controls. Feature summary, validation, dependency state, and diagnostics remain available after generation as secondary controls.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The world map is deterministic and synthetic, not real latitude/longitude, real Earth imagery, calibrated survey data, or an operational forecast. Large domains use deterministic tiles/chunks plus preview decimation for interactivity, while exported source grids remain deterministic and reproducible.

Environment Studio does not launch missions from edited projects yet. It preserves compact world/window metadata, sampled context, bathymetry/coastline digests, open boundaries, feature zones, regime hints, axes, and grid shapes. The ENV-WORLD-R1A visual acceptance package is generated under `test-results/env-world-r1a-owner-review/` by the focused browser workflow. Currents, scalars, hotspots, start/drop-zone validation, benchmark bundles, and launch-to-planning remain staged follow-ups unless a later validated workflow generates them. This does not add sculpting, import real GEBCO/ETOPO patches, alter mission simulation, change official scoring, add planners, or create a fifth Product Hub pillar. See [Environment Studio Architecture](docs/environment_studio_architecture.md) and [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md).
