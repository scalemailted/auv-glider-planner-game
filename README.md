...
Environment Studio now starts from a **Reference Bathymetry Atlas** workflow: inspect a lon/lat bathymetry map, select a bounding box, review the selected patch, and generate regional 3D bathymetry from that patch. The checked-in fallback is explicitly labeled `NO_REFERENCE_DATA_FIXTURE`; no GEBCO/ETOPO-derived fixture is bundled yet, so REAL-BATHY-R1 remains blocked for true public-reference-data claims until a preprocessed fixture is added. The procedural synthetic world/globe path remains available only as an experimental sandbox / compatibility workflow. Currents, scalars, hotspots, and launch into Planning remain staged follow-ups unless explicit adapters generate those artifacts.
...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Reference Bathymetry Atlas**: pan/zoom the bathymetry map, inspect source metadata, select a lon/lat bounding box, review selected patch depth/wet-land/coastline summaries, then generate a regional 3D bathymetry project from that selected patch. The checked-in fallback is labeled `NO_REFERENCE_DATA_FIXTURE`, so it exercises the workflow but does not claim GEBCO/ETOPO-derived data. The procedural synthetic world/globe workflow is demoted to an experimental sandbox / compatibility path.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The placeholder reference atlas is deterministic workflow data, not public GEBCO/ETOPO, calibrated survey data, certified navigation data, HYCOM, Copernicus, or an operational forecast. A real preprocessed reference bathymetry fixture is required before claiming true REAL-BATHY-R1 completion.

Environment Studio does not launch missions from edited projects yet. It preserves compact reference dataset identity, atlas digest, selected patch digest, bathymetry/coastline digests, validation status, deferred dependency states, axes, and grid shapes. The REAL-BATHY-R1 owner-review package is generated under `test-results/real-bathy-r1-owner-review/` by the focused browser workflow. Currents, scalars, hotspots, start/drop-zone validation, benchmark bundles, and launch-to-planning remain staged follow-ups unless a later validated workflow generates them. This does not add sculpting, alter mission simulation, change official scoring, add planners, or create a fifth Product Hub pillar. See [Environment Studio Architecture](docs/environment_studio_architecture.md) and [ENV-STUDIO-R1 Feedback Response](docs/feedback_r1_environment_studio.md).
