...
Environment Studio now starts from a **Reference Bathymetry Atlas** workflow backed by compact preprocessed public bathymetry/topography artifacts under `assets/reference_bathymetry/`. The browser app does not download NOAA or GEBCO data at runtime, and raw source files belong under gitignored `external_data/reference_bathymetry/`. The checked-in manifest currently reports `AVAILABLE` with a true ETOPO 2022 15 arc-second Monterey Canyon `missionReadyPatch` cropped from the NOAA surface-elevation non-ice fallback tile, while preserving the older ETOPO 2022 60 arc-second bedrock patch as a `lowResolutionReferencePatch`. The procedural synthetic world/globe path remains available only as an experimental sandbox / compatibility workflow. The implemented reference-derived path is: public reference bathymetry patch -> bathymetry artifact -> deterministic synthetic bathymetry-conditioned fields -> package-backed environment artifact -> validated Planning launch -> public benchmark bundle. Generated currents, scalars, and hotspots are deterministic synthetic benchmark fields, not operational forecast products, and the path does not change simulation or scoring.
