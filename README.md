...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Synthetic World Map**: choose a world style and seed, inspect semantic layers, select or draw an operational boundary window, inspect sampled regional context, then generate a regional 3D bathymetry project from that selected window. Feature summary, validation, dependency state, and diagnostics remain available after generation as secondary controls.

Environment Studio authors bathymetry as a 2.5D bottom surface rendered as 3D terrain. The world map is deterministic and synthetic, not real latitude/longitude, real Earth imagery, calibrated survey data, or an operational forecast. Large domains use preview decimation for interactivity, while exported source grids remain deterministic and reproducible.
