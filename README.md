...
Normal browser use does not require a build step, backend, or Playwright. Phaser is vendored locally at `vendor/phaser.min.js`; the GFX-R2/GFX-R3A Three.js renderers use the normal npm `three` dependency, so run `npm install` after a fresh checkout if `node_modules/three` is not present.
...
`Renderer Architecture Preview` is a Simulation Lab scaffold for the rendering stack. It keeps Phaser as the app/game shell, while the active `3D Bathymetric World View` uses a dedicated Three.js/WebGL renderer over public-safe view models. GFX-R3A also adds an optional `Three.js Bathymetric 3D` renderer inside the live Mission Planning workspace; it consumes the same public-safe mission world view model as the tactical map and must not mutate plans, simulation state, scoring, replay semantics, or visibility permissions. Renderer modules do not own simulation, scoring, planning, hidden truth, WebGPU fluid simulation, Python simulation, or MARL/RL. See [Renderer Architecture and WebGPU Strategy](docs/renderer_architecture_and_webgpu_strategy.md) and [Three.js Mission Renderer Migration](docs/threejs_mission_renderer_migration.md).
...
- [Bathymetric World View](docs/bathymetric_world_view.md) documents the GFX-R2 Three.js/WebGL bathymetry renderer over the existing 2.5D water-column model, and [Three.js Mission Renderer Migration](docs/threejs_mission_renderer_migration.md) tracks GFX-R3A live Mission Planning parity.
