...
- Simulation Lab field demos with Flow Fields Demo for current arrows and Spatiotemporal Sampling Process Lab for deterministic/seeded sampling-value processes
...
- [Spatiotemporal Sampling Process Lab](docs/sampling_process_lab.md)
...
`Spatiotemporal Sampling Process Lab` is the renamed companion sandbox for pure sampling value `S(x,y,t)`: where, when, and how a deterministic or seeded grid/cellular process creates value to sample. Its normal workflow starts with `Pattern Source / Mode`: Reference Signature, Custom Composer, Process Paint, or Random Rule Lab. Reference Signature mode keeps the 14 broad observable-process labels, including Propagating Fronts, Excitable Waves, Local Birth-Death Emergence, Recurrent Stationary Hotspots, Diffusive / Epidemic Spread, Directed Drift / Transport, Domain / Cluster Formation, Threshold Cascades / Avalanches, Pattern Formation / Morphogenesis, Congestion / Density Waves, and Structured Signal Propagation. Source / Initial Field replaces Event Likelihood as the primary concept: it is deterministic or seeded process support, not uncertainty or belief. Formal likelihood and information gain belong in the Uncertainty / Forecast Demo. The lab can inspect sampling value, source overlays, cell/node states, graph topology, process influence messages, transitions, communities/groups, ROI meaning, depletion/freshness value, and diagnostics. Exports prefer `anchor.demo.sampling-process-field` and preserve legacy `anchor.demo.sample-roi-field` aliases.

Process Paint and Random Rule Lab use the deterministic `sampling-process-rule-families-v1` catalog. New paint exports write canonical rule IDs such as `inert`, `propagatingFront`, `localBirthDeath`, `diffusiveSpread`, `directedTransport`, `thresholdCascade`, `interactingPopulation`, and `structuredSignal`; older IDs such as `none`, `frontPropagation`, `birthDeath`, `diffusionSpread`, `driftTransport`, `cascade`, `predatorPreyMigration`, and `signalPropagation` remain accepted aliases.
