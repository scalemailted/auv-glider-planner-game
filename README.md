...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Synthetic World Map**: choose a world style and seed, tune broad world controls, pan/zoom through the tiled virtual map, select or draw an operational boundary window, inspect sampled regional context, then generate a regional 3D bathymetry project from that selected window. Feature summary, validation, dependency state, and diagnostics remain available after generation as secondary controls.
