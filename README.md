...
Environment Studio now starts from a **Reference Bathymetry Atlas** workflow backed by compact preprocessed public bathymetry/topography artifacts under `assets/reference_bathymetry/`. Stage 1 is the Global Atlas Selector, Stage 2 is the Regional Patch Workspace, and Stage 3 is the Generated Reference Environment. The browser app does not download NOAA or GEBCO data at runtime, and raw source files belong under gitignored `external_data/reference_bathymetry/`. The checked-in manifest currently reports `AVAILABLE` with a 60 arc-second global overview, a true ETOPO 2022 15 arc-second Monterey Canyon `missionReadyPatch` cropped from the NOAA surface-elevation non-ice fallback tile, and the older ETOPO 2022 60 arc-second bedrock patch preserved as a `lowResolutionReferencePatch`. The procedural synthetic world/globe path remains available only as an experimental sandbox / compatibility workflow. The implemented reference-derived path is: public reference bathymetry patch -> bathymetry artifact -> deterministic synthetic bathymetry-conditioned fields -> package-backed environment artifact -> validated Planning launch -> public benchmark bundle. Generated currents, scalars, and hotspots are deterministic synthetic benchmark fields, not operational forecast products, and the path does not change simulation or scoring.
...
Environment Studio is exposed under `Simulation Lab -> Editor & Import Tools`. It now starts from a **Reference Bathymetry Atlas**: users inspect the global overview, see available patch coverage, select or load a staged Monterey patch, review selected patch depth/wet-land/coastline summaries, then generate a regional 3D bathymetry project from that loaded patch. The checked-in manifest is labeled `AVAILABLE` with a 60 arc-second global overview, a Monterey Canyon ETOPO 2022 15 arc-second `missionReadyPatch`, and a preserved 60 arc-second `lowResolutionReferencePatch` fallback. The procedural synthetic world/globe workflow is demoted to an experimental sandbox / compatibility path.
