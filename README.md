...
The left Mission Console uses collapsible accordion sections for menus, setup, briefing, planning, simulation, editor, import, dataset, saved-level, and debrief controls. The main menu has three primary accordions: `Challenge Mode`, `Simulation Lab`, and `Learning Labs`. Section headers stay visible when collapsed, controls remain mounted inside the section body, and session state is saved in `localStorage` under `anchorGliderCommand.ui.accordions.v1` with mode-specific defaults.
