...
All field demos include `Export Demo JSON` for notebook artifacts. Choose start time, end time, and timeframe count to export a bounded `frames[]` series using the current demo settings, while the top-level `fields` preserve the currently visible frame. These exports capture row-major field arrays, scene config, demo time, selected-cell inspector state, units, and coordinate convention. See [Export Formats](docs/export_formats.md).
