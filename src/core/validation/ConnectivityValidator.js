import { getDeploymentZones, normalizeDeploymentState } from '../deployment/DeploymentZones.js';
import { roiScalar } from '../sim/ROIValue.js';
import { normalizeEndCondition } from '../sim/MissionRules.js';

export function getNavigableMask(level) {
...
export function getDeploymentCells(level, mission = null, agentId = null) {
  if (mission) normalizeDeploymentState(level, mission);
...
export function floodFillReachable(mask, startCells = []) {
...
export function findLargestNavigableRegion(mask) {
  const height = mask.length;
  const width = mask[0]?.length ?? 0;
...
export function computeReachabilitySummary(level, mission = null) {
  if (mission) normalizeDeploymentState(level, mission);
...
export function validateGeneratedLevelConnectivity(level, mission = null, config = {}) {
...
export function stripReachableSet(summary) {
  const { reachableCells, ...rest } = summary ?? {};
  return rest;
}

export function isNavigableCell(level, x, y) {
...
export function getScoringRoiCells(level, mission = null) {
  ...
  for (let y = 0; y < roi.length; y += 1) {
    for (let x = 0; x < (roi[y]?.length ?? 0); x += 1) {
      if (roiScalar(roi[y][x], 'expectedValue') >= threshold && isNavigableCell(level, x, y)) cells.push({ x, y });
    }
  }
  cells.sort((a, b) => roiScalar(roi[b.y][b.x], 'expectedValue') - roiScalar(roi[a.y][a.x], 'expectedValue'));
  return cells.slice(0, Math.max(1, Math.min(24, cells.length)));
}

export function getRecoveryCells(level, mission = null) {
  if (!mission) return [];
  const config = normalizeEndCondition(mission);
