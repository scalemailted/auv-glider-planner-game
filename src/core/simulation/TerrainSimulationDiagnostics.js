...
import { stableDigest } from '../rendering/BathymetrySurfaceViewModel.js';

export const TERRAIN_SIMULATION_DIAGNOSTICS_VERSION = 'terrain-simulation-diagnostics-three-r1-2c-1';

export const TERRAIN_SIMULATION_EVENT_TYPES = Object.freeze({
...
export function createTerrainSimulationDiagnostics(options = {}) {
...
export function updateTerrainSimulationDiagnostics(diagnostics, canonicalSnapshot = {}, context = {}) {
...
export function recordTerrainSimulationSurfacing(diagnostics, event = {}, context = {}) {
  if (!diagnostics) return diagnostics;
  diagnostics.counters.surfacingCount += 1;
  return diagnostics;
}

export function finalizeTerrainSimulationDiagnostics(diagnostics, context = {}) {
...
export function terrainSimulationDiagnosticsSummary(diagnostics = {}) {
...
export function validateTerrainSimulationDiagnostics(diagnostics = {}) {
  const errors = [];
...
export function terrainSimulationEventsDigest(events = []) {
  return stableDigest((events ?? []).map((event) => ({
    id: event.id,
    type: event.type,
...
function terrainSourceDigest(level = {}) {
  ...
  return stableDigest({ depth, land, levelId: level?.levelId ?? null });
}
