export const MOTION_COST_GRAPH_PUBLIC_SAFETY_VERSION = 'motion-cost-graph-public-safety-sim-r1';
...
export function sanitizeMotionCostGraphForPublicExport(graph = {}) {
...
export function sanitizeMotionCostMatrixForPublicExport(matrix = {}) {
...
export function validateMotionCostGraphPublicSafety(graph = {}) {
  return validatePublicSafety(graph, 'Motion cost graph');
}

export function validateMotionCostMatrixPublicSafety(matrix = {}) {
...
function validatePublicSafety(payload, label) {
  ...
  for (const token of HIDDEN_TOKENS) {
    if (text.includes(token)) errors.push(`${label} public export must not include hidden/oracle token ${token}.`);
  }
