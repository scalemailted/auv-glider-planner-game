export const MOTION_COST_GRAPH_SCHEMA_VERSION = 'motion-cost-graph-schema-sim-r1';

export const MOTION_COST_METRIC_IDS = Object.freeze([
  'distance',
  'time',
  'energy',
  'currentAssist',
...
export const MOTION_COST_NODE_SOURCE_IDS = Object.freeze([
  'regularGrid',
...
export const MOTION_COST_EDGE_STATUS_IDS = Object.freeze([
  'feasible',
  'warning',
  'blocked',
  'outOfBounds',
...
export const MOTION_COST_TIME_MODE_IDS = Object.freeze([
  'staticSnapshot',
  'configuredDepartureTimes'
]);

export const MOTION_COST_MATRIX_FORMAT_IDS = Object.freeze([
  'sparse',
  'dense',
  'auto'
]);

export const MOTION_COST_NEIGHBOR_MODE_IDS = Object.freeze([
  'grid4',
  'grid8',
  'radius',
  'planSequence',
...
export function normalizeMotionCostMetricId(id) {
  return normalizeEnum(id, MOTION_COST_METRIC_IDS, 'energy', { cost: 'energy', duration: 'time', battery: 'energy' });
}

export function normalizeMotionCostNodeSourceId(id) {
...
export function createMotionCostGraphNode(options = {}) {
...
export function createMotionCostGraphEdge(options = {}) {
...
export function validateMotionCostGraphConfig(config = {}) {
  const errors = [];
  const warnings = [];
...
export function validateMotionCostGraphNode(node = {}) {
  const errors = [];
  ...
  if (node?.publicSafe === false) errors.push('Cost graph nodes must be public-safe for public export.');
  return { valid: errors.length === 0, status: errors.length ? 'FAIL' : 'PASS', errors, warnings: [] };
}

export function validateMotionCostGraphEdge(edge = {}) {
  const errors = [];
  const warnings = [];
...
export function motionCostGraphConfigSummary(configInput = {}) {
...
export function defaultMotionCostGraphWeights() {
