...
export const MOTION_ENVIRONMENT_SAMPLER_VERSION = 'motion-environment-sampler-motion-r1';

export function sampleMotionEnvironment({
  fieldPack,
  waterColumnConfig,
  bathymetry,
  hazardField,
  ...
  const depthLayerId = sampleState.depthLayerId
    ...
    ?? 'surface';
  const depthMeters = finiteNumber(sampleState.depthMeters, waterColumnLayerMetadata(depthLayerId).nominalDepthMeters ?? 0);
  ...
  const hazard = hazardField
    ? sampleNearest3d(wrapScalar2d(hazardField), sampleState.x, sampleState.y, zIndex)
    : sampleHazardForMotion(fieldPack, { ...sampleState, zIndex }, options);
  const constraint = constraintMask
    ? sampleNearest3d(wrapScalar2d(constraintMask), sampleState.x, sampleState.y, zIndex)
  ...
  return {
    ...
    waterColumnLayer: {
      id: depthLayerId,
      ...waterColumnLayerMetadata(depthLayerId)
    },
    warnings
  };
}

export function sampleCurrentVectorForMotion(fieldPack, state = {}, options = {}) {
  ...
  const u = sampleNearest3d(fields.F_u ?? fields.u ?? fields.flowU, x, y, z) * strength;
  const v = sampleNearest3d(fields.F_v ?? fields.v ?? fields.flowV, x, y, z) * strength;
  const w = sampleNearest3d(fields.F_w ?? fields.w ?? fields.flowW, x, y, z) * strength;
  return { u: round(u), v: round(v), w: round(w) };
}

export function sampleBathymetryForMotion(bathymetry, state = {}, _options = {}) {
...
export function sampleHazardForMotion(fieldPack, state = {}, _options = {}) {
  return sampleNearest3d(fieldPack?.fields?.hazard, state.x, state.y, state.zIndex ?? state.z ?? 0);
}

export function sampleDepthAccessibilityForMotion(fieldPack, state = {}, _options = {}) {
  const constraint = sampleNearest3d(fieldPack?.fields?.constraintMask, state.x, state.y, state.zIndex ?? state.z ?? 0);
  return {
    constraint,
...
export function motionEnvironmentSummary(samples = []) {
