import { sampleHeadlessObservation } from '../headless/runtime/HeadlessObservation.js';
import { depthIndexForDiveProfile, depthLayerForDiveProfile, normalizeDiveProfile } from '../science/DiveProfileModel.js';
...
import { computeDesiredHeadingToWaypoint, computeTrackError, stepGliderMotion } from './GliderDynamicsModel.js';
import {
  buildControlScheduleFromWaypoints,
  normalizeMotionPlanInput,
  plannedPathSummary
} from './PlanControlAdapter.js';
import { computeMotionFeasibilityDiagnostics, motionDiagnosticsSummary } from './MotionDiagnostics.js';

export const GLIDER_TRAJECTORY_SIMULATOR_VERSION = 'glider-trajectory-simulator-motion-r1';

export function simulateGliderMotionTrajectory({
  plan,
  fieldPack,
  waterColumnConfig,
  bathymetry,
  ...
  const planInput = normalizeMotionPlanInput(plan ?? {}, options);
  const config = createGliderMotionConfig({
    ...(motionConfig ?? {}),
    enabled: true,
    motionAware: true,
  ...
  const configValidation = validateGliderMotionConfig(config);
  const waterConfig = normalizeWaterColumnConfig(waterColumnConfig ?? fieldPack?.waterColumnConfig ?? fieldPack?.grid?.waterColumnConfig ?? {
    depthLayerIds: fieldPack?.grid?.depthLayers,
    diveProfileId: planInput.diveProfileId ?? glider?.diveProfileId
  });
  const profile = normalizeDiveProfile(planInput.diveProfileId ?? glider?.diveProfile ?? glider?.diveProfileId ?? waterConfig.diveProfileId, waterConfig);
  const plannedWaypoints = ensureStartWaypoint(planInput.waypoints, glider, fieldPack, profile);
  const controlSchedule = buildControlScheduleFromWaypoints({
  ...
  for (let step = 0; step < maxSteps && state.timeSeconds <= maxSimTime && state.energyRemaining > 0; step += 1) {
    ...
    const depthLayerId = target.depthLayerId ?? target.depthLayer ?? depthLayerForDiveProfile(profile, routeProgressForTarget(targetIndex, plannedWaypoints.length));
    const depthMeters = waterColumnLayerMetadata(depthLayerId).nominalDepthMeters ?? state.depthMeters;
    const command = {
    ...
    const environment = sampleMotionEnvironment({
      fieldPack,
      waterColumnConfig: waterConfig,
      bathymetry,
      state,
    ...
    const stepped = stepGliderMotion({ state, control: command, environment, config: { ...config, grid: fieldPack?.grid }, dt });
    state = stepped.state;
    const activeSegment = {
      start: plannedWaypoints[Math.max(0, targetIndex - 1)] ?? plannedWaypoints[0],
      end: target
    };
    const trackError = computeTrackError(state, activeSegment);
    trackErrors.push(trackError);
    const trackPoint = {
      ...stepped.trackPoint,
      zIndex: depthIndexForDiveProfile(profile, routeProgressForTarget(targetIndex, plannedWaypoints.length), waterConfig),
    ...
    if (state.timeSeconds - lastSampleTime >= config.sampleIntervalSeconds - 1e-6) {
      sampledObservations.push(sampleHeadlessObservation({
        fieldPack,
        x: state.x,
        y: state.y,
  ...
  const diagnostics = computeMotionFeasibilityDiagnostics(preliminaryTrace);
...
export function buildPlannedVsRealizedTrace({
  plannedWaypoints,
  realizedTrack,
  observations,
  controls,
  ...
  const planned = plannedPathSummary(plannedWaypoints);
  ...
  const motionDiagnostics = {
    ...(diagnostics ?? {}),
    plannedVsRealized,
    summary: motionDiagnosticsSummary({
      ...(diagnostics ?? {}),
...
export function trajectoryMotionSummary(trace = {}) {
...
export function validateMotionTrajectory(trace = {}) {
  const errors = [];
  const warnings = [];
...
function createInitialMotionState({ glider, plannedWaypoints, fieldPack, waterConfig, profile, config }) {
  ...
  const depthLayerId = start.depthLayerId ?? start.depthLayer ?? fieldPack?.grid?.depthLayers?.[zIndex] ?? depthLayerForDiveProfile(profile, 0);
  return createGliderMotionState({
    ...
    zIndex,
    depthLayerId,
    depthMeters: waterColumnLayerMetadata(depthLayerId).nominalDepthMeters ?? 0,
