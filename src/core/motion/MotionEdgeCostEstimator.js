...
import { createGliderMotionConfig } from './GliderMotionSchema.js';
import {
  createMotionCostGraphConfig,
  createMotionCostGraphEdge
} from './MotionCostGraphSchema.js';

export const MOTION_EDGE_COST_ESTIMATOR_VERSION = 'motion-edge-cost-estimator-sim-r1';

export function estimateMotionEdgeCost({
  fromNode,
  toNode,
  fieldPack = null,
  ...
  const motionConfig = motionConfigInput?.type === 'anchor.motion.config' ? motionConfigInput : createGliderMotionConfig(motionConfigInput);
  ...
  const currentAssistMean = mean(samples.map((sample) => currentAssist(sample.currentVector, direction)));
  const crossCurrentMean = mean(samples.map((sample) => crossCurrentMagnitude(sample.currentVector, direction)));
  ...
  const weightedCost = ['feasible', 'warning'].includes(statusInfo.status)
    ...
    : null;
  return createMotionCostGraphEdge({
...
export function edgeCostEstimatorSummary(edges = []) {
...
function sampleEdgeEnvironment({ from, to, sampleCount, fieldPack, waterColumnConfig, bathymetry, config, departureTimeSeconds }) {
  ...
  for (let index = 0; index < sampleCount; index += 1) {
    ...
    samples.push(sampleMotionEnvironment({
      fieldPack,
      waterColumnConfig,
      bathymetry,
