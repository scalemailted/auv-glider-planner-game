import { normalizeHeadlessFieldId, normalizeHeadlessVisibilityTier } from './HeadlessSchemaContract.js';

export const HEADLESS_FIELD_SCHEMA_VERSION = 'headless-field-schema-h0';

export const HEADLESS_CANONICAL_FIELDS = Object.freeze([
...
export const HEADLESS_DEPTH_LAYER_IDS = Object.freeze([
  'surface',
  'shallow',
  'thermocline',
  'midwater',
...
export const HEADLESS_FIELD_DIMENSION_IDS = Object.freeze([
  'x',
  'y',
  'z',
  'time',
...
export function headlessFieldOptions() {
  return HEADLESS_CANONICAL_FIELDS.map((entry) => ({ ...entry }));
}

export function headlessDepthLayerOptions() {
  return HEADLESS_DEPTH_LAYER_IDS.map((id) => ({ id, label: labelize(id) }));
}

export function normalizeHeadlessDepthLayerId(id) {
  if (HEADLESS_DEPTH_LAYER_IDS.includes(id)) return id;
  if (id === 'waterColumn' || id === 'integrated') return 'integratedWaterColumn';
  return 'surface';
}

export function createHeadlessFieldDescriptor(options = {}) {
  const id = normalizeHeadlessFieldId(options.id ?? options.fieldId ?? options.name);
  ...
  return compactObject({
    ...
    visibilityTier: normalizeHeadlessVisibilityTier(options.visibilityTier ?? canonical.visibilityTier),
    dimensions,
    depthLayers,
...
export function validateHeadlessFieldDescriptor(descriptor = {}) {
  const errors = [];
  const warnings = [];
...
export function headlessFieldDescriptorSummary(descriptor = {}) {
