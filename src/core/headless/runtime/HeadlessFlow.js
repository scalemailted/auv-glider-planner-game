...
export function sampleHeadlessFlow(fieldPack, x, y, zIndex = 0) {
  const fields = fieldPack?.fields ?? {};
  return {
    u: sampleNearest3d(fields.F_u, x, y, zIndex),
    v: sampleNearest3d(fields.F_v, x, y, zIndex)
  };
}
...
export function advectPoint({ x, y }, flowVector, dt, scale = 1) {
  return {
...
export function headlessFlowSummary(fieldPack) {
  const uStats = field3dStats(fieldPack?.fields?.F_u);
  const vStats = field3dStats(fieldPack?.fields?.F_v);
  return {
