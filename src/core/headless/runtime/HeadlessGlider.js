...
import { depthIndexForDiveProfile as depthIndexForWaterColumnDiveProfile, normalizeDiveProfile } from '../../science/DiveProfileModel.js';
...
export function simulateHeadlessGliderRoute({ fieldPack, glider, waypoints, missionConfig, seed = 'demo-001' } = {}) {
  ...
  const waterColumnConfig = normalizeWaterColumnConfig(missionConfig?.world?.waterColumnConfig ?? {
  ...
  const diveProfile = normalizeDiveProfile(missionGlider.diveProfile ?? missionGlider.diveProfileId ?? waterColumnConfig.diveProfileId, waterColumnConfig);
  ...
  for (let index = 0; index < route.length; index += 1) {
    ...
    const zIndex = point.hasExplicitDepth
      ...
      : depthIndexForWaterColumnDiveProfile(pointProfile, point.routeProgress ?? 0, waterColumnConfig);
