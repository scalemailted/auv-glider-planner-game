...
import { sampleBathymetryAt } from './BathymetryFieldModel.js';
import { normalizeWaterColumnConfig, waterColumnLayerMetadata } from './WaterColumnSchema.js';

export const BATHYMETRY_MESH_MODEL_VERSION = 'bathymetry-mesh-model-env-r1';

export function createBathymetryMesh(options = {}) {
  const bathymetry = options.bathymetry ?? options.field ?? null;
  const config = createBathymetryConfig(options.bathymetryConfig ?? bathymetry?.config ?? options);
...
export function createBathymetrySurfaceMesh(bathymetry, options = {}) {
  const config = createBathymetryConfig(options.bathymetryConfig ?? bathymetry?.config ?? options);
  ...
  for (let y = 0; y < config.height; y += stride) {
    for (let x = 0; x < config.width; x += stride) {
      const depthMeters = sampleBathymetryAt(bathymetry, x, y);
      points.push({
        x,
        y,
        z: -depthMeters,
...
export function createWaterSurfacePlane(options = {}) {
  const config = createBathymetryConfig(options.bathymetryConfig ?? options);
  return plane({
    id: 'waterSurface',
    label: 'Water Surface',
    z: 0,
...
export function createDepthLayerPlanes(waterColumnConfig = {}, bathymetryConfig = {}, options = {}) {
  const config = createBathymetryConfig(bathymetryConfig ?? options);
  const water = normalizeWaterColumnConfig(waterColumnConfig ?? options.waterColumnConfig ?? {});
  return water.depthLayerIds.map((layerId) => {
    const meta = waterColumnLayerMetadata(layerId);
...
export function projectBathymetryPoint(point, cameraInput = {}) {
...
export function projectBathymetryMesh(mesh, cameraInput = {}) {
  const camera = createBathymetryCamera(cameraInput);
  return {
    ...mesh,
    projected: true,
...
export function createBathymetryCamera(options = {}) {
  return {
...
export function updateBathymetryCamera(camera, patch = {}) {
  return createBathymetryCamera({ ...(camera ?? {}), ...(patch ?? {}) });
}

export function bathymetryMeshSummary(mesh = {}) {
  return {
...
export function validateBathymetryMesh(mesh = {}) {
  const errors = [];
  const warnings = [];
