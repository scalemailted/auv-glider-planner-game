import { createBathymetryConfig } from './BathymetrySchema.js';
import {
  bathymetryFeatureSummary,
  bathymetryFieldStats
} from './BathymetryFieldModel.js';
import {
  bathymetryMeshSummary,
  createBathymetryMesh,
  createDepthLayerPlanes,
  createWaterSurfacePlane
} from './BathymetryMeshModel.js';
import { normalizeWaterColumnConfig, waterColumnLayerMetadata } from './WaterColumnSchema.js';

export const OCEAN_WORLD_GEOMETRY_ADAPTER_VERSION = 'ocean-world-geometry-adapter-env-r1';

export function buildOceanWorldGeometry({
  missionConfig,
  fieldPack,
  bathymetry,
  waterColumnConfig,
  ...
  const bathymetryConfig = createBathymetryConfig(options.bathymetryConfig ?? bathymetry?.config ?? missionConfig?.world?.bathymetryConfig ?? {
  ...
  const waterConfig = normalizeWaterColumnConfig(waterColumnConfig ?? missionConfig?.world?.waterColumnConfig ?? {
    depthLayerIds: missionConfig?.world?.depthLayers ?? fieldPack?.grid?.depthLayers
  });
  const mesh = createBathymetryMesh({ bathymetry, bathymetryConfig, waterColumnConfig: waterConfig, stride: options.meshStride ?? 1 });
  ...
  const geometry = {
    ...
    bathymetrySummary: bathymetry ? bathymetryFieldStats(bathymetry) : bathymetryConfigSummaryFromConfig(bathymetryConfig),
    bathymetryFeatureSummary: bathymetry ? bathymetryFeatureSummary(bathymetry) : { featureIds: bathymetryConfig.features.map((feature) => feature.id), publicSafe: true },
    bathymetryMeshSummary: bathymetryMeshSummary(mesh),
    waterSurface: createWaterSurfacePlane({ bathymetryConfig }),
    bottomSurface: mesh.bottomSurface,
    depthLayerPlanes: createDepthLayerPlanes(waterConfig, bathymetryConfig, options),
    surfaceWaypoints,
    plannedPath,
...
export function surfaceWaypointsFromPlan(plan, options = {}) {
...
export function samplingPointsFromObservations(observations, options = {}) {
...
export function diveProfilePathFromTracks(tracks, options = {}) {
...
export function realizedTrajectoryGeometry(motionTrajectory, tracks, options = {}) {
...
export function waterColumnLayerGeometry(waterColumnConfig, bathymetryConfig, options = {}) {
  const water = normalizeWaterColumnConfig(waterColumnConfig ?? {});
  const bathy = createBathymetryConfig(bathymetryConfig ?? options);
  return water.depthLayerIds.map((id) => {
    ...
    return {
      ...
      plane: createDepthLayerPlanes({ ...water, depthLayerIds: [id] }, bathy, options)[0],
      publicSafe: true
    };
  });
}

export function oceanWorldGeometrySummary(geometry = {}) {
...
export function validateOceanWorldGeometry(geometry = {}) {
  const errors = [];
  const warnings = [];
...
function depthForLayer(id) {
  const meta = waterColumnLayerMetadata(id ?? 'surface');
