...
} from './ReferenceBathymetryAtlas.js';
import {
...
export function createEnvironmentStudioSession(options = {}) {
  ...
  const explicitWorldMode = options.studioStage === 'worldMap'
    || options.sourceMode === 'proceduralSyntheticSandbox'
  ...
  const referenceAtlas = normalizeReferenceBathymetryAtlas(options.referenceAtlas ?? options.referenceBathymetryAtlas ?? {
    ...
    overviewArtifact: options.overviewArtifact ?? options.referenceOverviewArtifact,
  ...
  const selectedReferenceFixtureId = options.selectedReferenceFixtureId
    ?? options.selectedFixtureId
    ?? options.loadedReferenceFixtureId
    ?? options.loadedFixtureId
    ?? null;
  const selectedReferenceAvailability = selectedReferenceWindow
    ? referenceFixtureAvailabilityForBounds(referenceAtlas, selectedReferenceWindow.bounds, { preferredFixtureId: selectedReferenceFixtureId })
    : null;
  const selectedReferenceBoundaryBudget = referenceBoundaryBudgetForSelection(referenceAtlas, selectedReferenceWindow, {
  ...
  const session = {
    ...
    referencePatchRequest: options.referencePatchRequest ?? null,
    worldMap,
...
export function setEnvironmentStudioAtlasPreset(sessionInput = {}, atlasPreset = 'mixedRegionalWorld', options = {}) {
  ...
  return refreshEnvironmentStudioSession({
    ...session,
    sourceMode: 'proceduralSyntheticSandbox',
...
export function setEnvironmentStudioSourceMode(sessionInput = {}, sourceMode = 'referenceBathymetryAtlas') {
  const session = normalizeSession(sessionInput);
  const mode = normalizeSourceMode(sourceMode);
  return refreshEnvironmentStudioSession({
    ...session,
    sourceMode: mode,
    studioStage: mode === 'proceduralSyntheticSandbox' ? 'worldMap' : 'globalAtlasSelector',
    selectedReferenceFixtureId: mode === 'referenceBathymetryAtlas' ? session.selectedReferenceFixtureId : null,
    ...
    selectedOperationalWindow: mode === 'proceduralSyntheticSandbox' ? session.selectedOperationalWindow : null,
    lastAction: 'source-mode-changed'
  });
}
...
export function setEnvironmentStudioReferenceBathymetryManifest(sessionInput = {}, manifestInput = null, options = {}) {
  ...
  const referenceAtlas = createReferenceBathymetryAtlas({
    manifest: referenceBathymetryManifest,
    referenceFixtures: options.referenceFixtures ?? [],
    overviewArtifact: options.overviewArtifact ?? options.referenceOverviewArtifact ?? null,
...
export function setEnvironmentStudioReferenceLayer(sessionInput = {}, layerId = 'topographyBathymetry') {
  ...
  return refreshEnvironmentStudioSession({
    ...session,
    referenceLayer: referenceBathymetryLayerById(layerId).id,
    lastAction: 'reference-layer-changed'
  });
}
...
export function selectEnvironmentStudioReferenceWindow(sessionInput = {}, bounds = {}, options = {}) {
  ...
  return refreshEnvironmentStudioSession({
    ...
    selectedOperationalWindow: null,
    fieldRegenerationResult: null,
    lastAction: 'reference-window-selected'
  });
}

export function loadEnvironmentStudioReferenceFixture(sessionInput = {}, fixtureId = null) {
  const session = normalizeSession(sessionInput);
  const fixtures = session.referenceAtlas?.referenceFixtures ?? session.referenceBathymetryManifest?.fixtures ?? [];
  const selectedAvailability = session.selectedReferenceWindow
    ? referenceFixtureAvailabilityForBounds(session.referenceAtlas, session.selectedReferenceWindow.bounds, { preferredFixtureId: session.selectedReferenceFixtureId ?? fixtureId })
    : null;
  if (!fixtureId && session.selectedReferenceWindow && selectedAvailability?.available !== true) {
    throw new Error('REFERENCE_PATCH_NOT_STAGED: selected atlas region is not staged. Export a patch request or select an available mission patch.');
  }
  const requestedFixtureId = fixtureId ?? session.selectedReferenceFixtureId ?? selectedAvailability?.matchedFixtureId;
  const fixture = fixtures.find((entry) => entry.fixtureId === requestedFixtureId)
    ?? fixtures.find((entry) => entry.role === 'missionReadyPatch')
    ?? fixtures[0]
    ?? null;
  if (!fixture) throw new Error('No staged reference bathymetry fixture is available to load.');
  const selectedReferenceWindow = createReferenceBathymetryWindow({
    ...fixture.bounds,
    selectedResolutionMeters: session.selectedReferenceWindow?.selectedResolutionMeters ?? 1500,
    previewResolutionMeters: session.selectedReferenceWindow?.previewResolutionMeters ?? 6000
  }, session.referenceAtlas);
  const selectedReferenceAvailability = referenceFixtureAvailabilityForBounds(session.referenceAtlas, selectedReferenceWindow.bounds, { preferredFixtureId: fixture.fixtureId });
  const selectedReferenceBoundaryBudget = referenceBoundaryBudgetForSelection(session.referenceAtlas, selectedReferenceWindow, {
    availability: selectedReferenceAvailability,
  ...
  return refreshEnvironmentStudioSession({
    ...
    referencePatchRequest: null,
    selectedOperationalWindow: null,
    lastAction: 'reference-fixture-loaded'
  });
}

export function patchEnvironmentStudioReferenceWindow(sessionInput = {}, patch = {}) {
  const session = normalizeSession(sessionInput);
  const current = session.selectedReferenceWindow?.bounds ?? {};
  return selectEnvironmentStudioReferenceWindow(session, {
    ...current,
    ...patch
  });
}

export function clearEnvironmentStudioReferenceWindow(sessionInput = {}) {
  const session = normalizeSession(sessionInput);
  return refreshEnvironmentStudioSession({
    ...session,
    selectedReferenceFixtureId: null,
    ...
    referencePatchRequest: null,
    fieldRegenerationResult: null,
    lastAction: 'reference-window-cleared'
  });
}
...
export function generateEnvironmentStudioRegionFromReferenceWindow(sessionInput = {}, options = {}) {
  ...
  const availability = referenceFixtureAvailabilityForBounds(session.referenceAtlas, selectedReferenceWindow.bounds, { preferredFixtureId: session.selectedReferenceFixtureId });
  const boundaryBudget = referenceBoundaryBudgetForSelection(session.referenceAtlas, selectedReferenceWindow, {
    availability,
  ...
  if (!options.allowUnloadedReferencePatch && (!loadedFixtureId || availability.matchedFixtureId !== loadedFixtureId)) {
    throw new Error('REFERENCE_PATCH_NOT_LOADED: load an available missionReadyPatch or fallback fixture before generating regional bathymetry.');
  }
  if (selectedReferenceWindow.validation?.valid === false) {
    throw new Error(selectedReferenceWindow.validation.errors?.[0] ?? 'Selected reference bathymetry patch failed validation.');
  }
  const builderResult = buildBathymetryFromReferenceWindow(session.referenceAtlas, selectedReferenceWindow, {
    seed: options.seed ?? `${session.seed}:${selectedReferenceWindow.patchDigest}`
  });
  const shape = builderResult.bathymetryField?.operationalDomain?.horizontal ?? {};
  const prepared = createEnvironmentStudioSession({
    ...session,
    projectId: session.projectId,
    sourceMode: 'referenceBathymetryAtlas',
    studioStage: 'regionalPatchWorkspace',
    referenceAtlas: session.referenceAtlas,
    selectedReferenceFixtureId: session.selectedReferenceFixtureId ?? loadedFixtureId,
    selectedReferenceWindow,
    ...
    domainSpec: {
      ...(session.domainSpec ?? {}),
      horizontal: {
        ...(session.domainSpec?.horizontal ?? {}),
        widthMeters: shape.widthMeters ?? session.domainSpec?.horizontal?.widthMeters,
        heightMeters: shape.heightMeters ?? session.domainSpec?.horizontal?.heightMeters,
        cellSizeMeters: shape.cellSizeMeters ?? session.domainSpec?.horizontal?.cellSizeMeters,
        columns: shape.columns ?? null,
...
function referenceBoundaryBudgetForSelection(referenceAtlas = {}, selectedReferenceWindow = null, context = {}) {
  if (!selectedReferenceWindow?.bounds) return null;
  const source = context.loadedFixture ?? context.availability?.matchedFixture ?? context.availability?.matchedFixture ?? referenceAtlas?.sourceDataset ?? {};
  const sourceResolutionArcSeconds = context.availability?.available === false || context.availability?.status === 'notStaged'
    ? 15
    : sourceResolutionArcSecondsFromReference(source, 15);
  return estimateReferenceAtlasBoundaryBudget(selectedReferenceWindow.bounds, {
...
function assertReferenceBoundaryBudgetAllowsGeneration(boundaryBudget = null, actionLabel = 'Reference generation') {
  if (!boundaryBudget || referenceAtlasBudgetAllowsGeneration(boundaryBudget)) return;
  const error = new Error(`${actionLabel} blocked: ${REFERENCE_ATLAS_BUDGET_BLOCKED_MESSAGE}`);
  error.code = 'REFERENCE_ATLAS_BOUNDARY_BUDGET_BLOCKED';
  error.boundaryBudget = boundaryBudget;
  throw error;
}
...
function buildReferenceEnvironmentResultForSession(session = {}, options = {}) {
  ...
  assertReferenceBoundaryBudgetAllowsGeneration(refreshed.selectedReferenceBoundaryBudget, 'Generate Currents & Science Fields');
...
export function regenerateEnvironmentStudioFields(sessionInput = {}, options = {}) {
  ...
  assertReferenceBoundaryBudgetAllowsGeneration(session.selectedReferenceBoundaryBudget, 'Generate Currents & Science Fields');
...
export function composeEnvironmentStudioReferenceEnvironment(sessionInput = {}, options = {}) {
  const session = refreshEnvironmentStudioSession(normalizeSession(sessionInput));
  assertReferenceBoundaryBudgetAllowsGeneration(session.selectedReferenceBoundaryBudget, 'Compose Environment Artifact');
...
export function createEnvironmentStudioMosaicFromBuilderResult(sessionInput = {}, builderResult = {}, options = {}) {
  ...
  const tiles = windows.map((window, index) => {
    ...
    return createTileFromBathymetry({
      ...
      featureRole: window.featureRole,
      bathymetrySourceMode: builderResult.type === 'anchor.reference-patch-bathymetry-builder-result'
        ? 'referenceBathymetryPatch'
        : 'windowConditionedAtlasGenerated',
      provenance: {
  ...
  const mosaicManifest = normalizeTileMosaicManifest({
    ...
    editProvenance: {
      ...
      deterministicSeed: seed,
      operations: [
        { id: 'build-window-conditioned-bathymetry', type: builderResult.type === 'anchor.reference-patch-bathymetry-builder-result' ? 'reference-patch-bathymetry-builder' : 'regional-atlas-window-bathymetry-builder', target: 'bottomDepthMeters' },
  ...
  return refreshEnvironmentStudioSession({
    ...
    tiles,
    mosaic,
    bathymetryBuilderResult: compactBathymetryBuilderResult(builderResult),
...
export function buildEnvironmentStudioProject(sessionInput = {}) {
  ...
  const projectBase = {
    ...
    loadedReferenceFixtureRole: session.loadedReferenceFixtureRole ?? null,
    loadedReferenceFixture: session.loadedReferenceFixture ?? null,
...
export function normalizeEnvironmentStudioProject(input = {}) {
  ...
  const session = createEnvironmentStudioSession({
    ...
    selectedReferenceAvailability: source.selectedReferenceAvailability,
    selectedReferenceBoundaryBudget: source.selectedReferenceBoundaryBudget ?? source.boundaryBudget,
    loadedReferenceFixtureId: source.loadedReferenceFixtureId ?? source.loadedFixtureId,
    loadedReferenceFixtureRole: source.loadedReferenceFixtureRole ?? source.loadedFixtureRole,
    loadedReferenceFixture: source.loadedReferenceFixture,
    referencePatchRequest: source.referencePatchRequest,
...
export function refreshEnvironmentStudioSession(sessionInput = {}) {
  ...
  const selectedReferenceAvailability = session.selectedReferenceWindow
    ? referenceFixtureAvailabilityForBounds(session.referenceAtlas, session.selectedReferenceWindow.bounds, { preferredFixtureId: session.selectedReferenceFixtureId })
    : null;
  const selectedReferenceBoundaryBudget = referenceBoundaryBudgetForSelection(session.referenceAtlas, session.selectedReferenceWindow, {
    availability: selectedReferenceAvailability,
    loadedFixture: session.loadedReferenceFixture
  });
  const selectedReferenceOperationalWindow = selectedReferenceBoundaryBudget?.operationalWindow ?? selectedReferenceAvailability?.operationalWindow ?? null;
...
export function environmentStudioDebugPayload(sessionInput = {}) {
  ...
  return {
    ...
    loadedFixtureRole: session.loadedReferenceFixtureRole ?? null,
    ...
    selectedPatchStats: session.selectedReferenceWindow?.sampledStats ?? null,
    selectedPatchValidation: session.selectedReferenceWindow?.validation ?? null,
    loadedReferenceFixture: session.loadedReferenceFixture ?? null,
...
function referenceFixtureIdForSession(session = {}) {
  if (session.loadedReferenceFixtureId) return String(session.loadedReferenceFixtureId);
  ...
  const base = {
    ...
    claimBoundary: {
      ...
      note: isReferenceBathymetryProject
        ? 'Reference bathymetry + deterministic synthetic bathymetry-conditioned fields. Currents/scalars/hotspots are deterministic synthetic benchmark artifacts, not HYCOM, Marine Copernicus, an operational forecast, or certified navigation data.'
...
function projectStateFromProject(project = {}) {
  ...
  return refreshEnvironmentStudioSession({
    ...
    referencePatchRequest: project.referencePatchRequest,
...
function normalizeSession(input = {}) {
  ...
  const explicitWorldMode = input.studioStage === 'worldMap'
    || input.sourceMode === 'proceduralSyntheticSandbox'
  ...
  const referenceAtlas = normalizeReferenceBathymetryAtlas(input.referenceAtlas ?? input.referenceBathymetryAtlas ?? {
    ...
    overviewArtifact: input.overviewArtifact ?? input.referenceOverviewArtifact,
  ...
  const selectedReferenceFixtureId = input.selectedReferenceFixtureId
    ?? input.selectedFixtureId
    ?? input.loadedReferenceFixtureId
    ?? input.loadedFixtureId
    ?? null;
  const selectedReferenceAvailability = selectedReferenceWindow
    ? referenceFixtureAvailabilityForBounds(referenceAtlas, selectedReferenceWindow.bounds, { preferredFixtureId: selectedReferenceFixtureId })
    : null;
  const selectedReferenceBoundaryBudget = input.selectedReferenceBoundaryBudget
    ?? input.boundaryBudget
    ?? referenceBoundaryBudgetForSelection(referenceAtlas, selectedReferenceWindow, {
      availability: selectedReferenceAvailability,
  ...
  return {
    ...
    referencePatchRequest: input.referencePatchRequest ?? null,
    worldMap,
    worldStyle: worldMap.style,
    worldSeed: worldMap.seed,
    worldGeneratorParameters: worldMap.generatorParameters,
...
function normalizeStudioStage(value = 'worldMap') {
  ...
  if (text === 'globalAtlasSelector' || text === 'referenceAtlas') return 'globalAtlasSelector';
  if (text === 'regionalPatchWorkspace' || text === 'generatedReferenceEnvironment') return text;
...
function normalizeSourceMode(value = 'referenceBathymetryAtlas') {
  const key = String(value ?? 'referenceBathymetryAtlas');
  return REFERENCE_BATHYMETRY_SOURCE_MODES.some((entry) => entry.id === key)
    ? key
    : 'referenceBathymetryAtlas';
}

function referenceBathymetryLayerById(id = 'topographyBathymetry') {
  const key = String(id ?? 'topographyBathymetry');
  return REFERENCE_BATHYMETRY_LAYER_OPTIONS.find((entry) => entry.id === key)
    ?? REFERENCE_BATHYMETRY_LAYER_OPTIONS[0];
}

function compactBathymetryBuilderResult(result = {}) {
  if (result?.type === 'anchor.reference-patch-bathymetry-builder-result') {
    return compactReferencePatchBathymetryResult(result);
  }
  return compactWindowConditionedBathymetryResult(result);
}
