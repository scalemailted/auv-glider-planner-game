import { generateCurrentFrames } from '../generation/CurrentFieldGenerator.js';
import { buildAgentSpecs } from '../agents/AgentSpecs.js';
import { makeForecastFromTruth } from '../generation/ForecastGenerator.js';
import { createSeededRandom } from '../generation/Random.js';
import { ensureLevelIdentity } from '../identity/GameInstanceId.js';
import { buildTemporalFrameTimes } from '../time/TemporalFrameTimes.js';

export function normalizeLevelForEditor(rawLevel) {
...
export function applyEditorCellBrush(level, mission, x, y, brush, config = {}) {
...
export function editCurrentVector(level, startCell, endCell, config = {}) {
  normalizeLevelForEditor(level);
...
export function copyCurrentFrameToAll(level, frameIndex = 0) {
  normalizeLevelForEditor(level);
...
export function updateLevelTime(level, config = {}) {
  normalizeLevelForEditor(level);
...
export function buildDefaultMissionForLevel(level, config = {}) {
  normalizeLevelForEditor(level);
  ...
  const agents = Array.from({ length: count }, (_, index) => {
    const specs = buildAgentSpecs(index, {
      ...config,
      battery,
      maxSpeed,
      fuel: battery,
...
export function updateMissionAgents(mission, level, config = {}) {
...
function normalizeFrames(frames, level) {
  ...
  const times = buildTemporalFrameTimes({ duration, dt: Math.max(0.1, dt) });
...
function maybeRefreshForecast(level, config) {
  ...
  const random = createSeededRandom(config.seed ?? level.meta?.seed ?? level.instanceId ?? 'editor');
  level.layers.forecast = {
    frames: makeForecastFromTruth(level.layers.truth.frames, { forecastMode: 'noisy', forecastNoise: Number(config.forecastNoise ?? 0.12) }, random)
  };
}
