import {
  MISSION_SCORING_SCHEMA_VERSION,
  createMissionScoreComponentDefinition,
  validateMissionScoreComponentDefinition
} from './MissionScoringSchema.js';

export const MISSION_SCORE_COMPONENTS_VERSION = 'mission-score-components-score-r1';
...
export const MISSION_SCORE_COMPONENTS = Object.freeze(RAW_COMPONENTS.map((component) => Object.freeze(component)));

export function missionScoreComponentById(id) {
  return MISSION_SCORE_COMPONENTS.find((component) => component.id === id) ?? null;
}

export function missionScoreComponentOptions() {
  return MISSION_SCORE_COMPONENTS.map(({ id, label, groupId, refereeOnly }) => ({ id, label, groupId, refereeOnly }));
}

export function validateMissionScoreComponentCatalog() {
  const errors = [];
  const warnings = [];
  const ids = new Set();
  for (const component of MISSION_SCORE_COMPONENTS) {
    ...
    const validation = validateMissionScoreComponentDefinition(component);
...
export function missionScoreComponentCatalogSummary() {
...
function c(id, groupId, label, description, direction, unit, defaultBounds, defaultTarget, dataSources, options = {}) {
  return createMissionScoreComponentDefinition({
    id,
    groupId,
    label,
    description,
