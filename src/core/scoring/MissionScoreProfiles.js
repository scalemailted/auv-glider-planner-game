...
import { MISSION_SCORE_BOUNDARY_NOT_A, MISSION_SCORE_PROFILE_IDS, normalizeMissionScoreProfileId } from './MissionScoringSchema.js';

export const MISSION_SCORE_PROFILES_VERSION = 'mission-score-profiles-score-r1';
...
export const MISSION_SCORE_PROFILES = Object.freeze([
...
export function missionScoreProfileById(id) {
  const normalized = normalizeMissionScoreProfileId(id);
  return MISSION_SCORE_PROFILES.find((profileEntry) => profileEntry.id === normalized) ?? MISSION_SCORE_PROFILES[0];
}

export function missionScoreProfileOptions() {
...
export function missionScoreProfileForObjective(objectiveId) {
...
export function validateMissionScoreProfile(profileEntry = {}) {
  const errors = [];
  const warnings = [];
...
export function missionScoreProfileSummary(profileEntry = {}) {
...
function profile(id, label, description, objectiveIds, componentWeights, requiredComponents, optionalComponents, minimumCoverageFraction, refereeOnlyComponents = []) {
  return Object.freeze({
    ...
    changesOfficialBrowserScoring: false,
    notA: [...MISSION_SCORE_BOUNDARY_NOT_A, 'not one opaque universal profile']
  });
}
...
function freezeWeights(value) {
  ...
  for (const [key, weight] of Object.entries(value ?? {})) {
    if (missionScoreComponentById(key)) result[key] = Number(weight);
  }
