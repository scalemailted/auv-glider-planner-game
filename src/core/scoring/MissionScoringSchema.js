export const MISSION_SCORING_SCHEMA_VERSION = 'mission-scoring-schema-score-r1';

export const MISSION_SCORE_PROFILE_IDS = Object.freeze([
  'balancedMission',
...
export const MISSION_SCORE_GROUP_IDS = Object.freeze([
  'science',
  'feasibility',
  'efficiency',
  'safety',
...
export const MISSION_SCORE_STATUS_IDS = Object.freeze([
  'complete',
  'partial',
  'insufficientData',
...
export const MISSION_REGRET_REFERENCE_IDS = Object.freeze([
  'none',
  'configuredBaseline',
...
export const MISSION_SCORE_DATA_SOURCE_IDS = Object.freeze([
  'publicObservation',
  'publicBelief',
  'publicForecast',
  'publicMissionRecord',
...
export const MISSION_SCORE_COMPONENT_IDS = Object.freeze([
...
export const MISSION_SCORE_BOUNDARY_NOT_A = Object.freeze([
...
export function normalizeMissionScoreProfileId(id) {
  return normalizeId(id, MISSION_SCORE_PROFILE_IDS, 'balancedMission');
}

export function normalizeMissionScoreComponentId(id) {
  return normalizeId(id, MISSION_SCORE_COMPONENT_IDS, null);
}

export function normalizeMissionScoreGroupId(id) {
  return normalizeId(id, MISSION_SCORE_GROUP_IDS, null);
}

export function normalizeMissionScoreStatusId(id) {
  return normalizeId(id, MISSION_SCORE_STATUS_IDS, 'insufficientData');
}

export function normalizeMissionRegretReferenceId(id) {
  return normalizeId(id, MISSION_REGRET_REFERENCE_IDS, 'none');
}

export function createMissionScoreConfig(options = {}) {
...
export function createMissionScoreComponentDefinition(options = {}) {
  return {
...
export function validateMissionScoreConfig(config = {}) {
  const errors = [];
  const warnings = [];
  ...
  if (config?.allowRefereeOnlyPostMissionMetrics === true && !/referee|oracle|debug|post/i.test(String(config.visibilityTier))) {
    warnings.push('Referee-only metrics are enabled; public exports must label derived metrics clearly.');
  }
  return { valid: errors.length === 0, status: errors.length ? 'FAIL' : warnings.length ? 'WARN' : 'PASS', errors, warnings };
}

export function validateMissionScoreComponentDefinition(component = {}) {
  const errors = [];
  const warnings = [];
...
export function missionScoreConfigSummary(config = {}) {
