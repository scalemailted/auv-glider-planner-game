...
import { FluidField2D } from './FluidField2D.js';
import { currentMagnitudeStats, fieldToCurrentMatrix } from './FluidFieldExporter.js';
...
export function isFluidCurrentPattern(pattern) {
  return pattern === 'fluid' || FLUID_PRESETS.includes(pattern);
}

export function normalizeFluidPreset(config = {}) {
...
export function buildFluidGeneratorMetadata(config = {}) {
  return {
    type: 'fluid',
...
export function generateFluidCurrentFrames(config = {}) {
  ...
  const random = createSeededRandom(`${config.seed ?? 1}:${preset}`);
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    ...
    const current = fieldToCurrentMatrix(field, { scale: Number(config.outputScale ?? 1), terrain: config.terrain });
    frames.push({
      t: frameTimes[frameIndex] ?? frameIndex * dt,
      current,
      currentStats: currentMagnitudeStats(current)
    });
  }
  return frames;
}
