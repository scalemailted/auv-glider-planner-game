...
export const NAVIGATION_UNCERTAINTY_LEVELS = ['off', 'low', 'medium', 'high'];

export const NAVIGATION_UNCERTAINTY_PRESETS = {
  off: {
    label: 'Off',
...
export function createDefaultNavigationUncertaintyConfig(level = 'off') {
  return normalizeNavigationUncertaintyConfig({ level });
}

export function normalizeNavigationUncertaintyConfig(config = {}) {
...
export function navigationUncertaintyLabel(value) {
  const config = normalizeNavigationUncertaintyConfig(typeof value === 'object' ? value : { level: value });
  return config.label;
}

export function getNavigationUncertaintyConfig({ level = null, mission = null, generationConfig = null } = {}) {
...
export function estimateDeadReckoningCone({
  from,
  to,
  durationHours = 0,
  level = null,
...
export function sampleSeededNavigationOffset({
  seed,
  agentId = 'agent',
  segmentIndex = 0,
  timeFraction = 1,
  ...
  const rng = createSeededRng(`${seed ?? 'anchor'}:navigation:${agentId}:${segmentIndex}`);
...
export function assessDeadReckoningConeRisk({ level, segmentCells = [], cone = {}, config = null } = {}) {
