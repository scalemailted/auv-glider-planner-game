...
export const CHALLENGE_MODES = ['perfectKnowledge', 'forecast'];

export function normalizeChallengeMode(mode) {
  return CHALLENGE_MODES.includes(mode) ? mode : 'perfectKnowledge';
}

export function ensureForecastFields(level, config = {}) {
  if (!level) return level;
  const frames = level.layers?.forecast?.frames ?? [];
  if (frames.length > 0 || (level.layers?.forecasts ?? []).length > 0) return level;

  const random = createSeededRandom(config.seed ?? level.meta?.seed ?? level.levelId ?? 'forecast');
  level.layers.forecast = {
    frames: makeForecastFromTruth(level.layers.truth?.frames ?? [], {
      forecastMode: 'noisy',
...
export function getPlanningFrame(level, time = 0, { challengeMode = 'perfectKnowledge', revealTruth = false, forecastMemberId = null } = {}) {
  ...
  if (ensemble.length > 0) {
    ...
    if (member) {
      const frame = getFrameAtTime(member.frames ?? [], time, level.world.time.dt || 1);
  ...
  const frame = getFrameAtTime(selectedFrames, time, dt);
  return frame ? { ...frame, source: frames.length > 0 ? source : 'truth' } : null;
}

export function getForecastMembers(level) {
...
function getEnsembleMeanFrame(ensemble, time, dt) {
  const frames = ensemble.map((member) => getFrameAtTime(member.frames ?? [], time, dt)).filter(Boolean);
...
export function getVisiblePlanningSource({ challengeMode = 'perfectKnowledge', revealTruth = false } = {}) {
