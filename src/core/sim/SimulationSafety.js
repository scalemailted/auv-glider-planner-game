export const DEBUG_SIM_STABILITY = false;
...
export function validateSimulationConfig(level, mission, plan) {
  const errors = [];
...
export function validateAgentState(agent) {
...
export function trimArrayToLimit(array, limit) {
  if (!Array.isArray(array) || array.length <= limit) return array;
  array.splice(0, array.length - limit);
  return array;
}

export function debugSimulation(message, payload = {}) {
