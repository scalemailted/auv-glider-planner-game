...
export function replayReviewSessionSummary(session = {}) {
  ...
  return {
    ...
    publicStateDigest: playbackSummary.publicStateDigest ?? null,
    activeTimeSeconds: Number((session.playbackState?.currentTick ?? 0) * (session.replayArtifacts?.manifest?.timingModel?.dtSeconds ?? session.replayArtifacts?.manifest?.timestepSeconds ?? 1)),
    ...
    replayReducerRunCount: playbackSummary.replayReducerRunCount ?? 0,
    checkpointRestoreCount: playbackSummary.checkpointRestoreCount ?? 0,
    forwardReplayEventCount: playbackSummary.forwardReplayEventCount ?? 0,
    reverseNavigationCount: playbackSummary.reverseNavigationCount ?? 0,
    reducedStateCacheHitCount: playbackSummary.reducedStateCacheHitCount ?? 0,
    reducedStateCacheMissCount: playbackSummary.reducedStateCacheMissCount ?? 0,
