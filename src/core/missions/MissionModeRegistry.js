import { createDefaultCurrentFieldConfig, normalizeCurrentFieldConfig } from '../generation/FlowFieldConfig.js';
import { createDefaultSampleFieldConfig, normalizeSampleFieldConfig } from '../generation/SampleFieldConfig.js';
import { normalizeNavigationUncertaintyConfig } from '../navigation/NavigationUncertainty.js';

export const MISSION_MODE_IDS = [
  'surveySweep',
  'signalHunt',
  'surfaceAdapt',
  'fleetSplit',
...
export const DEFAULT_MISSION_MODE_ID = 'surveySweep';

export const MISSION_MODE_PRESETS = [
  {
    id: 'surveySweep',
    label: 'Survey Sweep',
...
export function normalizeMissionModeId(value = DEFAULT_MISSION_MODE_ID) {
  return MISSION_MODE_IDS.includes(value) ? value : DEFAULT_MISSION_MODE_ID;
}

export function getMissionModePreset(value = DEFAULT_MISSION_MODE_ID) {
  const id = normalizeMissionModeId(value);
  return MISSION_MODE_PRESETS.find((preset) => preset.id === id) ?? MISSION_MODE_PRESETS[0];
}

export function applyMissionModeDefaults(config = {}) {
  const preset = getMissionModePreset(config.missionMode);
  const mode = config.mode ?? preset.defaults.mode ?? 'perfectKnowledge';
  const currentFieldConfig = normalizeCurrentFieldConfig(config.currentFieldConfig ?? createDefaultCurrentFieldConfig(mode), {
    mode,
  ...
  return {
    ...
    sampleFieldConfig: normalizeSampleFieldConfig({
      ...createDefaultSampleFieldConfig(mode),
    ...
    navigationUncertainty: normalizeNavigationUncertaintyConfig({
...
export function missionModeLabel(value) {
