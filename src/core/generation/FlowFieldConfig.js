import { CURRENT_PRESET_CHOICES, VECTOR_FIELD_PRESETS, normalizeVectorPreset } from './VectorFieldPresets.js';

export const FLOW_FIELD_CONFIG_SCHEMA_VERSION = '1.0';
...
export const FLOW_FIELD_PRESET_CHOICES = CURRENT_PRESET_CHOICES.filter((preset) => [
  'calm',
  'uniformDrift',
  'shearFlow',
  'eddyField',
...
export const FLOW_FIELD_UNCERTAINTY_GROWTH_LEVELS = ['none', 'slow', 'moderate', 'fast'];
export const FLOW_FIELD_HIDDEN_TRUTH_VARIATION_LEVELS = ['low', 'medium', 'high'];

export function createDefaultCurrentFieldConfig(mode = 'perfectKnowledge') {
...
export function normalizeCurrentFieldLayers(layers = [], { basePreset = 'uniformDrift' } = {}) {
...
export function createDefaultCurrentFieldLayer(existingLayers = [], basePreset = 'uniformDrift') {
...
export function currentFieldConfigToGeneratorConfig(currentFieldConfig = {}, context = {}) {
...
export function currentFieldVariationToNumber(config = {}) {
  return Math.max(
...
export function variationLevelToNumber(level = 'medium') {
...
export function summarizeCurrentFieldConfig(config = {}) {
