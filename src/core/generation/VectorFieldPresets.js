...
export const FLOW_FIELD_CLAIM_LEVELS = [
  'teachingVectorField',
...
export const FLOW_FIELD_PRESET_METADATA = {
  calm: flowMetadata({
    id: 'calm',
    label: 'Calm',
  ...
  stormPulse: flowMetadata({
    ...
    recommendedUse: 'Teaching transient events, timing risk, and why exporting time windows matters.',
...
export function getVectorPresetScientificMetadata(value = 'currentCorridor') {
  const preset = normalizeVectorPreset(value);
  const base = VECTOR_FIELD_PRESETS[preset];
...
export function validateVectorPresetScientificMetadata(metadata = {}) {
  const errors = [];
