...
function normalizeEditorDisplaySettings(displaySettings = {}, doc = {}) {
  return {
    ...
    waterColumn: {
      ...
      currentDisplayMode: displaySettings.waterColumn?.currentDisplayMode ?? 'activeSlice',
      currentLayerMode: displaySettings.waterColumn?.currentLayerMode ?? 'followSelectedGlider',
      currentVectorDensity: displaySettings.waterColumn?.currentVectorDensity ?? 1,
      currentMagnitudeScale: displaySettings.waterColumn?.currentMagnitudeScale ?? 1.8,
      currentColorMode: displaySettings.waterColumn?.currentColorMode ?? 'speed',
      showContextCurrents: displaySettings.waterColumn?.showContextCurrents === true,
