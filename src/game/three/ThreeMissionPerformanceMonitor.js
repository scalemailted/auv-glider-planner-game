...
export function createThreePerformanceDebugPayload(input = {}) {
  ...
  return {
    ...
    planningValidationBuildCount: Number(input.planningValidationBuildCount ?? counters.planningValidationBuildCount ?? 0),
    planningValidationCacheHitCount: Number(input.planningValidationCacheHitCount ?? counters.planningValidationCacheHitCount ?? 0),
    planningValidationCacheMissCount: Number(input.planningValidationCacheMissCount ?? counters.planningValidationCacheMissCount ?? 0),
    lastPlanningValidationInvalidationReason: input.lastPlanningValidationInvalidationReason ?? counters.lastPlanningValidationInvalidationReason ?? null,
    incrementalTerrainDiagnosticsUpdateCount: Number(input.incrementalTerrainDiagnosticsUpdateCount ?? counters.incrementalTerrainDiagnosticsUpdateCount ?? counters.runtimeTerrainDiagnosticsUpdateCount ?? 0),
    runtimeTerrainDiagnosticsUpdateCount: Number(input.runtimeTerrainDiagnosticsUpdateCount ?? counters.runtimeTerrainDiagnosticsUpdateCount ?? counters.incrementalTerrainDiagnosticsUpdateCount ?? 0),
    fullTerrainDiagnosticsRebuildCount: Number(input.fullTerrainDiagnosticsRebuildCount ?? counters.fullTerrainDiagnosticsRebuildCount ?? 0),
    trajectoryPointsScannedDuringLastDiagnosticsUpdate: Number(input.trajectoryPointsScannedDuringLastDiagnosticsUpdate ?? counters.trajectoryPointsScannedDuringLastDiagnosticsUpdate ?? counters.trajectoryPointsScannedDuringLastUpdate ?? 0),
    eventsScannedDuringLastDiagnosticsUpdate: Number(input.eventsScannedDuringLastDiagnosticsUpdate ?? counters.eventsScannedDuringLastDiagnosticsUpdate ?? counters.eventsScannedDuringLastUpdate ?? 0),
    terrainEventSummaryIncrementCount: Number(input.terrainEventSummaryIncrementCount ?? counters.terrainEventSummaryIncrementCount ?? 0),
    terrainEventSummaryFullRebuildCount: Number(input.terrainEventSummaryFullRebuildCount ?? counters.terrainEventSummaryFullRebuildCount ?? 0),
    validationLayerFullRebuildCount: Number(input.validationLayerFullRebuildCount ?? rendererSummary?.terrainValidationSummary?.validationLayerFullRebuildCount ?? 0),
    validationLayerIncrementalUpdateCount: Number(input.validationLayerIncrementalUpdateCount ?? rendererSummary?.terrainValidationSummary?.validationLayerIncrementalUpdateCount ?? 0),
    validationLayerObjectReuseCount: Number(input.validationLayerObjectReuseCount ?? rendererSummary?.terrainValidationSummary?.validationLayerObjectReuseCount ?? 0),
    validationLayerObjectCreateCount: Number(input.validationLayerObjectCreateCount ?? rendererSummary?.terrainValidationSummary?.validationLayerObjectCreateCount ?? 0),
    validationLayerObjectDisposeCount: Number(input.validationLayerObjectDisposeCount ?? rendererSummary?.terrainValidationSummary?.validationLayerObjectDisposeCount ?? 0),
    validationLayerDigest: input.validationLayerDigest ?? rendererSummary?.terrainValidationSummary?.validationLayerDigest ?? null,
    missionReadinessRenderCount: Number(input.missionReadinessRenderCount ?? counters.missionReadinessRenderCount ?? 0),
    missionReadinessRenderCountDuringCameraGesture: Number(input.missionReadinessRenderCountDuringCameraGesture ?? counters.missionReadinessRenderCountDuringCameraGesture ?? 0),
    resultExportBuildCount: Number(input.resultExportBuildCount ?? counters.resultExportBuildCount ?? 0),
    replayManifestBuildCount: Number(input.replayManifestBuildCount ?? counters.replayManifestBuildCount ?? 0),
    replayCheckpointBuildCount: Number(input.replayCheckpointBuildCount ?? counters.replayCheckpointBuildCount ?? 0),
    replayDigestBuildCount: Number(input.replayDigestBuildCount ?? counters.replayDigestBuildCount ?? 0),
    headlessBundleBuildCount: Number(input.headlessBundleBuildCount ?? counters.headlessBundleBuildCount ?? 0),
    roundtripReportBuildCount: Number(input.roundtripReportBuildCount ?? counters.roundtripReportBuildCount ?? 0),
