...
import { ALPHA_POSITIONING, ALPHA_RELEASE_VERSION, ALPHA_STATUS, ALPHA_TAGLINE } from '../../../core/alpha/AlphaRelease.js';
...
function mountProductHub(context) {
  ...
  root.innerHTML = `<section id="main-menu-hub" class="main-menu-hub"><header class="main-menu-hero"><div><p class="main-menu-kicker">ANCHOR mission systems</p><h1 id="next-shell-route-heading">ANCHOR: Glider Command</h1><p class="main-menu-subtitle">Scientific AUV Glider Adaptive-Sampling Game</p><div class="alpha-badge-row" aria-label="Alpha release status"><span class="alpha-release-badge">ANCHOR Alpha</span><span class="alpha-release-version">${escapeHtml(ALPHA_RELEASE_VERSION)}</span><span class="alpha-release-tagline">${escapeHtml(ALPHA_TAGLINE)}</span></div></div><p class="main-menu-runtime-note">${escapeHtml(ALPHA_POSITIONING)} Local Python: ${escapeHtml(ALPHA_STATUS.localPythonExecution)}. Google Colab Hosting Smoke: ${escapeHtml(ALPHA_STATUS.googleColabHostingSmoke)}.</p></header><div class="main-menu-primary-grid" aria-label="Primary ANCHOR paths">${hubCard('open-mission-setup', 'Challenge Mode', 'Play missions', 'Learn objectives, chase scores, compare routes, and race the greedy baseline.')}${hubCard('open-planner-benchmark', 'Simulation Lab', 'Inspect systems', 'Open scientific sandboxes, benchmark modes, headless bundles, and solver workflows.')}${hubCard('open-legacy-lab', 'Learning Labs', 'Read + experiment', 'Use interactive articles and companion sandboxes to learn the science step by step.')}${hubCard('open-methods-validation', 'Methods & Validation', 'Inspect evidence', 'Inspect model assumptions, numerical tests, reference comparisons, provenance, and known limitations.')}</div><div class="main-menu-secondary-row" aria-label="Secondary tools"><button type="button" data-action="open-mission-setup">Start Alpha Tour</button><button type="button" data-action="open-methods-validation">View Known Limitations</button><button type="button" data-action="open-import-export">Feedback & Diagnostics</button><button type="button" data-action="open-import-export">Import JSON</button><button type="button" data-action="open-headless-viewer">Headless Bundle Viewer</button><button type="button" data-action="open-methods-validation">Methods & Validation</button><button type="button" data-action="open-import-export">External Solver Evaluation</button><button type="button" data-action="open-tutorial-browser">Tutorial Browser</button></div></section>`;
