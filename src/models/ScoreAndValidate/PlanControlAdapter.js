...
const PLAN_CONTROL_ADAPTER_VERSION = 'plan-control-adapter-motion-r1';

 function buildControlScheduleFromWaypoints({
  waypoints,
  glider,
  motionConfig,
  diveProfile,
  ...
  const config = motionConfig?.type === 'anchor.motion.config' ? motionConfig : GliderMotionSchema.createGliderMotionConfig(motionConfig);
  const profile = DiveProfileModel.normalizeDiveProfile(diveProfile ?? glider?.diveProfile ?? glider?.diveProfileId ?? sourceWaypoints[0]?.diveProfileId ?? 'sawtoothProfile', options.waterColumnConfig ?? {});
  const controls = [];
  let elapsed = 0;
  let currentState = {
  ...
  for (let index = 1; index < sourceWaypoints.length; index += 1) {
    ...
    elapsed += duration;
    currentState = { ...currentState, x: target.x, y: target.y, headingRadians: GliderDynamicsModel.computeDesiredHeadingToWaypoint(currentState, target) };
  }
 ...
 function controlCommandForWaypointSegment({
  state,
  segment,
  progress,
  motionConfig,
  ...
  const depthLayerId = target.depthLayerId
    ?? target.depthLayer
    ?? DiveProfileModel.depthLayerForDiveProfile(diveProfile ?? target.diveProfileId ?? 'sawtoothProfile', progress);
  const depthMeters = finiteNumber(target.depthMeters, WaterColumnSchema.waterColumnLayerMetadata(depthLayerId).nominalDepthMeters ?? 0);
  return GliderMotionSchema.createGliderControlCommand({
 ...
 function normalizeMotionPlanInput(plan, options = {}) {
 ...
 function motionPlanIntentSummary(plan = {}) {
 ...
 function plannedPathSummary(waypoints = []) {
...
module.exports = {buildControlScheduleFromWaypoints, controlCommandForWaypointSegment, normalizeMotionPlanInput, motionPlanIntentSummary, plannedPathSummary}