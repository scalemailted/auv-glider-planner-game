...
const MOTION_COST_GRAPH_NODES_VERSION = 'motion-cost-graph-nodes-sim-r1';

 function createMotionCostGraphNodes({
  config: configInput = {},
  fieldPack = null,
  ...
  const config = configInput?.type === 'anchor.motion.cost-graph-config'
    ? configInput
    : MotionCostGraphSchema.createMotionCostGraphConfig(configInput);
 ...
 function motionCostNodeSummary(nodes = [], configInput = {}) {
  ...
  for (let z = 0; z < depthLayers.length; z += 1) {
    for (let row = 0; row < height; row += 1) {
      for (let col = 0; col < width; col += 1) {
        const value = HeadlessGrid.sampleNearest3d(priority, col, row, z);
...
function buildNode({ col, row, z, timeSeconds = null, config, fieldPack, waterColumnConfig, bathymetry, source, nodeId = null }) {
  ...
  const env = MotionEnvironmentSampler.sampleMotionEnvironment({
    fieldPack,
    waterColumnConfig,
    bathymetry,
  ...
  const sciencePriority = HeadlessGrid.sampleNearest3d(fieldPack?.fields?.A_global, col, row, zIndex);
  const uncertainty = HeadlessGrid.sampleNearest3d(fieldPack?.fields?.U_uncertainty, col, row, zIndex);
  const node = MotionCostGraphSchema.createMotionCostGraphNode({
    nodeId,
    x: col,
    y: row,
    row,
...
module.exports = {MOTION_COST_GRAPH_NODES_VERSION, createMotionCostGraphNodes, motionCostNodeSummary}