...
function createHeadlessObservation(options = {}) {
  ...
  const depthMeters = finiteNumber(options.depthMeters ?? WaterColumnSchema.waterColumnLayerMetadata(depthLayerId).nominalDepthMeters, null);
  return {
    ...
    uncertaintyValue,
    rawObservedValue,
    observedValue: HeadlessGrid.clamp01(rawObservedValue),
    sensorNoise,
    innovation,
    surprise,
 ...
 function sampleHeadlessObservation({
  fieldPack,
  x,
  y,
  zIndex = 0,
  ...
  const truthValue = HeadlessGrid.sampleNearest3d(fields.T_hiddenTruth, x, y, zIndex);
  const forecastValue = HeadlessGrid.sampleNearest3d(fields.E_forecast, x, y, zIndex);
  const beliefValue = HeadlessGrid.sampleNearest3d(fields.mu_belief, x, y, zIndex);
  const uncertaintyValue = HeadlessGrid.sampleNearest3d(fields.U_uncertainty, x, y, zIndex);
  ...
  return createHeadlessObservation({
    ...
    depthMeters: WaterColumnSchema.waterColumnLayerMetadata(depthLayer).nominalDepthMeters,
    diveProfileId,
    truthValue,
    forecastValue,
    beliefValue,
...
function gaussianNoise(seed) {
  const u1 = Math.max(1e-9, SeededRng.seededUnit(`${seed}:u1`));
  const u2 = SeededRng.seededUnit(`${seed}:u2`);
...
module.exports = {sampleHeadlessObservation, observationSummary}