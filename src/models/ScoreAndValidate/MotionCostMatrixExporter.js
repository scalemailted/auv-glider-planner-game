...
const MOTION_COST_MATRIX_EXPORTER_VERSION = 'motion-cost-matrix-er-sim-r1';

function buildMotionCostMatrix(graph = {}, options = {}) {
  const config = graph.config?.type === 'anchor.motion.cost-graph-config'
    ? graph.config
    : MotionCostGraphSchema.createMotionCostGraphConfig({ ...(graph.config ?? {}), ...(options.config ?? {}) });
  ...
  return MotionCostGraphPublicSafety.sanitizeMotionCostMatrixForPublicExport(matrix);
}

 function motionCostMatrixSummary(matrix = {}) {
 ...
 function validateMotionCostMatrix(matrix = {}) {
  const errors = [];
  const warnings = [];
  ...
  const safety = MotionCostGraphPublicSafety.validateMotionCostMatrixPublicSafety(matrix);
 ...
 function motionCostMatrixToCsv(matrix = {}) {
...
module.exports = {buildMotionCostMatrix, motionCostMatrixSummary, validateMotionCostMatrix, motionCostMatrixToCsv}