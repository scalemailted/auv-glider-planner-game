const EPSILON = 1e-6;
 const DEBUG_TIMELINE_FRAMES = false;

 function buildMissionTimelineFrames({ durationHours = 0, surfaceIntervalHours = null, dtHours = null } = {}) {
 ...
 function getTimelineFrameForTime(frames = [], time = 0) {
  const index = getNearestFrameIndex(frames, time);
  return frames[index] ?? null;
}

 function getFrameIndexAtOrBefore(frames = [], time = 0) {
 ...
 function getNearestFrameIndex(frames = [], time = 0) {
 ...
 function getNextFrame(frames = [], time = 0) {
  if (!frames.length) return null;
  const value = finiteNumber(time, 0);
 ...
 function getPrevFrame(frames = [], time = 0) {
  if (!frames.length) return null;
  const value = finiteNumber(time, 0);
  const current = getNearestFrameIndex(frames, value);
  const exact = nearlyEqual(frames[current]?.t, value);
...
module.exports = {DEBUG_TIMELINE_FRAMES, buildMissionTimelineFrames, getTimelineFrameForTime, getFrameIndexAtOrBefore, getNearestFrameIndex, getNextFrame, getPrevFrame}