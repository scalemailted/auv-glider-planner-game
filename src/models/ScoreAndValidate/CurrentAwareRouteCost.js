...
function estimateCurrentAwareSegment({
  start,
  end,
  level,
  agent = null,
  ...
  for (let index = 0; index < stepCount; index += 1) {
    ...
    const current = CurrentFieldSampler.sampleCurrentField({ frame: sampleFrame, level, x, y, time: sampleTime });
 ...
 function currentEnergyMultiplier({
  alongTrackCurrent = 0,
  crossTrackCurrent = 0,
  currentMagnitude = 0,
  driftGain = 0.5,
 ...
 function computeHeadingCurrentComponents(currentVector = {}, direction = {}) {
 ...
 function currentAlignmentLabel(currentAssist, crossCurrent, baseSpeed = 1) {
...
function frameForTime(level, frame, time) {
  ...
  return MissionTime.getFrameAtTime(frames ?? [], time, level.world?.time?.dt ?? 1) ?? frame;
}
...
module.exports = {currentEnergyMultiplier, computeHeadingCurrentComponents, currentAlignmentLabel}