...
function getActiveWaypoint(agent, plan) {
 ...
 function advanceWaypointIfReached(agent, plan, tolerance = agent.waypointTolerance ?? 0.35) {
  const waypoint = getActiveWaypoint(agent, plan);
  if (!waypoint || RuntimeMath.distance(agent, waypoint) > tolerance) return null;
  ...
  agent.completedWaypoints.push({
    ...
    semanticEventType: WaypointSemantics.waypointKindEventType(waypoint),
 ...
 function markWaypointMissed(agent, waypoint, reason, t, details = {}) {
  ...
  const event = {
    ...
    waypointKind: WaypointSemantics.normalizeWaypointKind(waypoint),
    semanticEventType: WaypointSemantics.waypointKindEventType(waypoint),
    gpsFix: Boolean(details.gpsFix ?? waypoint.gpsFix ?? WaypointSemantics.normalizeWaypointKind(waypoint) === 'surface'),
    canReplan: Boolean(details.canReplan ?? waypoint.canReplan ?? WaypointSemantics.normalizeWaypointKind(waypoint) === 'surface'),
    reason,
 ...
 function detectMissedWaypoint(agent, waypoint, t, config = {}) {
  if (!waypoint) return null;
 ...
 function getWaypointProgress(agent, plan) {
  const agentPlan = getPlanForAgent(plan, agent.id);
...
module.exports = {advanceWaypointIfReached, markWaypointMissed, detectMissedWaypoint, getWaypointProgress}