...
const GLIDER_DIVE_STATE_MACHINE_VERSION = 'glider-dive-state-machine-dive-r1';
 const GLIDER_DIVE_KINEMATICS_MODEL = Object.freeze({
 ...
 function advanceGliderDiveStateMachine(state = {}, inputs = {}) {
  const current = ContinuousGliderState.normalizeContinuousGliderState(state);
  const dt = positive(inputs.dt ?? inputs.deltaTimeSeconds, 0.25);
  const config = WaterColumnProfileRuntime.normalizeWaterColumnConfig(inputs.waterColumnConfig ?? inputs.mission?.waterColumnConfig ?? inputs.level?.world?.waterColumnConfig ?? { depthLayerIds: ['surface'] });
  const effectiveDiveProfile = inputs.effectiveDiveProfile ?? explicitEffectiveDiveProfile(inputs, config) ?? WaterColumnProfileRuntime.resolveEffectiveDiveProfile({ ...inputs, waterColumnConfig: config });
  const profile = effectiveDiveProfile.profile ?? WaterColumnProfileRuntime.normalizeDiveProfile(effectiveDiveProfile.profileId ?? 'surfaceOnly', config);
  const effectiveInputs = { ...inputs, targetDepthLayerId: inputs.targetDepthLayerId ?? effectiveDiveProfile.targetDepthLayerId };
  const model = { ...GLIDER_DIVE_KINEMATICS_MODEL, effectiveDiveProfile: WaterColumnProfileRuntime.effectiveDiveProfileSummary(effectiveDiveProfile) };
  ...
  if (feasibleCycleCount > 0) {
    ...
    const next = ContinuousGliderState.normalizeContinuousGliderState({
      ...current,
    ...
    return {
      ...
      version: GLIDER_DIVE_STATE_MACHINE_VERSION,
      model,
      effectiveDiveProfile: WaterColumnProfileRuntime.effectiveDiveProfileSummary(effectiveDiveProfile),
      state: next,
 ...
 function gliderDiveStateMachineSummary(result = {}) {
  return {
    version: GLIDER_DIVE_STATE_MACHINE_VERSION,
...
function explicitEffectiveDiveProfile(inputs, config) {
  ...
  const profile = WaterColumnProfileRuntime.normalizeDiveProfile(explicit, config);
...
function requestedTargetDepth(inputs, profile, config) {
  ...
  return Math.max(0, finite(WaterColumnProfileRuntime.waterColumnLayerMetadata(targetLayer).nominalDepthMeters, 0));
}
...
function layerCrossings(fromDepth, toDepth, config, current, inputs) {
  ...
  return config.depthLayerIds.map((layerId) => ({ layerId, depthMeters: finite(WaterColumnProfileRuntime.waterColumnLayerMetadata(layerId).nominalDepthMeters, 0) }))
...
module.exports = {GLIDER_DIVE_KINEMATICS_MODEL, advanceGliderDiveStateMachine, gliderDiveStateMachineSummary}