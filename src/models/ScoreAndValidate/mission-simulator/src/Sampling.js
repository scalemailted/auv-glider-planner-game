...
function updateSampling(agent, world, missionState, t) {
  ...
  const rules = missionState.samplingRules ?? MissionRules.normalizeSamplingRules(missionState);
  ...
  const depthScienceValue = depthAware ? DepthAwareScienceValue.evaluateDepthAwareSampleValue({
  ...
  const scoreEvent = depthAware ? DepthAwareScienceValue.depthAwareSampleScoreEvent(depthScienceValue, {
    sampleId,
    agentId: agent.id,
...
function resolveROIOutcome(roi, x, y, missionState) {
  ...
  if (!missionState.roiOutcomes.has(key)) {
    const roll = SeededRng.seededUnit(`${missionState.rngSeed ?? 'anchor'}:${key}`);
...
function resolveSampleDepthContext(agent, missionState, sample, t) {
  const config = WaterColumnProfileRuntime.normalizeWaterColumnConfig(missionState.waterColumnConfig ?? { depthLayerIds: ['surface'], diveProfileId: 'surfaceOnly' });
  ...
  const effectiveDiveProfile = WaterColumnProfileRuntime.resolveEffectiveDiveProfile({
  ...
  const targetDepthLayerId = WaterColumnProfileRuntime.normalizeWaterColumnLayerId(
  ...
  const profile = WaterColumnProfileRuntime.normalizeDiveProfile(profileId, config);
  const progress = routeProgressForAgent(agent, agentPlan, missionState, t);
  const profileLayer = WaterColumnProfileRuntime.depthLayerForDiveProfile(profile, progress);
  ...
  const depthLayerId = WaterColumnProfileRuntime.normalizeWaterColumnLayerId(actualLayer ?? explicitLayer ?? (profile.id === 'surfaceOnly' ? 'surface' : profileLayer), targetDepthLayerId);
  const safeLayerId = config.depthLayerIds.includes(depthLayerId) ? depthLayerId : config.depthLayerIds[0] ?? 'surface';
  const metadata = WaterColumnProfileRuntime.waterColumnLayerMetadata(safeLayerId);
  return {
...
function depthLayerForMeters(depthMeters, config, fallback = 'surface') {
  ...
  for (const id of config.depthLayerIds ?? []) {
    const layerDepth = Number(WaterColumnProfileRuntime.waterColumnLayerMetadata(id).nominalDepthMeters ?? 0);
