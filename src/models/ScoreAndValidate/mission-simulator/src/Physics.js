...
function stepAgentToward(agent, target, world, dt, config = {}) {
  ...
  const [nx, ny] = RuntimeMath.normalize(dxToTarget, dyToTarget);
  ...
  const driftSample = StochasticDrift.applySeededStochasticDrift(sampledCurrent, {
  ...
  const nextX = RuntimeMath.clamp(agent.x + vx * dt, 0, world.grid.width - 1);
  const nextY = RuntimeMath.clamp(agent.y + vy * dt, 0, world.grid.height - 1);
  ...
  const beachingRisk = ShorelineRisk.estimateBeachingRiskAtCell({ level: world.level, frame, x: nextX, y: nextY });
  ...
  const segmentProgress = RuntimeMath.clamp(1 - remainingDistance / segmentInitialDistance, 0, 1);
  ...
  const currentComponents = CurrentAwareRouteCost.computeHeadingCurrentComponents({ u: currentX, v: currentY }, { x: nx, y: ny });
  const currentMultiplier = CurrentAwareRouteCost.currentEnergyMultiplier({
    ...currentComponents,
    driftGain,
...
function advanceContinuousDiveState(agent, target, world, dt, config, step) {
  const waterColumnConfig = WaterColumnProfileRuntime.normalizeWaterColumnConfig(config.waterColumnConfig
  ...
  const result = GliderDiveStateMachine.advanceGliderDiveStateMachine(state, {
    dt,
    timeSeconds: config.t ?? 0,
    waterColumnConfig,
    mission: config.mission,
  ...
  agent.effectiveDiveProfile = WaterColumnProfileRuntime.effectiveDiveProfileSummary(effectiveDiveProfile);
...
module.exports = {stepAgentToward}