...
const MISSION_SIMULATION_MANIFEST_VERSION = 'mission-simulation-manifest-sim-pkg-r1';
 ...
 function normalizeMissionSimulationState(value = {}) {
  const state = {
    ...
    stepCount: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(value.stepCount, 0))),
 ...
 function missionSimulationStateDigest(value = {}) {
  ...
  return MissionSimulationUtil.stableDigest({
    ...
    stepCount: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(state.stepCount, 0))),
 ...
 function normalizeMissionSimulationEvent(event = {}, sequence = 0) {
  const normalized = {
    ...
    sequence: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(event.sequence, sequence))),
 ...
 function normalizeMissionSimulationObservation(observation = {}, sequence = 0) {
  const normalized = {
    ...
    sequence: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(observation.sequence, sequence))),
 ...
 function createMissionSimulationRawMetrics(source = {}) {
  ...
  return {
    ...
    collisions: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(source.collisions ?? source.collisionCount, 0))),
    nearMisses: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(source.nearMisses ?? source.nearMissCount, 0))),
    hazards: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(source.hazards ?? source.hazardsHit ?? events.filter((event) => String(event.type).toLowerCase().includes('hazard')).length, 0))),
    duplicateSamples: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(source.duplicateSamples ?? source.samplingMetrics?.duplicateSamples, 0))),
    sampleCount: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(source.sampleCount ?? events.filter((event) => event.type === 'sample').length, observations.length))),
    observationCount: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(source.observationCount, observations.length))),
    eventCount: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(source.eventCount, events.length))),
    ...
    steps: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(source.steps ?? source.stepCount, 0))),
 ...
 function normalizeAgentState(agent = {}, index = 0) {
  ...
  return {
    ...
    observationCount: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(agent.observationCount, 0))),
    sampleCount: Math.max(0, Math.round(MissionSimulationUtil.finiteNumber(agent.sampleCount ?? agent.samplesCollected, 0))),
