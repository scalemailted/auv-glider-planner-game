...
const MotionCostGraphNodes = require('./MotionCostGraphNodes.js')
const MotionCostGraphNeighbors = require('./MotionCostGraphNeighbors.js')
const MotionEdgeCostEstimator = require('./MotionEdgeCostEstimator.js')
const MotionCostGraphPublicSafety = require('./MotionCostGraphPublicSafety.js')
const MOTION_COST_GRAPH_BUILDER_VERSION = 'motion-cost-graph-builder-sim-r1';

 function buildMotionCostGraph({
  config: configInput = {},
  fieldPack = null,
  ...
  const nodeSet = MotionCostGraphNodes.createMotionCostGraphNodes({ config, fieldPack, waterColumnConfig, bathymetry, plan, importedNodes });
  const neighborSet = MotionCostGraphNeighbors.createMotionCostGraphNeighborPairs(nodeSet.nodes, config);
  const nodeById = new Map(nodeSet.nodes.map((node) => [node.nodeId, node]));
  const edges = neighborSet.pairs.map((pair) => MotionEdgeCostEstimator.estimateMotionEdgeCost({
  ...
  const graph = {
    ...
    config,
    configSummary: MotionCostGraphSchema.motionCostGraphConfigSummary(config),
    nodes: nodeSet.nodes,
    edges,
    nodeSummary: nodeSet.summary,
    neighborSummary: neighborSet.summary,
    edgeSummary: MotionEdgeCostEstimator.edgeCostEstimatorSummary(edges),
    summary: null,
  ...
  return sanitize ? MotionCostGraphPublicSafety.sanitizeMotionCostGraphForPublicExport(graph) : graph;
}

 function validateMotionCostGraph(graph = {}) {
  const errors = [];
  const warnings = [];
  ...
  const configValidation = MotionCostGraphSchema.validateMotionCostGraphConfig(graph?.config ?? {});
  ...
  for (const [index, node] of (graph.nodes ?? []).entries()) {
    const validation = MotionCostGraphSchema.validateMotionCostGraphNode(node);
    errors.push(...validation.errors.map((entry) => `node ${index + 1}: ${entry}`));
  }
  for (const [index, edge] of (graph.edges ?? []).entries()) {
    const validation = MotionCostGraphSchema.validateMotionCostGraphEdge(edge);
    errors.push(...validation.errors.map((entry) => `edge ${index + 1}: ${entry}`));
    warnings.push(...validation.warnings.map((entry) => `edge ${index + 1}: ${entry}`));
  }
  const safety = MotionCostGraphPublicSafety.validateMotionCostGraphPublicSafety(graph);
 ...
 function motionCostGraphSummary(graph = {}) {
...
module.exports = {MOTION_COST_GRAPH_BUILDER_VERSION, buildMotionCostGraph, validateMotionCostGraph, motionCostGraphSummary}