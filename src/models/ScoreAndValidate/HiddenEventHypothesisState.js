...
const HIDDEN_EVENT_HYPOTHESIS_STATE_VERSION = 'hidden-event-hypothesis-state-p9';

 const HIDDEN_EVENT_HYPOTHESIS_STATUSES = Object.freeze([
  'notStarted',
  'collectingEvidence',
  'hypothesisCandidate',
  'hypothesisLikely',
 ...
 function createHiddenEventHypothesisState(options = {}) {
  const diagnosisId = ScienceDiagnosisTypes.normalizeScienceDiagnosisId(options.diagnosisId ?? options.primaryDiagnosis ?? 'insufficientEvidence');
  return {
 ...
 function updateHiddenEventHypothesisState(previous = {}, evidence = {}, options = {}) {
  const diagnosisId = ScienceDiagnosisTypes.normalizeScienceDiagnosisId(options.diagnosisId ?? evidence.primaryDiagnosis ?? evidence.diagnosisId ?? previous.diagnosisId ?? 'insufficientEvidence');
  ...
  return createHiddenEventHypothesisState({
    ...
    recommendedObjectiveId: ScienceDiagnosisTypes.recommendedObjectiveForScienceDiagnosis(diagnosisId, { ...options, eventFamily: options.eventFamily ?? evidence.eventFamily ?? previous.eventFamily }),
 ...
 function hiddenEventHypothesisSummary(state = {}) {
...
module.exports = {HIDDEN_EVENT_HYPOTHESIS_STATE_VERSION, HIDDEN_EVENT_HYPOTHESIS_STATUSES, createHiddenEventHypothesisState, updateHiddenEventHypothesisState, hiddenEventHypothesisSummary}