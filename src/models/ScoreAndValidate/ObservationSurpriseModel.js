 const OBSERVATION_SURPRISE_MODEL_VERSION = 'observation-surprise-model-p9';

 const DEFAULT_SURPRISE_THRESHOLDS = Object.freeze({
  low: 1.5,
  medium: 3,
  high: 5
});

 function computeInnovation(observation = {}, options = {}) {
 ...
 function computeObservationSurprise(observation = {}, options = {}) {
 ...
 function computeObservationSurpriseBatch(observations = [], options = {}) {
  const list = Array.isArray(observations) ? observations : [];
  return list.map((observation, index) => computeObservationSurprise({ observationId: `obs-${index + 1}`, ...observation }, options));
}

 function observationSurpriseSummary(observationsOrSurprises = [], options = {}) {
 ...
 function surpriseLevelFor(value, thresholds = DEFAULT_SURPRISE_THRESHOLDS) {
...
module.exports = {OBSERVATION_SURPRISE_MODEL_VERSION, DEFAULT_SURPRISE_THRESHOLDS, computeInnovation, computeObservationSurprise, computeObservationSurpriseBatch, observationSurpriseSummary, surpriseLevelFor}