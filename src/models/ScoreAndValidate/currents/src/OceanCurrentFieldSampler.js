...
const OCEAN_CURRENT_FIELD_SAMPLER_VERSION = 'ocean-current-field-sampler-flow-r2a-5-1';
...
const samplerRuntimeCounters = {
 ...
 function resetOceanCurrentSamplerRuntimeCounters() {
 ...
 function getOceanCurrentSamplerRuntimeCounters() {
  return { ...samplerRuntimeCounters };
}

 function getOceanCurrentSampler(field = {}, options = {}) {
  const source = field ?? {};
 ...
 function createOceanCurrentSampler(field = {}, options = {}) {
  const normalized = OceanCurrentField4D.normalizeOceanCurrentField4D(field ?? {});
  const sampler = {
 ...
 function sampleOceanCurrent(samplerOrOptions = {}, eastMeters = undefined, northMeters = undefined, depthMeters = undefined, timeSeconds = undefined, options = {}) {
 ...
 function samplePreparedOceanCurrent(sampler, options = {}) {
 ...
 function normalizeCurrentTemporalBoundary(value = {}) {
  return temporalBoundaryForField(value?.field ?? value);
}

 function resolveCurrentSamplingTime(currentFieldOrMetadata = {}, timeSeconds = 0) {
 ...
 function sampleCurrentDepthProfile(samplerOrField, eastMeters, northMeters, timeSeconds, options = {}) {
 ...
 function sampleCurrentTimeSeries(samplerOrField, eastMeters, northMeters, depthMeters, options = {}) {
  const sampler = samplerOrField?.type === 'anchor.science.ocean-current-sampler'
    ? samplerOrField
    : getOceanCurrentSampler(samplerOrField ?? {}, { interpolation: options.interpolation });
 ...
 function oceanCurrentSampleSummary(sample = {}) {
  return {
...
module.exports = {resetOceanCurrentSamplerRuntimeCounters, getOceanCurrentSamplerRuntimeCounters, getOceanCurrentSampler, createOceanCurrentSampler, sampleOceanCurrent, samplePreparedOceanCurrent, normalizeCurrentTemporalBoundary, resolveCurrentSamplingTime, sampleCurrentDepthProfile, sampleCurrentTimeSeries, sampleOceanCurrentVector, validateOceanCurrentSample, oceanCurrentSampleSummary}