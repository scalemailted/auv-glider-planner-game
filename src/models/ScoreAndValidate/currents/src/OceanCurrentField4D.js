...
const OCEAN_CURRENT_FIELD_4D_VERSION = 'ocean-current-field-4d-flow-r2a-5-1';
...
const oceanCurrentFieldRuntimeCounters = { buildCount: 0, normalizeCount: 0, normalizeHitCount: 0, digestCount: 0, summaryBuildCount: 0 };
 ...
 function resetOceanCurrentFieldRuntimeCounters() {
 ...
 function getOceanCurrentFieldRuntimeCounters() {
  return { ...oceanCurrentFieldRuntimeCounters };
}

 function createOceanCurrentField4D(options = {}) {
 ...
 function normalizeOceanCurrentField4D(field = {}) {
 ...
 function validateOceanCurrentField4D(field = {}) {
  const rawErrors = [];
  ...
  const metadataValidation = OceanCurrentSourceMetadata.validateOceanCurrentSourceMetadata(normalized.sourceMetadata ?? {});
 ...
 function oceanCurrentField4DSummary(field = {}) {
  const normalized = normalizeOceanCurrentField4D(field);
  ...
  const diagnostics = normalized.scientificDiagnostics ?? CurrentFieldScientificDiagnostics.computeCurrentFieldScientificDiagnostics(normalized);
  ...
  const summary = {
    ...
    environmentGeneratorBackendId: sourceMetadata.environmentGeneratorBackendId ?? null,
    generatorBackend: sourceMetadata.generatorBackend ?? sourceMetadata.environmentGeneratorBackendId ?? null,
    generatorVersion: sourceMetadata.generatorVersion ?? null,
    verticalStructureId: sourceMetadata.verticalStructureId ?? sourceMetadata.sourceDepthRegime ?? null,
    verticalStructureVersion: sourceMetadata.verticalStructureVersion ?? null,
    verticalProfileFamilies: Array.isArray(sourceMetadata.verticalProfileFamilies) ? [...sourceMetadata.verticalProfileFamilies] : [],
    currentVerticalProfileContractVersion: sourceMetadata.currentVerticalProfileContractVersion ?? null,
    sourceDepthRegime: sourceMetadata.sourceDepthRegime ?? null,
    mixedRegionalRegimeId: sourceMetadata.mixedRegionalRegimeId ?? null,
    barotropicControl: sourceMetadata.barotropicControl === true,
    rendererOwnsVerticalStructure: sourceMetadata.rendererOwnsVerticalStructure === true,
    displayChangesVerticalStructure: sourceMetadata.displayChangesVerticalStructure === true,
    ...
    verticalShearMaximum: diagnostics.verticalShearMaximum ?? null,
    surfaceToBottomVectorDifferenceRms: diagnostics.surfaceToBottomVectorDifferenceRms ?? null,
    surfaceToDeepVectorDifferenceRms: diagnostics.surfaceToDeepVectorDifferenceRms ?? null,
    surfaceToDeepMagnitudeRatioMean: diagnostics.surfaceToDeepMagnitudeRatioMean ?? null,
    surfaceToDeepBearingDifferenceMean: diagnostics.surfaceToDeepBearingDifferenceMean ?? null,
    verticallyUniformColumnFraction: diagnostics.verticallyUniformColumnFraction ?? null,
    materiallyDistinctColumnFraction: diagnostics.materiallyDistinctColumnFraction ?? null,
    materialMagnitudeColumnFraction: diagnostics.materialMagnitudeColumnFraction ?? null,
    materialBearingColumnFraction: diagnostics.materialBearingColumnFraction ?? null,
    multiLayerWetColumnCount: diagnostics.multiLayerWetColumnCount ?? null,
    depthLayerDigests: Array.isArray(diagnostics.depthLayerDigests) ? [...diagnostics.depthLayerDigests] : [],
    depthLayerDigestCount: diagnostics.depthLayerDigestCount ?? null,
    copiedLayerDetected: diagnostics.copiedLayerDetected === true,
    depthCorrelationMatrix: diagnostics.depthCorrelationMatrix ?? null,
    bottomBoundaryGradientCheck: diagnostics.bottomBoundaryGradientCheck ?? null,
    verticalStructureStatus: diagnostics.verticalStructureStatus ?? null,
    verticalStructureWarnings: diagnostics.verticalStructureWarnings ?? [],
    verticalStructureFailures: diagnostics.verticalStructureFailures ?? [],
    ...
    componentIds: sourceMetadata.componentIds ?? (sourceMetadata.components ?? []).map((component) => component.id).filter(Boolean),
    sourceDepthMeters: [...normalized.depthAxisMeters],
    sourceTimeSeconds: [...normalized.timeAxisSeconds],
    calmThresholdMetersPerSecond: sourceMetadata.calmThresholdMetersPerSecond ?? diagnostics.calmThresholdMetersPerSecond ?? null,
    calmVectorCount: diagnostics.calmVectorCount ?? 0,
    adjacentDirectionDifferenceMeanDegrees: diagnostics.adjacentDirectionDifferenceMeanDegrees ?? null,
    adjacentDirectionDifferenceP50Degrees: diagnostics.adjacentDirectionDifferenceP50Degrees ?? null,
    adjacentDirectionDifferenceP95Degrees: diagnostics.adjacentDirectionDifferenceP95Degrees ?? null,
    adjacentMagnitudeDifferenceMean: diagnostics.adjacentMagnitudeDifferenceMean ?? null,
    adjacentMagnitudeDifferenceP95: diagnostics.adjacentMagnitudeDifferenceP95 ?? null,
    spatialAutocorrelation: diagnostics.spatialAutocorrelation ?? null,
    estimatedCorrelationLengthMeters: diagnostics.estimatedCorrelationLengthMeters ?? null,
    coherentRegionCount: diagnostics.coherentRegionCount ?? null,
    calmRegionCount: diagnostics.calmRegionCount ?? null,
    cellwiseDirectionNoiseScore: diagnostics.cellwiseDirectionNoiseScore ?? null,
    lowFrequencyEnergyFraction: diagnostics.lowFrequencyEnergyFraction ?? null,
    highFrequencyEnergyFraction: diagnostics.highFrequencyEnergyFraction ?? null,
    alongIsobathSpeedRms: diagnostics.alongIsobathSpeedRms ?? null,
    crossIsobathSpeedRms: diagnostics.crossIsobathSpeedRms ?? null,
    canyonExchangeVectorCount: diagnostics.canyonExchangeVectorCount ?? 0,
    undeclaredCrossShelfVectorCount: diagnostics.undeclaredCrossShelfVectorCount ?? 0,
    sourceMetadata,
 ...
 function oceanCurrentField4DDigest(field = {}) {
...
function defineHidden(target, key, value) {
 ...
 function dimensionsForField(field = {}) {
...
function normalizeSourceMetadata(input = {}, seed = 17) {
  return OceanCurrentSourceMetadata.normalizeOceanCurrentSourceMetadata(input, { seed });
}
...
module.exports = {resetOceanCurrentFieldRuntimeCounters, getOceanCurrentFieldRuntimeCounters, createOceanCurrentField4D, normalizeOceanCurrentField4D, validateOceanCurrentField4D, oceanCurrentField4DSummary, oceanCurrentField4DDigest, dimensionsForField}