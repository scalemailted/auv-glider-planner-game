...
const CurrentVerticalProfileContract = require('./CurrentVerticalProfileContract.js')
const BATHYMETRY_CONDITIONED_CURRENT_BUILDER_VERSION = 'bathymetry-conditioned-current-builder-flow-pkg-r2';
 const CURRENT_GENERATION_BACKEND_V2_ID = 'cpuBathymetryConditionedSyntheticV2';
 const CURRENT_GENERATION_BACKEND_V3_ID = 'cpuBathymetryConditionedSyntheticV3';

 function createBathymetryConditionedCurrentField(options = {}) {
  ...
  const wetMask = options.wetMask ?? CurrentTerrainBoundaryCondition.createWetMaskFromBathymetry({ bottomDepthMeters, landMask, width, height });
  ...
  const verticalStructure = isV3Backend ? CurrentVerticalProfileContract.createCurrentVerticalStructureDescriptor(options.verticalStructure ?? { id: options.verticalStructureId ?? 'mixedRegionalBaroclinicV1' }) : null;
    ...
    for (let zi = 0; zi < depthAxisMeters.length; zi += 1) {
      ...
      for (let y = 0; y < height; y += 1) {
        ...
        for (let x = 0; x < width; x += 1) {
          ...
          const projected = CurrentTerrainBoundaryCondition.projectCoastlineNoNormalVelocity({ u: vector.u, v: vector.v, wetMask, x, y });
  ...
  const field = OceanCurrentField4D.createOceanCurrentField4D({
    ...
    boundaryFlags: {
      ...
      usesWebGpu: false,
      terrainBoundaryDigest: CurrentTerrainBoundaryCondition.terrainBoundaryDigest({ wetMask, bottomDepthMeters }),
      wetMaskSource: landMask ? 'bathymetryAndTerrain' : 'bottomDepthMeters'
    }
  });
  field.scientificDiagnostics = CurrentFieldScientificDiagnostics.computeCurrentFieldScientificDiagnostics(field);
  return field;
}

 function evaluateSyntheticCurrentAt(options = {}) {
  return evaluateSyntheticComponents(options);
}

 function bathymetryConditionedComponentCatalog() {
...
function evaluateSyntheticComponentsV3(context) {
  ...
  if (enabledComponents.has('barotropicTide')) {
    ...
    const tide = CurrentVerticalProfileContract.applyVerticalProfileSequence({ u: tideU, v: tideV }, {
      verticalStructure: CurrentVerticalProfileContract.createCurrentVerticalStructureDescriptor({ id: 'barotropicDepthUniform', profileFamilies: ['barotropicDepthUniform', 'bottomBoundaryDecay'] }),
      depthMeters,
  ...
  if (enabledComponents.has('alongShelfJet')) {
    ...
    const jet = CurrentVerticalProfileContract.applyVerticalProfileSequence({ u: terrain.tangentX * speed, v: terrain.tangentY * speed }, {
      verticalStructure,
      depthMeters,
      bottomDepthMeters: bottom,
      parameters: {
  ...
  if (enabledComponents.has('shelfBreakJet')) {
    ...
    const jet = CurrentVerticalProfileContract.applyVerticalProfileSequence({ u: terrain.tangentX * speed, v: terrain.tangentY * speed }, {
      verticalStructure: CurrentVerticalProfileContract.createCurrentVerticalStructureDescriptor({ id: 'thermoclineShearV1', profileFamilies: ['thermoclineJet', 'bottomBoundaryDecay'] }),
      depthMeters,
      bottomDepthMeters: bottom,
      parameters: {
  ...
  if (enabledComponents.has('optionalWindDrivenSurfaceShear')) {
    const wind = CurrentVerticalProfileContract.applyVerticalProfileSequence({ u: params.windSurfaceSpeed * (0.84 + 0.16 * Math.sin(phase + 1.2)), v: -0.25 * params.windSurfaceSpeed }, {
      verticalStructure: CurrentVerticalProfileContract.createCurrentVerticalStructureDescriptor({ id: 'surfaceIntensifiedWindV1', profileFamilies: ['surfaceIntensifiedExponential'] }),
      depthMeters,
      bottomDepthMeters: bottom,
  ...
  const bottomProfile = CurrentVerticalProfileContract.applyVerticalProfileSequence({ u, v }, {
    verticalStructure: CurrentVerticalProfileContract.createCurrentVerticalStructureDescriptor({ id: 'bottomBoundaryDecayV1', profileFamilies: ['bottomBoundaryDecay'] }),
    depthMeters,
    bottomDepthMeters: bottom,
...
module.exports = {CURRENT_GENERATION_BACKEND_V2_ID, CURRENT_GENERATION_BACKEND_V3_ID, createBathymetryConditionedCurrentField, evaluateSyntheticCurrentAt, bathymetryConditionedComponentCatalog}