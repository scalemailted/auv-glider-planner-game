 const DEFAULT_CURRENT_STATS_THRESHOLDS = {
  nearCalmThreshold: 0.05,
  strongCurrentThreshold: 0.75
};

 function computeCurrentMagnitudeStats(currentMatrix, config = {}) {
 ...
 function computeCurrentFrameSetStats(frames, config = {}) {
  const thresholds = { ...DEFAULT_CURRENT_STATS_THRESHOLDS, ...(config.thresholds ?? {}) };
  const speeds = [];
 ...
 function classifyCurrentField(stats) {
 ...
 function buildCurrentFieldWarnings(stats) {
  const warnings = [];
  const mean = Number(stats?.meanSpeed ?? 0);
 ...
 function summarizeCurrentField(stats) {
...
module.exports = {DEFAULT_CURRENT_STATS_THRESHOLDS, computeCurrentMagnitudeStats, computeCurrentFrameSetStats, classifyCurrentField, buildCurrentFieldWarnings, summarizeCurrentField}