...
function buildRouteSegmentsForAgent({ level, mission, agent, agentPlan, surfacedAgents = [], planningAnchor = null } = {}) {
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    const clipped = RoutePreview.clipLineToTerrain(from, to, level, { mission });
    segments.push({
      from,
      to,
      kind: index === 1 ? 'startToWaypoint' : 'waypointToWaypoint',
      waypointKind: WaypointSemantics.normalizeWaypointKind(to),
      gpsCorrectionAtEnd: WaypointSemantics.normalizeWaypointKind(to) === 'surface',
      canReplanAtEnd: WaypointSemantics.normalizeWaypointKind(to) === 'surface',
 ...
 function resolveRouteAnchor({ agent, agentPlan = null, surfacedAgents = [], planningAnchor = null } = {}) {
