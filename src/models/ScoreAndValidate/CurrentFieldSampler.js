...
const CURRENT_COORDINATES = {
 ...
 function sampleCurrentField({
  ...
  const boundaryConditions = FlowFieldConfig.normalizeBoundaryConditions(config.boundaryConditions
  ...
  const presetConfig = preset ? VectorFieldPresets.getVectorPresetConfig(preset, config) : {};
  ...
  sample = normalizeCurrentVector(CurrentFieldGenerator.sampleGeneratedCurrent({
    x: point.x,
    y: point.y,
    width,
    height,
 ...
 function sampleCurrentVector(options = {}) {
  const sample = sampleCurrentField(options);
  return [sample.u, sample.v];
}

 function gridToNormalized({ x = 0, y = 0, width = 1, height = 1 } = {}) {
  return {
 ...
 function normalizedToGridCell({ x = 0, y = 0, width = 1, height = 1 } = {}) {
  return {
 ...
 function normalizeCurrentVector(vector, metadata = {}) {
 ...
 function estimateTopologyAtCell({ terrain = null, x = 0, y = 0, width = 1, height = 1, radius = 3 } = {}) {
...
function applyTopologyAdjustment(sample, { terrain = null, x = 0, y = 0, width = 1, height = 1, boundaryConditions = {} } = {}) {
  const boundary = FlowFieldConfig.normalizeBoundaryConditions(boundaryConditions);
...
function enrichCurrentSampleMetadata(sample, {
  ...
  const topology = terrain
    ? TopologyAwareComposite.classifyTopologyRegionAtCell({ terrain, x, y, width, height })
    : null;
  ...
  const metadata = {
    ...
    boundaryMode: adjustment?.boundaryMode ?? FlowFieldConfig.normalizeBoundaryConditions(boundaryConditions).mode,
    hazardExposure,
    channelAxis
  };
  return {
...
module.exports = {sampleCurrentField, sampleCurrentVector, gridToNormalized, normalizedToGridCell, normalizeCurrentVector, estimateTopologyAtCell}