...
const MISSION_OUTCOME_METRIC_ADAPTER_VERSION = 'mission-outcome-metric-adapter-score-r1';

 function extractMissionOutcomeMetrics({
  result = null,
 ...
 function missionOutcomeMetricSummary(record = {}) {
 ...
 function validateMissionOutcomeMetrics(record = {}) {
  const errors = [];
  const warnings = [];
  ...
  for (const metric of record?.metrics ?? []) {
    if (!MissionScoreComponents.missionScoreComponentById(metric.componentId)) errors.push(`Unknown metric component ${metric.componentId}.`);
...
function caveatsFor(component, value) {
  ...
  return component.refereeOnly ? ['Post-mission scalar summary; raw hidden truth is not ed.'] : [];
}
...
module.exports = {extractMissionOutcomeMetrics, missionOutcomeMetricSummary, validateMissionOutcomeMetrics}