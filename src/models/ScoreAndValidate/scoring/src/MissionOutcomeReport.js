...
const MISSION_OUTCOME_REPORT_VERSION = 'mission-outcome-report-score-r1';

 function buildMissionOutcomeReport({ scoreConfig, profile, metrics, normalizedMetrics, missionScore, regretReport = null, sourceArtifacts = null, options = {} } = {}) {
  const scoreSummary = MissionScoreAggregator.missionScoreAggregationSummary(missionScore);
  ...
  return {
    ...
    regretSummary: regretReport ? MissionRegretModel.missionRegretReportSummary(regretReport) : null,
    fairnessSummary: {
      profileId: profile?.id ?? scoreConfig?.profileId ?? null,
      profileVersion: profile?.version ?? scoreConfig?.profileVersion ?? null,
 ...
 function validateMissionOutcomeReport(report = {}) {
  const errors = [];
  const warnings = [];
 ...
 function missionOutcomeReportSummary(report = {}) {
  return {
...
module.exports = {buildMissionOutcomeReport, validateMissionOutcomeReport, missionOutcomeReportSummary}