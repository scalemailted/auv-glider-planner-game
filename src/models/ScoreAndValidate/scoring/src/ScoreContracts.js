...
const SCORING_PROFILE_VERSION = 'score-profile-score-pkg-r1';
 const SCORING_INPUT_VERSION = 'score-input-score-pkg-r1';
 const SCORING_RESULT_VERSION = 'score-result-score-pkg-r1';
 const SCORING_COMPONENT_VERSION = 'score-component-score-pkg-r1';
 const SCORE_PACKAGE_VERSION = 'anchor-scoring-score-pkg-r1';

 function createScoreProfile(options = {}) {
  return normalizeScoreProfile(options);
}

 function normalizeScoreProfile(value = {}) {
  const source = MissionScoreProfiles.missionScoreProfileById(value?.id ?? value?.profileId ?? value);
 ...
 function validateScoreProfile(value = {}) {
  ...
  const legacy = MissionScoreProfiles.validateMissionScoreProfile(MissionScoreProfiles.missionScoreProfileById(profile.id));
 ...
 function scoreProfileSummary(value = {}) {
  const profile = normalizeScoreProfile(value);
  return {
    ...MissionScoreProfiles.missionScoreProfileSummary(profile.id),
 ...
 function scoreProfileDigest(value = {}) {
  return normalizeScoreProfile(value).profileDigest;
}

 function scoreComponentDefinitions(profile = {}) {
  const source = profile?.componentWeights ? profile : MissionScoreProfiles.missionScoreProfileById(profile?.id ?? profile?.profileId ?? profile);
 ...
 function scoreInputDigest(value = {}) {
  return normalizeScoreInput(value).inputDigest;
}

 function evaluateScore(scoreProfile, scoreInput, options = {}) {
  ...
  if (metricRecord && Array.isArray(metricRecord.metrics)) {
    const normalizedMetrics = MissionScoreNormalizer.normalizeMissionOutcomeMetrics(metricRecord, profile, options);
    legacyMissionScore = MissionScoreAggregator.aggregateMissionOutcomeScore({ normalizedMetrics, profile, scoreConfig: MissionScoringSchema.createMissionScoreConfig({ profileId: profile.id, profileVersion: profile.version }) });
 ...
 function validateScoreResult(value = {}) {
  ...
  const safety = MissionScorePublicSafety.auditMissionScorePublicSafety(publicScoreSummary(result));
  if (!safety.valid) errors.push(...safety.failures);
  warnings.push(...safety.warnings);
  return { valid: errors.length === 0, status: errors.length ? 'FAIL' : warnings.length ? 'WARN' : 'PASS', errors, warnings };
}

 function scoreResultSummary(value = {}) {
  const result = normalizeScoreResult(value);
  return {
 ...
 function scoreResultDigest(value = {}) {
  return normalizeScoreResult(value).resultDigest;
}

 function publicScoreSummary(result = {}) {
  const normalized = normalizeScoreResult(result);
  return MissionScorePublicSafety.sanitizeMissionOutcomeReportForPublicExport({
 ...
 function scoreMethodologySummary(profile = {}) {
 ...
 function normalizePlannerProvenance(value = {}) {
  return stableObject({
 ...
 function buildPlannerComparisonRecord({ environmentDigest, missionDigest, plannerMetadata, planDigest, simulationInputDigest, simulationResultDigest, scoreResult, metrics = {} } = {}) {
 ...
 function scoringDebugSummary({ scoreProfile, scoreInput, scoreResult, evaluationCount = 1, parity = {} } = {}) {
  const profile = normalizeScoreProfile(scoreProfile ?? {});
  ...
  return {
    ...
    publicSafetyStatus: MissionScorePublicSafety.auditMissionScorePublicSafety(publicScoreSummary(result)).status,
 ...
 function componentCatalogDigest() {
  return digestObject(MissionScoreComponents.missionScoreComponentCatalogSummary());
}

 const ALPHA_POSITIONING_STATEMENT = 'ANCHOR Alpha is a deterministic, scientifically constrained research-and-education sandbox for investigating adaptive underwater-glider mission planning. It supports reproducible comparison of human, classical, and learning-based planners. It is not an operational ocean forecast or certified vehicle-navigation system.';
 const ALPHA_TAGLINE = 'Plan. Simulate. Compare. Learn.';

function scoreResultComponentFromMissionScore(component = {}) {
  const definition = MissionScoreComponents.missionScoreComponentById(component.componentId);
...
module.exports = {SCORING_INPUT_VERSION, SCORING_RESULT_VERSION, SCORING_COMPONENT_VERSION, SCORE_PACKAGE_VERSION, createScoreProfile, normalizeScoreProfile, validateScoreProfile, scoreProfileSummary, scoreProfileDigest, scoreComponentDefinitions, scoreComponentById, createScoreInput, normalizeScoreInput, validateScoreInput, scoreInputDigest, evaluateScore, normalizeScoreResult, validateScoreResult, scoreResultSummary, scoreResultDigest, publicScoreSummary, scoreMethodologySummary, normalizePlannerProvenance, buildPlannerComparisonRecord, scoringDebugSummary, componentCatalogDigest, ALPHA_POSITIONING_STATEMENT, ALPHA_TAGLINE}