...
const MISSION_REGRET_MODEL_VERSION = 'mission-regret-model-score-r1';

 function missionScoreCompatibilityKey(attempt = {}) {
 ...
 function compatibleMissionScoreAttempts(attempts = [], target = {}, options = {}) {
 ...
 function buildMissionRegretReport({ achievedScore, compatibleAttempts = [], configuredBaseline = null, oracleAttempt = null, profile = {}, scoreConfig = {}, options = {} } = {}) {
  const referenceType = MissionScoringSchema.normalizeMissionRegretReferenceId(options.referenceType ?? scoreConfig?.regretReference ?? 'none');
 ...
 function validateMissionRegretReport(report = {}) {
  const errors = [];
  const warnings = [];
 ...
 function missionRegretReportSummary(report = {}) {
  return {
...
module.exports = {missionScoreCompatibilityKey, compatibleMissionScoreAttempts, buildMissionRegretReport, validateMissionRegretReport, missionRegretReportSummary}