...
const MISSION_SCORE_NORMALIZER_VERSION = 'mission-score-normalizer-score-r1';
 ...
 function normalizeTargetRange(value, targetRange = {}, bounds = {}) {
 ...
 function normalizeBinaryPass(value) {
 ...
 function normalizeMissionScoreMetric(metric = {}, componentDefinition = null, options = {}) {
  const definition = componentDefinition ?? MissionScoreComponents.missionScoreComponentById(metric.componentId);
  const warnings = [];
 ...
 function normalizeMissionOutcomeMetrics(metricsRecord = {}, profile = {}, options = {}) {
  const inputMetrics = Array.isArray(metricsRecord?.metrics) ? metricsRecord.metrics : Array.isArray(metricsRecord) ? metricsRecord : [];
  const normalizedMetrics = inputMetrics.map((metric) => normalizeMissionScoreMetric(metric, MissionScoreComponents.missionScoreComponentById(metric.componentId), options));
  return {
 ...
 function missionScoreNormalizationSummary(result = {}) {
...
module.exports = {normalizeHigherIsBetter, normalizeLowerIsBetter, normalizeTargetRange, normalizeBinaryPass, normalizeMissionScoreMetric, normalizeMissionOutcomeMetrics, missionScoreNormalizationSummary}