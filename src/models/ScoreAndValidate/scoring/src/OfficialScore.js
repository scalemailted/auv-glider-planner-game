...
function summarizeScore({ agents, events, t, scoring = {}, missionState = null, complete = false }) {
  ...
  const depthScienceProfile = index.depthScienceScoreProfileMetadata(missionState?.depthScienceScoreProfile ?? scoring?.depthScience ?? scoring?.scoreProfileId ?? 'legacySurfaceScienceV1', {
  ...
  const depthScience = index.summarizeDepthAwareScoreEvents(depthScoreEvents.length ? depthScoreEvents : missionState?.depthScienceEvents ?? [], {
