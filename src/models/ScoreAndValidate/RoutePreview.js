...
function clipLineToTerrain(start, end, level, { stepsPerCell = 4, mission = null } = {}) {
  void stepsPerCell;
  const blockage = Navigability.explainSegmentBlockage(start, end, { level, mission });
 ...
 function estimateRouteEnergy(start, target, level, agent, frame, {
  driftGain = 0.5,
  energyPerCell = 1,
  mission = null
} = {}) {
  ...
  const segmentCost = CurrentAwareRouteCost.estimateCurrentAwareSegment({
    start,
    end: endpoint,
    level,
    agent,
  ...
  const beachingRisk = ShorelineRisk.estimateSegmentBeachingRisk({ level, frame, start, end: endpoint });
...
module.exports = {estimateRouteEnergy}