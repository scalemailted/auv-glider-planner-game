...
const SimulationSafety = require('./SimulationSafety.js')
const TruthWorld = require('./TruthWorld.js')
const Navigability = require('./Navigability.js')
const CurrentFieldSampler = require('./CurrentFieldSampler.js')
function evaluateSegmentForExecution({
  level = null,
  mission = null,
  agent = null,
  from = null,
  ...
  const startNav = Navigability.isCellNavigable(level, mission, from.x, from.y);
  if (!startNav.ok) {
    return blockedResult(startNav.reason, 'Segment start is not navigable.', {
      blockedCell: startNav.cell,
      routeBlockDiagnostic: Navigability.buildRouteBlockDiagnostic({
        level,
        mission,
        agentId: agent.id,
        plannedFrom: from,
  ...
  const targetNav = Navigability.isCellNavigable(level, mission, to.x, to.y);
  if (!targetNav.ok) {
  ...
  const simAgent = Agent.createAgent({
    ...agent,
  ...
  for (let step = 0; step < maxSteps; step += 1) {
    ...
    lastOutcome = Physics.stepAgentToward(simAgent, target, world, dt, {
      t,
      mission,
      driftGain,
      energyPerCell
  ...
  return blockedResult(endedBlocked ? 'routeBlocked' : 'waypointTimeout', 'Simulation movement did not reach the waypoint within the segment budget.', {
    ...
    routeBlockDiagnostic: endedBlocked
      ? Navigability.buildRouteBlockDiagnostic({
        level,
        mission,
        agentId: agent.id,
        plannedFrom: from,
...
function createExecutionWorld(level, mission, frame = null) {
  const world = new TruthWorld.TruthWorld(level, mission);
  ...
  return {
    ...
    grid: world.grid,
    getFrame: () => frame,
    sampleCurrent: (x, y, t) => CurrentFieldSampler.sampleCurrentVector({ frame, level, x, y, time: t }),
