...
const RouteSegmentBuilder = require('./RouteSegmentBuilder.js')
const RoutePreview = require('./RoutePreview.js')
const ShorelineRisk = require('./ShorelineRisk.js')
const Navigability = require('./Navigability.js')
const SegmentExecutionValidator = require('./SegmentExecutionValidator.js')
const RouteDiagnostic = require('./RouteDiagnostic.js')
const WaypointSemantics = require('./WaypointSemantics.js')
function validateRoutePlanForExecution({
  level,
  mission,
  plan,
  agentId = null,
  ...
  const duration = Number(MissionTime.getTimeConfig(level).duration ?? level?.world?.time?.duration ?? Infinity);
  ...
  for (const agent of agents) {
    ...
    const route = RouteSegmentBuilder.buildRouteSegmentsForAgent({
      level,
      mission,
      agent,
      agentPlan,
    ...
    if (DeploymentZones.requiresDeploymentSelection(mission, agent.id) || route.missingAnchor || !isFinitePoint(route.anchor)) {
    ...
    for (const [index, waypoint] of (agentPlan.waypoints ?? []).entries()) {
      ...
      const navigability = Navigability.isCellNavigable(level, mission, waypoint.x, waypoint.y);
      if (!navigability.ok) {
        const issue = buildIssue({
          type: 'invalidCoordinate',
    ...
    for (const [index, segment] of (route.segments ?? []).entries()) {
      ...
      const frame = ChallengeMode.getPlanningFrame(level, Number(executionFrom.t ?? segment.from.t ?? waypoint.t ?? 0), {
        challengeMode: gameState?.challengeMode,
        revealTruth: gameState?.ui?.revealTruth,
        forecastMemberId: gameState?.ui?.forecastMemberId
      });
      const estimate = RoutePreview.estimateRouteEnergy(executionFrom, segment.to, level, agent, frame, {
      ...
      const execution = estimate.valid === false || segment.valid === false
        ? null
        : SegmentExecutionValidator.evaluateSegmentForExecution({
          level,
          mission,
          agent,
          from: executionFrom,
      ...
      if (!segment.valid || estimate.valid === false || execution?.ok === false) {
        ...
        const routeBlockDiagnostic = execution?.routeBlockDiagnostic ?? Navigability.buildRouteBlockDiagnostic({
          level,
          mission,
          agentId: agent.id,
          agentLabel: agent.label ?? agent.id,
      ...
      const beachingRisk = ShorelineRisk.estimateSegmentBeachingRisk({
        level,
        frame,
        start: executionFrom,
        end: estimate.valid === false ? estimate.lastValid : segment.to
      });
      if (ShorelineRisk.isBeachingRisk(beachingRisk)) {
        const issue = buildIssue({
          type: 'beachingRisk',
          reason: 'beachingRisk',
          severity: 'warning',
  ...
  result.solverFeedback = RouteDiagnostic.buildSolverValidationFeedback({ routeAudit: result, plan, level, mission });
  return result;
}

 function applyRouteAuditToPlan(plan, audit) {
  clearRouteAuditAnnotations(plan);
...
function buildIssue(issue) {
  ...
  enriched.diagnostic ??= RouteDiagnostic.buildRouteValidationDiagnostic(enriched);
  return enriched;
}
...
function waypointRef(waypoint, index) {
  return {
    kind: 'waypoint',
    waypointKind: WaypointSemantics.normalizeWaypointKind(waypoint),
    index,
    id: waypoint?.id ?? null,
    x: Number(waypoint?.x),
    y: Number(waypoint?.y)
...
function debugAdjacentRouteValidation({ level, mission, from, to, segment, estimate, execution } = {}) {
  ...
  const fromDisplayCell = isFinitePoint(from) ? Navigability.positionToCell(from) : null;
  const toDisplayCell = isFinitePoint(to) ? Navigability.positionToCell(to) : null;
  ...
  const fromNavigable = fromDisplayCell ? Navigability.isCellNavigable(level, mission, fromDisplayCell.x, fromDisplayCell.y) : null;
  const toNavigable = toDisplayCell ? Navigability.isCellNavigable(level, mission, toDisplayCell.x, toDisplayCell.y) : null;
  globalThis.console?.debug?.('[AdjacentRouteValidation]', {
    fromDisplayCell,
    toDisplayCell,
    fromValidationPosition: isFinitePoint(from) ? Navigability.cellToCenterPosition(from) : null,
    toValidationPosition: isFinitePoint(to) ? Navigability.cellToCenterPosition(to) : null,
...
module.exports = {applyRouteAuditToPlan, validateRoutePlanForExecution}