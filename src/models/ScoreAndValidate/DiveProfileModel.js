...
const DIVE_PROFILE_MODEL_VERSION = 'dive-profile-model-p11';
 ...
 function normalizeDiveProfile(profileInput = {}, configInput = {}) {
  ...
  const id = WaterColumnSchema.normalizeWaterColumnProfileId(source.id ?? source.profileId ?? source.mode ?? config.diveProfileId);
 ...
 function createDiveProfileSequence(profileInput = 'sawtoothProfile', configInput = {}, options = {}) {
  ...
  return Array.from({ length: count }, (_value, index) => {
    ...
    return {
      ...
      depthMeters: WaterColumnSchema.waterColumnLayerMetadata(layerId).nominalDepthMeters
    };
  });
}

 function depthIndexForDiveProfile(profileInput = 'sawtoothProfile', progress = 0, configInput = {}) {
  const profile = normalizeDiveProfile(profileInput, configInput);
  const layerId = depthLayerForDiveProfile(profile, progress);
 ...
 function diveProfileCoverage(profileInput = 'sawtoothProfile', configInput = {}, options = {}) {
  const profile = normalizeDiveProfile(profileInput, configInput);
 ...
 function estimateDiveProfileEnergy(profileInput = 'sawtoothProfile', configInput = {}, options = {}) {
  const profile = normalizeDiveProfile(profileInput, configInput);
 ...
 function validateDiveProfile(profileInput = {}, configInput = {}) {
  const profile = normalizeDiveProfile(profileInput, configInput);
  const errors = [];
  const warnings = [];
 ...
 function diveProfileSummary(profileInput = 'sawtoothProfile', configInput = {}) {
  const profile = normalizeDiveProfile(profileInput, configInput);
...
module.exports = {normalizeDiveProfile, createDiveProfileSequence, depthIndexForDiveProfile, depthLayerForDiveProfile, diveProfileCoverage, estimateDiveProfileEnergy, validateDiveProfile, diveProfileSummary}