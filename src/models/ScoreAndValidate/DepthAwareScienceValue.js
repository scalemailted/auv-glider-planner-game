...
const LAYER_BASELINE_FACTORS = Object.freeze({
 ...
 function evaluateDepthAwareSampleValue(options = {}) {
  const config = WaterColumnSchema.normalizeWaterColumnConfig(options.waterColumnConfig ?? options.config ?? options);
  const observation = options.observation ?? {};
  const position = normalizePosition(options.position ?? observation ?? options);
  const depthLayerId = WaterColumnSchema.normalizeWaterColumnLayerId(
  ...
  const metadata = WaterColumnSchema.waterColumnLayerMetadata(depthLayerId);
 ...
 function summarizeDepthAwareScoreEvents(events = [], options = {}) {
  const scoreProfile = DepthScoringProfiles.depthScienceScoreProfileMetadata(options.scoreProfile ?? options.scoreProfileId ?? 'depthAwareScienceV1');
  const config = WaterColumnSchema.normalizeWaterColumnConfig(options.waterColumnConfig ?? options.config ?? { depthLayerIds: uniqueLayers(events) });
...
function sensorCompatibility(depthLayerId, sensorProfile = null) {
  ...
  const depth = Number(WaterColumnSchema.waterColumnLayerMetadata(depthLayerId).nominalDepthMeters ?? 0);
...
function redundancyDiagnostics({ position, depthLayerId, depthMeters, timeSeconds, samplingHistory, fleetSamplingHistory }) {
  ...
  for (const entry of history) {
    ...
    const vertical = Math.abs(Number(entry.depthMeters ?? WaterColumnSchema.waterColumnLayerMetadata(entry.depthLayerId ?? entry.depthLayer ?? 'surface').nominalDepthMeters ?? 0) - depthMeters);
...
function normalizeOptionalLayer(value, config, fallback) {
  ...
  const layerId = WaterColumnSchema.normalizeWaterColumnLayerId(value, fallback);
