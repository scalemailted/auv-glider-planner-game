 function createScalarFieldGrid(config = {}) {
  ...
  return {
    type: 'anchor.scalar-processes.grid',
    width,
    height,
    depthLayers,
 ...
 function createScalarField3d(gridInput, fill = 0) {
  const grid = createScalarFieldGrid(gridInput);
 ...
 function createVectorField3d(gridInput, fillU = 0, fillV = 0) {
  const grid = createScalarFieldGrid(gridInput);
  return {
 ...
 function normalizeField3d(field) {
 ...
 function setCell3d(field, x, y, zIndex, value) {
  const shape = fieldShape(field);
 ...
 function validateField3d(field, gridInput) {
  const grid = createScalarFieldGrid(gridInput);
  const shape = fieldShape(field);
  const errors = [];
 ...
 function cloneField3d(field) {
  return field.map((layer) => layer.map((row) => row.slice()));
}

 function mapField3d(field, mapper) {
...
module.exports = {createScalarFieldGrid, cellCount, createScalarField3d, createVectorField3d, clamp01, normalizeField3d, field3dStats, sampleNearest3d, setCell3d, fieldShape, validateField3d, cloneField3d, mapField3d, forEachFieldCell}