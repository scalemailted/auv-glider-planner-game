...
const SCALAR_FIELD_DIAGNOSTICS_VERSION = 'scalar-field-diagnostics-process-pkg-r1';

 function computeScalarFieldDiagnostics(field = {}) {
 ...
 function validateScalarFieldDiagnostics(diagnostics = {}) {
  const errors = [];
  const warnings = [];
 ...
 function validateScalarClaimBoundary(value = {}) {
  const metadata = ScalarSourceMetadata.normalizeScalarSourceMetadata(value.sourceMetadata ?? value.metadata ?? value);
  const metadataValidation = ScalarSourceMetadata.validateScalarSourceMetadata(metadata);
...
function normalizeFieldInput(field = {}) {
  if (Array.isArray(field)) {
    ...
    return { values, shape: cubeShape(values), sourceMetadata: ScalarSourceMetadata.normalizeScalarSourceMetadata({}), boundaryFlags: {} };
  }
  ...
  return { ...field, values: cube, shape: cubeShape(cube), sourceMetadata: ScalarSourceMetadata.normalizeScalarSourceMetadata(field.sourceMetadata ?? field.provenance ?? {}) };
}
...
module.exports = {computeScalarFieldDiagnostics, validateScalarFieldDiagnostics, scalarFieldStats, scalarFieldMass, scalarFieldDigest, noCalibratedScalarClaims, validateScalarClaimBoundary}