...
const EvidenceCoherenceModel = require('./EvidenceCoherenceModel.js')
const ForecastCorrectionState = require('./ForecastCorrectionState.js')
const HiddenEventHypothesisState = require('./HiddenEventHypothesisState.js')
const ScienceDiagnosisTypes = require('./ScienceDiagnosisTypes.js')
const SCIENCE_DISCOVERY_LIFECYCLE_VERSION = 'science-discovery-lifecycle-p9';

 function createScienceDiscoveryState(options = {}) {
  const primaryDiagnosis = ScienceDiagnosisTypes.normalizeScienceDiagnosisId(options.primaryDiagnosis ?? 'insufficientEvidence');
  return {
    ...
    primaryDiagnosis,
    primaryDiagnosisLabel: ScienceDiagnosisTypes.scienceDiagnosisLabel(primaryDiagnosis),
    diagnosisClass: ScienceDiagnosisTypes.classifyScienceDiagnosis(primaryDiagnosis),
    confidence: clamp01(options.confidence, 0),
    recommendedObjectiveId: options.recommendedObjectiveId ?? ScienceDiagnosisTypes.recommendedObjectiveForScienceDiagnosis(primaryDiagnosis, options),
    forecastCorrection: ForecastCorrectionState.createForecastCorrectionState(options.forecastCorrection ?? {}),
    hiddenEventHypothesis: HiddenEventHypothesisState.createHiddenEventHypothesisState(options.hiddenEventHypothesis ?? {}),
    publicSafe: true,
 ...
 function analyzeScienceEvidence({ observations = [], context = {}, previousState = null, options = {} } = {}) {
  const surpriseRows = ObservationSurpriseModel.computeObservationSurpriseBatch(observations, options.surpriseOptions ?? options);
  const surpriseSummary = ObservationSurpriseModel.observationSurpriseSummary(surpriseRows, options.surpriseOptions ?? options);
  const coherence = EvidenceCoherenceModel.computeEvidenceCoherence(observations, options.coherenceOptions ?? options);
  const decision = chooseScienceDiagnosis({ surpriseSummary, coherence, context, options });
  const primaryDiagnosis = ScienceDiagnosisTypes.normalizeScienceDiagnosisId(options.primaryDiagnosis ?? decision.primaryDiagnosis);
  const recommendedObjectiveId = options.recommendedObjectiveId ?? ScienceDiagnosisTypes.recommendedObjectiveForScienceDiagnosis(primaryDiagnosis, {
    ...context,
  ...
  const forecastCorrection = ForecastCorrectionState.updateForecastCorrectionState(previousState?.forecastCorrection ?? {}, {
    ...surpriseSummary,
    ...coherence,
  ...
  const hiddenEventHypothesis = HiddenEventHypothesisState.updateHiddenEventHypothesisState(previousState?.hiddenEventHypothesis ?? {}, {
    ...coherence,
    diagnosisId: primaryDiagnosis,
    confidence: decision.confidence,
    evidenceConfidence: coherence.evidenceConfidence,
  ...
  return {
    ...
    recommendedObjectiveId,
    surpriseSummary,
    coherenceSummary: EvidenceCoherenceModel.evidenceCoherenceSummary(coherence),
    forecastCorrection: ForecastCorrectionState.forecastCorrectionSummary(forecastCorrection),
    hiddenEventHypothesis: HiddenEventHypothesisState.hiddenEventHypothesisSummary(hiddenEventHypothesis),
    state,
    publicSafe: true,
 ...
 function scienceDiscoverySummary(update = {}) {
  ...
  return {
    ...
    primaryDiagnosisLabel: source?.primaryDiagnosisLabel ?? (source?.primaryDiagnosis ? ScienceDiagnosisTypes.scienceDiagnosisLabel(source.primaryDiagnosis) : null),
    diagnosisClass: source?.diagnosisClass ?? (source?.primaryDiagnosis ? ScienceDiagnosisTypes.classifyScienceDiagnosis(source.primaryDiagnosis) : null),
 ...
 function buildScienceDiagnosticsArtifact(update = {}, options = {}) {
...
module.exports = {SCIENCE_DISCOVERY_LIFECYCLE_VERSION, createScienceDiscoveryState, analyzeScienceEvidence, scienceDiscoverySummary, buildScienceDiagnosticsArtifact}