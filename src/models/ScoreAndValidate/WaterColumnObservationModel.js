...
const WATER_COLUMN_OBSERVATION_MODEL_VERSION = 'water-column-observation-model-p11';

 function createWaterColumnObservation(options = {}) {
  const config = WaterColumnSchema.normalizeWaterColumnConfig(options.waterColumnConfig ?? options);
 ...
 function waterColumnObservationFromHeadlessObservation(observation = {}, configInput = {}, options = {}) {
 ...
 function summarizeWaterColumnObservations(observations = [], configInput = {}) {
 ...
 function buildWaterColumnSummary({
  config: configInput = {},
  observations = [],
  tracks = [],
  diveProfile = null,
  ...
  const config = WaterColumnSchema.normalizeWaterColumnConfig(configInput.waterColumnConfig ?? configInput);
  const profile = DiveProfileModel.normalizeDiveProfile(diveProfile ?? config.diveProfileId, config);
  ...
  return {
    ...
    diveProfile: DiveProfileModel.diveProfileSummary(profile, config),
    observationSummary,
 ...
 function validateWaterColumnObservationSummary(summary = {}) {
  const errors = [];
  const warnings = [];
...
module.exports = {createWaterColumnObservation, waterColumnObservationFromHeadlessObservation, summarizeWaterColumnObservations, buildWaterColumnSummary, validateWaterColumnObservationSummary}