...
const RouteValidityAudit = require('./RouteValidityAudit.js')
const Navigability = require('./Navigability.js')
MAX_EXECUTION_WAYPOINTS_PER_AGENT = 50;
const MAX_INVALID_WAYPOINTS_PER_AGENT = 3;

 function validatePlanForExecution({ level, mission, plan } = {}) {
  const errors = [];
  const warnings = [];
  const routeAudit = RouteValidityAudit.validateRoutePlanForExecution({ level, mission, plan });
  ...
  if (level && mission && usesDeploymentZones) {
    const connectivity = ConnectivityValidator.computeReachabilitySummary(level, mission);
  ...
  for (const agent of mission?.agents ?? []) {
    ...
    const selectedStart = DeploymentZones.getSelectedStart(agent);
    ...
    if (DeploymentZones.requiresDeploymentSelection(mission, agent.id)) {
      ...
      const validation = DeploymentZones.isValidSelectedStart(level, mission, agent.id, selectedStart);
    ...
    for (const [index, waypoint] of waypoints.entries()) {
      ...
      const navigability = Navigability.isCellNavigable(level, mission, x, y);
...
module.exports = {validatePlanForExecution}