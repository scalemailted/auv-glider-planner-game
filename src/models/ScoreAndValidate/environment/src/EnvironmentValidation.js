...
const ENVIRONMENT_VALIDATION_VERSION = 'environment-validation-env-pkg-r1';

 function validateEnvironmentArtifact(value = {}) {
  const artifact = value?.type === 'anchor.environment.artifact' ? value : null;
  const errors = [];
  const warnings = [];
  const checks = [];
  if (!artifact) {
    return EnvironmentUtil.createEnvironmentValidationReport({ errors: ['Environment artifact type must be anchor.environment.artifact.'], warnings, checks });
  }

  addReport('manifest', EnvironmentManifest.validateEnvironmentManifest(artifact.manifest ?? {}), errors, warnings, checks);
  addReport('field-registry', EnvironmentFieldRegistry.validateEnvironmentFieldRegistry(artifact.fieldRegistry ?? {}), errors, warnings, checks);

  if (artifact.bathymetry) addReport('bathymetry', bathymetryIndex.validateBathymetryArtifact(artifact.bathymetry), errors, warnings, checks);
  else warnings.push('Environment artifact has no bathymetry component.');

  for (const field of artifact.currentFields ?? []) addReport(`current:${field.id ?? 'unknown'}`, currentsIndex.validateCurrentField4D(field), errors, warnings, checks);
  for (const field of artifact.scalarFields ?? []) addReport(`scalar:${field.id ?? 'unknown'}`, scalarProcessesIndex.validateScalarField4D(field), errors, warnings, checks);
  ...
  return EnvironmentUtil.createEnvironmentValidationReport({ errors, warnings, checks });
}

 function environmentValidationSummary(report = {}) {
  return {
...
function validateFieldCoverage(field, warnings, checks, kind) {
  const depth = EnvironmentUtil.axisExtent(field.depthAxisMeters);
  const time = EnvironmentUtil.axisExtent(field.timeAxisSeconds);
...
module.exports = {validateEnvironmentArtifact, environmentValidationSummary}