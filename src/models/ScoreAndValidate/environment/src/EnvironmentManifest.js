...
const ENVIRONMENT_MANIFEST_VERSION = 'environment-manifest-env-pkg-r1';
 const SYNTHETIC_ENVIRONMENT_MANIFEST_VERSION = 'synthetic-environment-manifest-flow-r2a-5-1';

 function createEnvironmentManifest(options = {}) {
  const backend = EnvironmentGeneratorBackendContract.normalizeEnvironmentGeneratorBackend(options.generatorBackend ?? options.backendId ?? options.backend ?? 'cpuBathymetryConditionedSyntheticV3');
  const operationalDomain = EnvironmentUtil.normalizeOperationalDomain(
    options.operationalDomain,
    EnvironmentUtil.domainFromAxes({
  ...
  const manifest = {
    ...
    claimBoundary: EnvironmentUtil.normalizeClaimBoundary(options.claimBoundary ?? options.sourceMetadata?.claimBoundary ?? options.source ?? {})
  };
  return { ...manifest, manifestDigest: options.manifestDigest ?? environmentManifestDigest(manifest) };
}

 function normalizeEnvironmentManifest(value = {}) {
 ...
 function environmentManifestSummary(value = {}) {
  const manifest = normalizeEnvironmentManifest(value);
  return {
 ...
 function environmentManifestDigest(value = {}) {
  const manifest = { ...value };
  delete manifest.manifestDigest;
  delete manifest.digest;
  return EnvironmentUtil.stableDigest(manifest);
}

 function createSyntheticEnvironmentManifest(options = {}) {
  const backend = EnvironmentGeneratorBackendContract.normalizeEnvironmentGeneratorBackend(options.backendId ?? options.backend ?? 'cpuBathymetryConditionedSyntheticV3');
 ...
 function normalizeSyntheticEnvironmentManifest(input = {}) {
 ...
 function validateSyntheticEnvironmentManifest(input = {}) {
 ...
 function syntheticEnvironmentManifestDigest(manifest = {}) {
  const copy = { ...manifest };
  delete copy.digest;
...
module.exports = {SYNTHETIC_ENVIRONMENT_MANIFEST_VERSION, createEnvironmentManifest, normalizeEnvironmentManifest, validateEnvironmentManifest, environmentManifestSummary, environmentManifestDigest, createSyntheticEnvironmentManifest, normalizeSyntheticEnvironmentManifest, validateSyntheticEnvironmentManifest, syntheticEnvironmentManifestDigest}