...
const MOTION_COST_GRAPH_NEIGHBORS_VERSION = 'motion-cost-graph-neighbors-sim-r1';

 function createMotionCostGraphNeighborPairs(nodes = [], configInput = {}) {
 ...
 function motionCostNeighborSummary(pairs = [], configInput = {}) {
  const config = configInput?.type === 'anchor.motion.cost-graph-config' ? configInput : MotionCostGraphSchema.createMotionCostGraphConfig(configInput);
  return {
...
module.exports = {MOTION_COST_GRAPH_NEIGHBORS_VERSION, createMotionCostGraphNeighborPairs, motionCostNeighborSummary}