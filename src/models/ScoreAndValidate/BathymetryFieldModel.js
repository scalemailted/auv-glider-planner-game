...
const BATHYMETRY_FIELD_MODEL_VERSION = 'bathymetry-field-model-gfx-r2';

 function createBathymetryField(options = {}) {
  if (options.depthMeters && Array.isArray(options.depthMeters)) {
    const config = BathymetrySchema.createBathymetryConfig({
      ...options,
    ...
    const bathymetry = {
      ...
      sourceMetadata: BathymetrySourceMetadata.createBathymetrySourceMetadata({ sourceId: options.seed ?? 'provided-bathymetry', synthetic: options.synthetic !== false }),
 ...
 function createSyntheticBathymetryField(options = {}) {
  const config = BathymetrySchema.createBathymetryConfig(options);
  const rng = SeededRng.createSeededRng(options.seed ?? 'bathymetry-env-r1');
  const depthMeters = [];
  ...
  for (let y = 0; y < config.height; y += 1) {
    ...
    for (let x = 0; x < config.width; x += 1) {
      ...
      const noise = (rng() - 0.5) * 2.2 + (SeededRng.seededUnit(`${options.seed ?? 'bathymetry'}:${x}:${y}`) - 0.5) * 1.8;
  ...
  const bathymetry = {
    ...
    calibrated: false,
    operationallyValidated: false,
    sourceMetadata: BathymetrySourceMetadata.createBathymetrySourceMetadata({ sourceId: options.seed ?? 'bathymetry-env-r1', seed: options.seed ?? 'bathymetry-env-r1', synthetic: true }),
    notA: config.notA.slice()
  };
 ...
 function createLandSeaMaskFromBathymetry(bathymetry, options = {}) {
 ...
 function createDepthAccessibilityField(bathymetry, options = {}) {
 ...
 function createBathymetryHazardField(bathymetry, options = {}) {
 ...
 function bathymetryFieldStats(bathymetry) {
 ...
 function bathymetryGradientField(bathymetry) {
  const depth = bathymetry?.depthMeters ?? [];
 ...
 function bathymetrySlopeField(bathymetry) {
  return bathymetryGradientField(bathymetry).map((row) => row.map((cell) => round(cell.magnitude)));
}

 function bathymetryFeatureSummary(bathymetry, options = {}) {
 ...
 function sampleBathymetryAt(bathymetry, x, y) {
  const depth = bathymetry?.depthMeters ?? [];
 ...
 function createCoastalOperationalBathymetry(options = {}) {
 ...
 function createIslandArcBathymetry(options = {}) {
 ...
 function createShelfCanyonBathymetry(options = {}) {
 ...
 function createBasinSeamountBathymetry(options = {}) {
...
// Production Three terrain uses BathymetryMeshGeometry plus shared terrain layers.
 function bathymetryToTerrainMeshData(bathymetry, options = {}) {
  const depth = bathymetry?.depthMeters ?? [];
 ...
 function extractCoastlineEdges(landMaskInput, options = {}) {
...
function createScenarioBathymetry(options = {}) {
  ...
  bathymetry.sourceMetadata = BathymetrySourceMetadata.createBathymetrySourceMetadata({ sourceId: seed, seed, label: "Synthetic  bathymetry", synthetic: true });
...
function scenarioDepthCell(nx, ny, scenarioId, config, seed, x, y) {
  ...
  const roughness = (SeededRng.seededUnit(`${seed}:rough:${x}:${y}`) - 0.5) * 5.6 + terrainNoise(x, y, seed) * 2.4;
...
function terrainNoise(x, y, seed) {
  return (SeededRng.seededUnit(`${seed}:terrain:${Math.floor(x / 2)}:${Math.floor(y / 2)}`) - 0.5)
    + (SeededRng.seededUnit(`${seed}:terrain-hi:${x}:${y}`) - 0.5) * 0.55;
}
 function validateBathymetryField(bathymetry, configInput = bathymetry?.config ?? {}) {
  const errors = [];
  const warnings = [];
  const configValidation = BathymetrySchema.validateBathymetryConfig(configInput);
...
module.exports = {createBathymetryField, createSyntheticBathymetryField, createLandSeaMaskFromBathymetry, createDepthAccessibilityField, createBathymetryHazardField, bathymetryFieldStats, bathymetryGradientField, bathymetrySlopeField, bathymetryFeatureSummary, sampleBathymetryAt, createCoastalOperationalBathymetry, createIslandArcBathymetry, createShelfCanyonBathymetry, createBasinSeamountBathymetry, bathymetryToTerrainMeshData, extractCoastlineEdges, validateBathymetryField}