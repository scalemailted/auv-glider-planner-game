const MissionObjectiveTaxonomy = require('./MissionObjectiveTaxonomy.js')
const HeadlessSchemaContract = require('./HeadlessSchemaContract.js')
const HeadlessFieldSchema = require('./HeadlessFieldSchema.js')
const HEADLESS_MISSION_SCHEMA_VERSION = 'headless-mission-schema-h0';

 function createHeadlessMissionConfig(options = {}) {
  ...
  const informationAccessTier = HeadlessSchemaContract.normalizeHeadlessVisibilityTier(options.informationAccessTier ?? mission.informationAccessTier ?? mission.visibilityTier ?? 'publicScenario');
  return compactObject({
    ...
    Policy: cloneJson(options.Policy ?? { includeHiddenTruth: informationAccessTier === 'hiddenTruth' || informationAccessTier === 'oracle' || informationAccessTier === 'debugAll' }),
    notes: normalizeStringList(options.notes)
  });
}

 function createHeadlessWorldConfig(options = {}) {
  ...
  const fieldDescriptors = normalizeArray(options.fieldDescriptors).map(HeadlessFieldSchema.createHeadlessFieldDescriptor);
  return compactObject({
    width,
    height,
 ...
 function createHeadlessGliderConfig(options = {}) {
 ...
 function createHeadlessObjectiveConfig(options = {}) {
  const objectiveId = MissionObjectiveTaxonomy.normalizeMissionObjectiveId(options.id ?? options.objectiveId ?? options.type ?? 'reconnaissanceSurvey');
  const taxonomy = MissionObjectiveTaxonomy.missionObjectiveById(objectiveId);
  return compactObject({
    id: taxonomy.id,
 ...
 function validateHeadlessMissionConfig(config = {}) {
  const errors = [];
  const warnings = [];
 ...
 function headlessMissionConfigSummary(config = {}) {
...
module.exports = {createHeadlessMissionConfig, createHeadlessWorldConfig, createHeadlessGliderConfig, createHeadlessObjectiveConfig, validateHeadlessMissionConfig, headlessMissionConfigSummary}