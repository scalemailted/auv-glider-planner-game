...
const GENERATION_VERSION = 'anchor-generator-v1';

 const REPLAY_SEED_NAMESPACES = [
  'terrain',
  'currents',
  'roi',
  'hazards',
 ...
 function deriveSeedFromUuid(uuid, namespace = 'default') {
  const anchor = String(uuid ?? '').trim();
  const key = String(namespace ?? 'default').trim() || 'default';
  if (!anchor) return null;
  return `${GENERATION_VERSION}:${key}:${SeededRng.hashSeed(`${anchor}:${key}`).toString(36)}`;
}

 function deriveReplaySeeds(uuid, namespaces = REPLAY_SEED_NAMESPACES) {
  const anchor = String(uuid ?? '').trim();
  if (!anchor) return {};
  return Object.fromEntries(namespaces.map((namespace) => [namespace, deriveSeedFromUuid(anchor, namespace)]));
}

 function buildReplaySeedContract({
  challengeId,
 ...
 function getReplaySeedContract(source = {}) {
 ...
 function evaluateExactReplayAvailability(source = {}) {
...
module.exports = {REPLAY_SEED_NAMESPACES, deriveSeedFromUuid, deriveReplaySeeds, buildReplaySeedContract, getReplaySeedContract, evaluateExactReplayAvailability}