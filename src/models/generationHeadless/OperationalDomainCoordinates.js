...
const OPERATIONAL_DOMAIN_COORDINATES_VERSION = 'operational-domain-coordinates-world-r1';

 function physicalPointToUv(point = {}, domainInput = {}) {
  const domain = OperationalDomainSpec.normalizeOperationalDomainSpec(domainInput);
 ...
 function uvToPhysicalPoint(uv = {}, domainInput = {}) {
  const domain = OperationalDomainSpec.normalizeOperationalDomainSpec(domainInput);
 ...
 function physicalPointToGridSample(point = {}, domainInput = {}, gridInput = {}, options = {}) {
 ...
 function gridCellToPhysicalPoint(cell = {}, domainInput = {}, gridInput = {}, options = {}) {
  const grid = normalizeGrid(gridInput);
 ...
 function physicalPointToPlanningCell(point = {}, domainInput = {}, profileInput = {}, options = {}) {
  const profile = MissionResolutionProfile.normalizeMissionResolutionProfile(profileInput);
  return physicalPointToGridSample(point, domainInput, profile.planningLattice, options);
}

 function physicalPointToRoleSample(point = {}, domainInput = {}, profileInput = {}, role = 'planning', options = {}) {
  const profile = MissionResolutionProfile.normalizeMissionResolutionProfile(profileInput);
  return physicalPointToGridSample(point, domainInput, MissionResolutionProfile.resolutionGridForRole(profile, role), options);
}

 function legacyGridPointToPhysicalPoint(point = {}, domainInput = {}, profileInput = {}, role = 'planning') {
  const profile = MissionResolutionProfile.normalizeMissionResolutionProfile(profileInput);
  return gridCellToPhysicalPoint(point, domainInput, MissionResolutionProfile.resolutionGridForRole(profile, role));
}

 function physicalPointToThreeWorld(point = {}, domainInput = {}, options = {}) {
  const domain = OperationalDomainSpec.normalizeOperationalDomainSpec(domainInput);
 ...
 function threeWorldToPhysicalPoint(worldPoint = {}, domainInput = {}, options = {}) {
  const domain = OperationalDomainSpec.normalizeOperationalDomainSpec(domainInput);
  const worldWidth = Math.max(1, finite(options.worldWidthUnits, 160));
  const worldDepth = Math.max(1, finite(options.worldDepthUnits, worldWidth * domain.horizontal.heightMeters / Math.max(1, domain.horizontal.widthMeters)));
  const depthScale = Math.max(0.000001, finite(options.depthScale, 0.045));
 ...
 function coordinateRoundtripDiagnostics(options = {}) {
  const domain = OperationalDomainSpec.normalizeOperationalDomainSpec(options.domain);
  const profile = MissionResolutionProfile.normalizeMissionResolutionProfile(options.profile);
...
module.exports = {physicalPointToUv, uvToPhysicalPoint, physicalPointToGridSample, gridCellToPhysicalPoint, physicalPointToPlanningCell, physicalPointToRoleSample, legacyGridPointToPhysicalPoint, physicalPointToThreeWorld, threeWorldToPhysicalPoint, coordinateRoundtripDiagnostics}