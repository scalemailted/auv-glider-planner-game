...
function generateROI(width, height, t, config = {}) {
  const sampleFieldConfig = SampleFieldConfig.normalizeSampleFieldConfig(
    config.sampleFieldConfig ?? config.sampleField ?? SampleFieldConfig.sampleFieldConfigFromLegacyRoi(config),
    {
 ...
 function generateSampleField(width, height, t, config = {}) {
  const fieldConfig = SampleFieldConfig.normalizeSampleFieldConfig(config.sampleFieldConfig ?? config.sampleField ?? {});
 ...
 function createHotspots(width, height, count, pattern, random = Math.random) {
...
function spatialBaseValue(x, y, width, height, fieldConfig, seed, t) {
  ...
  if (fieldConfig.spatialPattern === 'randomTexture') {
    const coarse = SeededRng.seededUnit(`${seed}:texture:${Math.floor(x / 2)}:${Math.floor(y / 2)}`);
    const fine = SeededRng.seededUnit(`${seed}:texture:fine:${x}:${y}`);
...
function getTemporalPulse(fieldConfig, t, index, hotspot, seed) {
  ...
  if (fieldConfig.temporalBehavior === 'bursty') {
    const cycle = 24;
    const peak = 3 + SeededRng.seededUnit(`${seed}:burst-peak:${index}`) * 18;
    const localTime = ((t + cycle - peak) % cycle);
    const centered = Math.min(localTime, cycle - localTime);
    const duration = 1.8 + SeededRng.seededUnit(`${seed}:burst-duration:${index}`) * 3.8;
  ...
  if (fieldConfig.temporalBehavior === 'markovNeighbor') {
    const activation = SeededRng.seededUnit(`${seed}:markov:${index}:${Math.floor(t / 3)}`) > 0.48 ? 1 : 0.45;
...
function temporalNoise(x, y, width, height, t, fieldConfig, seed) {
  ...
  const phase = SeededRng.seededUnit(`${seed}:phase:${x}:${y}`) * Math.PI * 2;
  const freq = 0.14 + SeededRng.seededUnit(`${seed}:freq:${x}:${y}`) * 0.28;
...
module.exports = {generateSampleField, createHotspots}