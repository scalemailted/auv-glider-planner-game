...
const BATHYMETRY_MANIFEST_VERSION = 'bathymetry-manifest-bathy-pkg-r1';

 function createBathymetryManifest(options = {}) {
  return normalizeBathymetryManifest(options);
}

 function normalizeBathymetryManifest(value = {}) {
  ...
  const sourceMetadata = BathymetrySourceMetadata.normalizeBathymetrySourceMetadata()
    ? BathymetrySourceMetadata.normalizeBathymetrySourceMetadata(source.sourceMetadata ?? source.source ?? {})
    : BathymetrySourceMetadata.createBathymetrySourceMetadata(source.sourceMetadata ?? source.source ?? {});
  const manifest = {
 ...
 function validateBathymetryManifest(value = {}) {
  ...
  return index.createValidationReport({ errors, warnings, checks: [{ id: 'bathymetry-manifest-shape', passed: errors.length === 0 }] });
}

 function bathymetryManifestSummary(value = {}) {
  const manifest = normalizeBathymetryManifest(value);
  return {
 ...
 function bathymetryManifestDigest(value = {}) {
  const payload = { ...value };
...
module.exports = {createBathymetryManifest, normalizeBathymetryManifest, validateBathymetryManifest, bathymetryManifestSummary, bathymetryManifestDigest}