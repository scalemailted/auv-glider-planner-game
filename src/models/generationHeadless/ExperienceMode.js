 ...
 const EXPERIENCE_MODE_DEFAULTS = {
  challenge: {
  ...
  simulationLab: {
    ...
    shortLabel: 'Lab',
    description: 'Configure reproducible experiments,  solver packets, and inspect current-field diagnostics.',
 ...
 function normalizeExperienceMode(value, fallback = EXPERIENCE_MODES.challenge) {
  return Object.values(EXPERIENCE_MODES).includes(value) ? value : fallback;
}

 function getExperienceModeDefaults(value) {
  const mode = normalizeExperienceMode(value);
  return EXPERIENCE_MODE_DEFAULTS[mode];
}

 function experienceModeLabel(value) {
  return getExperienceModeDefaults(value).label;
}

module.exports = {EXPERIENCE_MODES, EXPERIENCE_MODE_DEFAULTS, normalizeExperienceMode, getExperienceModeDefaults, experienceModeLabel}