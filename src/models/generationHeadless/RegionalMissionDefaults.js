...
const BathymetryArtifactAdapter = require('./BathymetryArtifactAdapter.js')
const SignedTerrainSurface = require('./bathymetry/src/SignedTerrainSurface.js')
...
const OperationalDomainCoordinates = require('./OperationalDomainCoordinates.js')
const MissionScaleModel = require('./MissionScaleModel.js')
const REGIONAL_MISSION_DEFAULTS_VERSION = 'regional-mission-defaults-world-r1';

 function createRegionalContinentalShelfScenario(options = {}) {
  const seed = String(options.seed ?? 'world-r1-regional-shelf');
  const domain = OperationalDomainSpec.createOperationalDomainSpec(options.domain ?? {});
  const profile = MissionResolutionProfile.normalizeMissionResolutionProfile(options.resolutionProfile ?? options.profile ?? 'regionalShelfFleet');
  ...
  const bathymetry = BathymetryFieldModel.createCoastalOperationalBathymetry({
  ...
  const signedTerrainSurface = SignedTerrainSurface.createSignedTerrainSurfaceFromBathymetry(bathymetry, { minimumNavigableDepthMeters: 8 });
  const bathymetryArtifact = BathymetryArtifactAdapter.createBathymetryArtifactFromField(bathymetry, {
  ...
  const bathymetryArtifactSummary = BathymetryArtifactAdapter.bathymetryArtifactAdapterSummary(bathymetryArtifact);
  bathymetry.operationalDomain = OperationalDomainSpec.operationalDomainSummary(domain);
  bathymetry.resolutionRole = 'terrainGrid';
  bathymetry.signedTerrainSurfaceDigest = signedTerrainSurface.digest;
  const regionalScience = buildField(scienceGrid, (u, v) => SignedTerrainSurface.sampleSignedTerrainSurfaceAtUv(signedTerrainSurface, u, v).navigable ? evaluateRegionalScienceValueAtUv(u, v, { seed }) : 0);
  const regionalUncertainty = buildField(scienceGrid, (u, v) => SignedTerrainSurface.sampleSignedTerrainSurfaceAtUv(signedTerrainSurface, u, v).navigable ? evaluateRegionalUncertaintyAtUv(u, v, { seed }) : 0);
  const regionalCurrent = buildVectorField(currentGrid, (u, v) => SignedTerrainSurface.sampleSignedTerrainSurfaceAtUv(signedTerrainSurface, u, v).navigable ? evaluateRegionalCurrentAtUv(u, v, { seed }) : { u: 0, v: 0, magnitude: 0 });
  const roi = buildField(planning, (u, v) => SignedTerrainSurface.sampleSignedTerrainSurfaceAtUv(signedTerrainSurface, u, v).navigable ? evaluateRegionalScienceValueAtUv(u, v, { seed }) : 0);
  const terrain = buildField(planning, (u, v) => SignedTerrainSurface.sampleSignedTerrainSurfaceAtUv(signedTerrainSurface, u, v).land ? 1 : 0, { integer: true });
  const hazards = buildField(planning, (u, v) => evaluateRegionalHazardAtUv(u, v, { seed, signedTerrainSurface }), { integer: false });
  const current = buildVectorField(planning, (u, v) => SignedTerrainSurface.sampleSignedTerrainSurfaceAtUv(signedTerrainSurface, u, v).navigable ? evaluateRegionalCurrentAtUv(u, v, { seed }) : { u: 0, v: 0, magnitude: 0 });
  ...
  const deploymentZoneCellsFinal = [];
  for (let y = 1; y <= 2; y += 1) {

    for (let x = 1; x <= 3; x += 1) {
      deploymentZoneCellsFinal.push({ "x":x, "y":y });
    }
  }
  const level = {
    schemaVersion: '2.0',
    ...
    meta: {
      ...
      operationalDomainDigest: OperationalDomainSpec.operationalDomainDigest(domain),
    ...
    terrainAuthority: SignedTerrainSurface.signedTerrainSurfaceSummary(signedTerrainSurface),
    signedTerrainSurface,
    world: {
      grid: {
    ...
    layers: {
      terrain,
      hazards,
      depth: buildField(planning, (u, v) => SignedTerrainSurface.sampleSignedTerrainSurfaceAtUv(signedTerrainSurface, u, v).bottomDepthMeters, { integer: false }),
    ...
    zones: deploymentZoneCellsFinal
  };
  return level;
}

 function createRegionalFleetMission(options = {}) {
  const seed = String(options.seed ?? 'world-r1-regional-shelf');
  const profile = MissionResolutionProfile.normalizeMissionResolutionProfile(options.resolutionProfile ?? options.profile ?? 'regionalShelfFleet');
  const planning = profile.planningLattice;
 ...
 function createRegionalMissionBundle(options = {}) {
 ...
 function createRegionalMissionCompactExport({ level = null, mission = null, seed = null } = {}) {
  ...
  const scaleModel = MissionScaleModel.createMissionScaleModel({
  ...
  const nominalRoutes = sourceMission.agents.map((agent, index) => {
    ...
    return { agentId: agent.id, routeScale: MissionScaleModel.estimateRouteScale([start, end], scaleModel) };
  });
  return {
    type: 'anchor.world.regional-compact-',
    version: REGIONAL_MISSION_DEFAULTS_VERSION,
    schemaVersion: '1.0',
    meta: {
    ...
    operationalDomain: OperationalDomainSpec.operationalDomainSummary(sourceLevel.operationalDomain),
    resolutionProfile: MissionResolutionProfile.missionResolutionProfileSummary(sourceLevel.resolutionProfile),
    fieldDigests: {
      operationalDomain: OperationalDomainSpec.operationalDomainDigest(sourceLevel.operationalDomain),
    ...
    notes: 'Use this compact  for roundtrip metadata checks; full level/regionalFields remain available in createRegionalMissionBundle().'
  };
}

 function evaluateRegionalScienceValueAtUv(u, v, options = {}) {
  const seed = String(options.seed ?? 'world-r1-regional-shelf');
  ...
  const texture = (SeededRng.seededUnit(`${seed}:science:${Math.round(u * 100)}:${Math.round(v * 100)}`) - 0.5) * 0.07;
  return round(clamp01(0.12 + 0.45 * shelfFront + 0.28 * plume + 0.2 * eddyRetained + texture));
}

 function evaluateRegionalUncertaintyAtUv(u, v, options = {}) {
  const seed = String(options.seed ?? 'world-r1-regional-shelf');
  const offshore = clamp01(u);
  const sparse = 0.22 + 0.38 * offshore;
  const front = 0.28 * Math.exp(-((u - 0.46) ** 2) / 0.018);
  const texture = (SeededRng.seededUnit(`${seed}:uncertainty:${Math.round(u * 80)}:${Math.round(v * 80)}`) - 0.5) * 0.05;
  return round(clamp01(sparse + front + texture));
}

 function evaluateRegionalCurrentAtUv(u, v, options = {}) {
  const seed = String(options.seed ?? 'world-r1-regional-shelf');
  ...
  const jitter = (SeededRng.seededUnit(`${seed}:current:${Math.round(u * 64)}:${Math.round(v * 64)}`) - 0.5) * 0.035;
  const vector = {
 ...
 function evaluateRegionalLandMaskAtUv(u, v) {
  const coastline = 0.08 + 0.035 * Math.sin(v * Math.PI * 3.2) + 0.045 * Math.exp(-((v - 0.62) ** 2) / 0.04);
  return u < coastline ? 1 : 0;
}

 function evaluateRegionalHazardAtUv(u, v, options = {}) {
  const terrainSample = options.signedTerrainSurface ? SignedTerrainSurface.sampleSignedTerrainSurfaceAtUv(options.signedTerrainSurface, u, v) : null;
  ...
  return round(clamp01(0.08 * shallowBank + 0.18 * steepCanyon + (SeededRng.seededUnit(`${options.seed ?? 'world-r1'}:hazard:${Math.round(u * 48)}:${Math.round(v * 30)}`) > 0.985 ? 0.55 : 0)));
}

 function evaluateRegionalBathymetryDepthAtUv(u, v, domain = null) {
  ...
  const zoneCells = (centerY) => {
    ...
    for (let y = Math.max(1, centerY - 2); y <= Math.min(planning.rows - 2, centerY + 2); y += 1) {
      for (let x = 6; x <= Math.min(planning.columns - 2, 11); x += 1) {
        ...
        if (x === 8 && y === 8) continue;
        const sample = SignedTerrainSurface.sampleSignedTerrainSurfaceAtUv(signedTerrainSurface, u, v);
...
function buildPriorityTargets(planning, seed) {
  const rng = SeededRng.createSeededRng(`${seed}:priority`);
  return Array.from({ length: 4 }, (_value, index) => {
    ...
    return {
      ...
      frames: [
        ...
        { t: index + 12, x: Math.min(planning.columns - 2, x + 2), y: Math.max(1, y - 1), active: true },
        { t: index + 16, active: false }
      ]
    };
  });
}
...
module.exports = {createRegionalContinentalShelfScenario, createRegionalFleetMission, createRegionalMissionBundle, createRegionalMissionCompactExport, evaluateRegionalScienceValueAtUv, evaluateRegionalUncertaintyAtUv, evaluateRegionalCurrentAtUv, evaluateRegionalLandMaskAtUv, evaluateRegionalHazardAtUv, evaluateRegionalBathymetryDepthAtUv}