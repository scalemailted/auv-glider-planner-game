...
const MISSION_SCALE_MODEL_VERSION = 'mission-scale-model-world-r1';

 function createMissionScaleModel(options = {}) {
  const domain = OperationalDomainSpec.normalizeOperationalDomainSpec(options.domain ?? options.operationalDomain, { grid: options.grid });
  const profile = MissionResolutionProfile.normalizeMissionResolutionProfile(options.profile ?? options.resolutionProfile ?? {});
  const glider = {
 ...
 function missionPointToPhysicalPoint(point = {}, scaleModel = {}) {
  ...
  return OperationalDomainCoordinates.gridCellToPhysicalPoint(point, model.domain, model.resolutionProfile.planningLattice);
}

 function distanceMetersBetweenMissionPoints(a = {}, b = {}, scaleModel = {}) {
  const model = normalizeScaleModel(scaleModel);
 ...
 function estimateSegmentDurationSeconds(a = {}, b = {}, scaleModel = {}) {
  const model = normalizeScaleModel(scaleModel);
  return round(distanceMetersBetweenMissionPoints(a, b, model) / model.glider.nominalSpeedMetersPerSecond);
}

 function estimateRouteScale(routePoints = [], scaleModel = {}) {
  const model = normalizeScaleModel(scaleModel);
 ...
 function missionScaleModelSummary(scaleModel = {}) {
  const model = normalizeScaleModel(scaleModel);
 ...
 function validateMissionScaleModel(scaleModel = {}) {
  const model = normalizeScaleModel(scaleModel);
  const errors = [];
...
module.exports = {createMissionScaleModel, missionPointToPhysicalPoint, distanceMetersBetweenMissionPoints, estimateSegmentDurationSeconds, estimateRouteScale, missionScaleModelSummary, validateMissionScaleModel}