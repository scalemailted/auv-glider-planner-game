...
const DEPTH_AWARE_SCIENCE_VALUE_VERSION = 'depth-aware-science-value-three-r1-2a-2';
 const DEPTH_OBJECTIVE_WEIGHT_PROFILE_VERSION = 'depth-objective-weight-profile-three-r1-2a-2';

 const OBJECTIVE_DEPTH_WEIGHT_PROFILES = Object.freeze({
...
const LAYER_BASELINE_FACTORS = Object.freeze({
 ...
 function objectiveDepthWeightProfileById(id = 'generalSurvey') {
  const normalized = normalizeObjectiveWeightProfileId(id);
  return cloneProfile(OBJECTIVE_DEPTH_WEIGHT_PROFILES[normalized]);
}

 function inferObjectiveWeightProfileId(missionObjective = null, fallback = 'generalSurvey') {
  const candidates = [
 ...
 function evaluateDepthAwareSampleValue(options = {}) {
  const config = WaterColumnProfileRuntime.normalizeWaterColumnConfig(options.waterColumnConfig ?? options.config ?? options);
  const observation = options.observation ?? {};
  const position = normalizePosition(options.position ?? observation ?? options);
  const depthLayerId = WaterColumnProfileRuntime.normalizeWaterColumnLayerId(
  ...
  const metadata = WaterColumnProfileRuntime.waterColumnLayerMetadata(depthLayerId);
  ...
  const scoreProfile = DepthScoringProfiles.depthScienceScoreProfileMetadata(options.scoreProfile ?? options.scoreProfileId ?? 'depthAwareScienceV1', {
 ...
 function evaluateDepthAwareProfileValue(options = {}) {
  const config = WaterColumnProfileRuntime.normalizeWaterColumnConfig(options.waterColumnConfig ?? options.config ?? options);
 ...
 function validateDepthAwareScienceValue(result = {}) {
  const errors = [];
 ...
 function depthAwareScienceValueSummary(result = {}) {
 ...
 function depthAwareSampleScoreEvent(sampleValue = {}, options = {}) {
 ...
 function summarizeDepthAwareScoreEvents(events = [], options = {}) {
  const scoreProfile = DepthScoringProfiles.depthScienceScoreProfileMetadata(options.scoreProfile ?? options.scoreProfileId ?? 'depthAwareScienceV1');
  const config = WaterColumnProfileRuntime.normalizeWaterColumnConfig(options.waterColumnConfig ?? options.config ?? { depthLayerIds: uniqueLayers(events) });
...
function resolveBaseDepthPriority({ priorityField, topDownField, x, y, depthLayerId, config, fallbackBase, objectiveProfile, integratedSample }) {
  if (priorityField) {
    const sampled = WaterColumnFieldModel.sampleWaterColumnScalar(priorityField, x, y, depthLayerId, config);
...
function sensorCompatibility(depthLayerId, sensorProfile = null) {
  ...
  const depth = Number(WaterColumnProfileRuntime.waterColumnLayerMetadata(depthLayerId).nominalDepthMeters ?? 0);
...
function redundancyDiagnostics({ position, depthLayerId, depthMeters, timeSeconds, samplingHistory, fleetSamplingHistory }) {
  ...
  for (const entry of history) {
    ...
    const vertical = Math.abs(Number(entry.depthMeters ?? WaterColumnProfileRuntime.waterColumnLayerMetadata(entry.depthLayerId ?? entry.depthLayer ?? 'surface').nominalDepthMeters ?? 0) - depthMeters);
...
function normalizeOptionalLayer(value, config, fallback) {
  ...
  const layerId = WaterColumnProfileRuntime.normalizeWaterColumnLayerId(value, fallback);
...
module.exports = {DEPTH_OBJECTIVE_WEIGHT_PROFILE_VERSION, OBJECTIVE_DEPTH_WEIGHT_PROFILES, objectiveDepthWeightProfileById, inferObjectiveWeightProfileId, evaluateDepthAwareSampleValue, evaluateDepthAwareProfileValue, validateDepthAwareScienceValue, depthAwareScienceValueSummary, depthAwareSampleScoreEvent, summarizeDepthAwareScoreEvents}