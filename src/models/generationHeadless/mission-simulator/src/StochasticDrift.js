...
function getDriftRules(mission = {}) {
 ...
 function applySeededStochasticDrift(current, { mission, agentId, t = 0 } = {}) {
  ...
  const nx = (SeededRng.seededUnit(`${rules.seed}:${agentId}:drift-x:${timeBucket}`) * 2 - 1) * rules.noiseScale;
  const ny = (SeededRng.seededUnit(`${rules.seed}:${agentId}:drift-y:${timeBucket}`) * 2 - 1) * rules.noiseScale;
  return {
...
module.exports = {applySeededStochasticDrift}