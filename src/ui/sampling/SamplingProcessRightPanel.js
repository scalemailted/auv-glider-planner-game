...
import { processExampleTypeLabel, spatiotemporalProcessExampleTrackLabel } from '../../core/demo/sampling/SpatiotemporalProcessExamples.js';
...
export function roiRecipeSignatureHtml(state) {
  ...
  return `
    <section class="cell-inspector-shell recipe-signature-shell" data-roi-recipe-signature-view>
      ...
      ? 'Legacy MVP preset compatibility view. The visible Process Lab contexts are the main workflow.'
      ...
      ['best views', (signature.bestDisplayLayers ?? []).join(', ')]
          ...
          <small>Process examples are deterministic or seeded CA/grid-process-inspired recipes. Sampling value is an optional interpretation layer, not the identity of this lab.</small>
      ...
      ['compatibility note', 'Legacy presets are kept for old examples, exports, and debugging; the visible Process Lab contexts are the main educational workflow.']
...
function processExampleSummaryCardHtml(example, signature) {
  ...
  const isOceanAnalog = example.exampleType === 'oceanProcessAnalog';
  return `
      
        <div class="cell-inspector-card selected" data-process-example-summary>
      
          <span>${escapeHtml(isFoundational ? 'Foundational CA Model' : isOceanAnalog ? 'Ocean-Relevant Process Analog' : 'Observable Process Pattern')}</span>
      
          ${metricRows([
      
            
      ['track', example.exampleTrackLabel ?? spatiotemporalProcessExampleTrackLabel(example.track)],
      ...
      ['mapped pattern', signature?.label],
      ...
      ['observable tags', (example.observableProcessPatternTags ?? []).join(', ') || 'n/a'],
      ...
      ...(isOceanAnalog ? [
        ...
        ['environmental process', example.environmentalProcess],
      
            
        ['underlying mechanism', example.underlyingCaMechanism],
      
            
        ['requires flow coupling', example.requiresFlowCoupling ? 'yes' : 'no'],
      
            
        ['requires uncertainty', example.requiresUncertaintyForMissionRealism ? 'yes' : 'no'],
      
            
        ['sampling strategy', example.recommendedSamplingStrategy],
      
            
        ['missing science layers', (example.missingScienceLayers ?? []).join(', ') || 'none']
      
            
      ] : [
      
            
        ['rule idea', example.canonicalRuleIdea],
      
            
        ['local update', example.localUpdateFunction],
      
            
        ['state vocabulary', (example.stateVocabulary ?? []).join(', ') || 'n/a'],
      
            
        ['neighborhood', example.neighborhood ?? example.neighborhoodDefinition],
      
            
        ['related ocean analogs', (example.relatedOceanAnalogs ?? []).join(', ') || 'n/a']
      ...
      ['not a', example.notA ?? signature?.notA]
          ...
          ${isOceanAnalog ? `<small>${escapeHtml(example.coupledDemoBridgeNote ?? '')}</small>` : '<small>Ocean relevance is conceptual unless an ocean analog and coupled flow/uncertainty demos are used.</small>'}
...
function processMetricExplanationCardHtml(state = {}) {
  ...
  return `
        <div class="cell-inspector-card" data-process-metric-explanation>
          <span>Current Display Metric</span>
          ${metricRows([
            ['display', metric?.metricLabel ?? state.displayModeLabel ?? roiDisplayModeLabel(state.displayMode)],
            ['metric', metric?.metricId ?? 'ruleSupport'],
            ['meaning', metric?.metricCaption ?? explanation.meaning],
            ['rule trigger', explanation.trigger],
            ['what to watch', explanation.watch]
          ])}
          <div class="hud-muted"><strong>What colors mean</strong></div>
          ${legend.length ? `<ul>${legend.map((entry) => `<li><span style="color:${escapeAttr(entry.color ?? '#ffffff')}">${escapeHtml(entry.label ?? entry.id)}</span>: ${escapeHtml(entry.description ?? '')}</li>`).join('')}</ul>` : `<p>${escapeHtml(explanation.meaning)}</p>`}
          <small>${escapeHtml(explanation.note)}</small>
        </div>
  `;
}

function processMetricExplanationForRule(ruleId, example = {}) {
  const oceanNote = example?.exampleType === 'oceanProcessAnalog'
    ? (example.requiresFlowCoupling
      ? 'This is an event/process-layer analog; physical downstream transport belongs in Flow Fields and Coupled Dynamic Sampling Space demos.'
      : example.notA ?? 'This is a simplified process-rule analog, not a calibrated ocean model.')
    : 'The heatmap is rule support or transition structure, not random intensity.';
  return {
    localBirthDeath: {
      meaning: 'Conway-style cells are classified by neighbor count and next transition.',
      trigger: 'A live cell survives with 2 or 3 live neighbors. An inactive cell is born with exactly 3 live neighbors. Otherwise it becomes or remains inactive.',
      watch: 'Click Step Generation and watch neighbor count create births, survival, and deaths.',
      note: 'Neighbor count is deterministic rule support, not survival probability.'
    },
    propagatingFront: {
      meaning: 'Bright boundary cells are under ignition pressure from active or burning neighbors plus source support.',
      trigger: 'Susceptible cells ignite near active fronts; active cells cool or leave a consumed trail.',
      watch: 'Follow the boundary: consumed cells are the trail behind the front, not random low value.',
      note: oceanNote
    },
    diffusiveSpread: {
      meaning: 'Infection pressure is active-neighbor pressure around susceptible cells.',
      trigger: 'Cells near infected/active neighbors have higher spread pressure; recovering cells show recent activity.',
      watch: 'Watch active patches expand and leave recovering cells behind.',
      note: oceanNote
    },
    excitableWave: {
      meaning: 'The active crest moves through susceptible cells, then leaves refractory and recovering phases behind.',
      trigger: 'Susceptible cells excite near active neighbors; refractory cells cannot immediately reactivate.',
      watch: 'Track the wavefront and the recovery tail over one generation at a time.',
      note: oceanNote
    },
    thresholdCascade: {
      meaning: 'High values mean cells are close to threshold or actively cascading.',
      trigger: 'Load plus neighbor pressure pushes cells toward a cascade trigger.',
      watch: 'Look for loaded cells becoming active cascades, then spent/recovering cells.',
      note: oceanNote
    },
    congestionWave: {
      meaning: 'High values mean density or jam pressure, not generic sample value.',
      trigger: 'Moving density becomes blocked when downstream cells are occupied or congested.',
      watch: 'Follow blocked fronts and release waves.',
      note: oceanNote
    },
    structuredSignal: {
      meaning: 'Color distinguishes conductor path, signal head, and refractory tail.',
      trigger: 'Conductor cells next to signal heads activate next; signal heads become refractory tails.',
      watch: 'Step once and follow the signal along the conductor path.',
      note: oceanNote
    }
  }[ruleId] ?? {
    meaning: 'Rule metric layer for the selected deterministic process.',
    trigger: 'Cells update from prior state, local neighbors, source support, and rule parameters.',
    watch: 'Step one generation and compare rule support with the next state.',
    note: oceanNote
  };
}
...
function currentLabStateRows(state = {}) {
  ...
  if (state.patternSource === 'referenceSignature' && state.referenceSignature) {
    return [
      ...
      ['Mode', state.processModeLabel ?? 'Foundational CA Models'],
      ...
      ['Requires Flow Coupling', state.requiresFlowCoupling === null || state.requiresFlowCoupling === undefined ? 'N/A' : state.requiresFlowCoupling ? 'yes' : 'no'],
...
function currentLabMetricRows(state = {}) {
  ...
  return [
    ...(state.usesDiscreteProcessClock ? [
      ['Generation', state.processGenerationIndex ?? 0],
      ['Tick Rate', `${state.processTickRate ?? 1} gen/s`],
      ['Metric', state.processDisplayMetric?.metricLabel ?? 'Rule Metric']
    ] : []),
